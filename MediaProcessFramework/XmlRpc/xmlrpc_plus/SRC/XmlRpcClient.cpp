...
XmlRpcClient::execute(const char* method, XmlRpcValue const& params, XmlRpcValue& result)
{
  ...
  result.clear();
  _disp.work(_respTimeout); // Process until exit is called
...
XmlRpcClient::handleEvent(unsigned eventType)
{
  ...
  {
	  int err = XmlRpcSocket::getError(getfd());
	  if (err == WSAEINPROGRESS || err == EAGAIN || err == WSAEWOULDBLOCK || err == WSAEINTR)
	    return (_connectionState == WRITE_REQUEST) 
        ? XmlRpcDispatch::WritableEvent|XmlRpcDispatch::Exception : XmlRpcDispatch::ReadableEvent|XmlRpcDispatch::Exception;
