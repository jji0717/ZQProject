...
static inline bool
nonFatalError(int socket)
{
  int err = XmlRpcSocket::getError(socket);
...
XmlRpcSocket::connect(int fd, std::string& host, int port)
{
  ...
  return result == 0 || nonFatalError(fd);
}
...
// Returns last errno
int 
XmlRpcSocket::getError(int socket)
{
#if defined(_WINDOWS)
	int error =0;
	int size = sizeof(error);
	int wsaerror = WSAGetLastError();
	if (0 != getsockopt(socket, SOL_SOCKET, SO_ERROR, (char*)&error, &size))
...
{
  return getErrorMsg(WSAGetLastError());
}
