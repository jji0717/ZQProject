...
// Revision History:
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/MediaProcessFramework/XmlRpc/RpcWpClient.h $
// 
...
namespace ZQ
{
	namespace rpc
	{
		...
		public:
			...
			///@param uri - XML uri
			RpcClient(const char* host = NULL, int port =0, const char* uri =NULL);

			///destructor
			virtual ~RpcClient();

			///open the association to the server at the specified host/port
			///@param host - The name or IP of the remote host to connect
			///@param port - The port number of the remote host
			///@param uri - XML uri
			bool open(const char* host, int port, const char* uri = NULL);
