...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/MediaProcessFramework/TrickPublisher/MCPSession/McpSessionzz.cpp $
// 
...
McpSession::McpSession(ZQ::common::ThreadPool& pool) : ZQ::common::Thread(pool)
{
	m_uiObject = 0;
	m_dwStatus = MCP_SESSION_NONE;
	wcscpy(_swErrorMessage, L"NOERROR");
...
Remark:         
*/
void McpSession::fatalFailed(const wchar_t* sWhy)
{	
	if (m_pPublisher)	
	{
		glog(Log::L_INFO, _T("%s: fatalFailed, delete publish"), getLogHeader());
        m_pPublisher->close();
		delete m_pPublisher;
		m_pPublisher = NULL;
	}
...
int McpSession::run(void)
{
	...
	if (iSubCount <=0)
	{
		wsprintf(buf, L"%s: No subscriber, publish failed.", getLogHeader());
		fatalFailed(buf);
		return 1;
	}
	...
	{
		wsprintf(buf,L"%s: Publisher(%s) open Failed.[%s] ", getLogHeader(), m_pPublisher->m_sAddr.c_str(), my_MCPStatusAsStr(status));
		glog(Log::L_ERROR, buf);
	...
	if(status != MCP_EX_NONE )
	{		
		wsprintf(buf,L"%s: MCPSession: Publisher bind Failed.[%s] ", getLogHeader(), my_MCPStatusAsStr(status));
	...
	if(status != MCP_EX_NONE )
	{	
		wsprintf(buf,L"%s: McpSession: Publisher setPrivateData Failed.[%s] ", getLogHeader(), my_MCPStatusAsStr(status));
	...
	{		
		...
		if(status != MCP_EX_NONE)
		{
			wsprintf(_swErrorMessage, L"%s: Open subscribe %s Failed.[%s] ", getLogHeader(), m_SubVector[i]->m_sAddr.c_str(),my_MCPStatusAsStr(status));
		...
		if (status != MCP_EX_NONE)
		{
			swprintf(_swErrorMessage, L"%s: bind subscribe %s Failed.[%s]", getLogHeader(), m_SubVector[i]->m_sAddr.c_str(), my_MCPStatusAsStr(status));
			glog(Log::L_ERROR, _swErrorMessage);
		...
		if(status != MCP_EX_NONE)
		{
			swprintf(_swErrorMessage, L"%s: subscribe %s subscribe() failed.[%s] ", getLogHeader(), m_SubVector[i]->m_sAddr.c_str(), my_MCPStatusAsStr(status));
	...
	{
		//added by salien
		OutputDebugString(there is a execption in subscrib init.\n");
	...
	if(status != MCP_EX_NONE)
	{
		wsprintf(buf, L"%s: Publish() error.[%s]!", getLogHeader(), my_MCPStatusAsStr(status));
		glog(Log::L_ERROR, buf);
	...
	while(_bKeepPolling)
	{
		...
		if(( MCP_DONE == pubStatus))
		{
			...
			while(_bKeepPolling)
			{
				...
				{	
					...
					else
					{
						...
						swprintf(_swErrorMessage, L"%s: MCPSession: Publish Done,but all Subscribe failed.", getLogHeader());
					}
			...
			{
				//added by salien
				OutputDebugString(there is a unknown excpetion in run. 1 in \"while\" \n");	
			...
			if (nDone != iSubCount)
			{
				...
				else
				{
					m_dwStatus = MCP_SESSION_FAILED;
					glog(Log::L_ERROR, _T("%s: MCP session exit with all subscribe failed"), getLogHeader());
					swprintf(_swErrorMessage, L"%s: Publish Done,but all Subscribe failed.", getLogHeader());
				}
			}
		...
		{	
			swprintf(buf, _T("%s: exit with publisher failed."), getLogHeader());
		...
		{		
			...
			for(unsigned int i = 0; i < iSubCount; i++)
			{
				...
				if( pSub->getCurStatus() == MCP_FAILED)	
				{
			...
			if(nDone == iSubCount)
			{
				swprintf(buf, _T("%s: All the subscriber failed."), getLogHeader());
				glog(Log::L_ERROR, buf);
...
bool waitForStatusChange(DWORD timeout)
{
	if (timeout<)
	if(_hStatus != NULL)

	if (OBJ_) == ::WaitForSingleObject(_hStatus, timeout)
		return true;
	return false;
}
