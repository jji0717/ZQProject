...
int
XmlRpcSocket::socket(int iSockType)
{
  ...
  SOCKET s=::socket(AF_INET, iSockType, 0);
...
bool XmlRpcSocket::bind(int fd, int port, const char* strIp)
{
...
{
  const int READ_SIZE = 8192;   // Number of bytes to attempt to read at a time
...
// Read available text from the specified socket. Returns false on error.
bool 
XmlRpcSocket::nbReadFrom(int fd, std::string& s, bool *eof, sockaddr *from, int *fromlen)
{
  const int READ_SIZE = 8192;   // Number of bytes to attempt to read at a time
  char readBuf[READ_SIZE];
  ...
  *eof = false;
  ...
  int n = recvfrom(fd, readBuf, READ_SIZE-1, 0, from, fromlen);
  XmlRpcUtil::log(5, "XmlRpcSocket::nbReadFrom: recvfrom returned %d.", n);
...
// Write text to the specified socket. Returns false on error.
bool 
XmlRpcSocket::nbWriteTo(int fd, std::string& s, int *bytesSoFar, sockaddr *to, int tolen)
{
  int nToWrite = int(s.length()) - *bytesSoFar;
  char *sp = const_cast<char*>(s.c_str()) + *bytesSoFar;
  bool wouldBlock = false;

  int n = sendto(fd, sp, nToWrite, 0, to, tolen);
  XmlRpcUtil::log(5, "XmlRpcSocket::nbWriteTo: sendto returned %d.", n);
