...
namespace XmlRpc {
  ...
  public:
    ...
    XmlRpcClient(const char* host, int port, const char* uri=0, int type = ST_TCP);
  ...
  protected:
    ...
    virtual void setResponseTimeout(int timeout = -1) // in sec, negative means infinit
		{_respTimeout = (timeout<0)? -1: timeout;}
