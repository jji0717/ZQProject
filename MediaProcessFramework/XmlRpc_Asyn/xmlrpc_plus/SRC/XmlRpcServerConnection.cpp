...
XmlRpcServerConnection::XmlRpcServerConnection(int fd, XmlRpcServer* server, bool deleteOnClose /*= false*/, int type) :
  XmlRpcSource(fd, deleteOnClose, type)
{
...
XmlRpcServerConnection::readHeader()
{
  ...
  bool bRet = false;
  if(isTcp()) {
    bRet = XmlRpcSocket::nbRead(this->getfd(), _header, &eof);
  }
  else if(isUdp()) {
    int fromlen = sizeof(_sockaddr);
    bRet = XmlRpcSocket::nbReadFrom(this->getfd(), _header, &eof, &_sockaddr, &fromlen);
  }
...
XmlRpcServerConnection::writeResponse()
{
  ...
  bool bRet = false;
  if(isTcp()) {
    bRet = XmlRpcSocket::nbWrite(this->getfd(), _response, &_bytesWritten);
  }
  else if(isUdp()) {
    int fromlen = sizeof(_sockaddr);
    bRet = XmlRpcSocket::nbWriteTo(this->getfd(), _response, &_bytesWritten, &_sockaddr, sizeof(_sockaddr));
  }
...
// for udp socket
// loop for udp socket
void XmlRpcServerConnection::loopUdp()
{
  while(1) {
	readHeader();
	readRequest();
