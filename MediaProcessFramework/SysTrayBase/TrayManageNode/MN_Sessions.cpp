...
void MN_Sessions::OnColumnclickListSessions(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	m_sessions.SortItems(ListCompare, (LPARAM)&m_sessions); 
...
void MN_Sessions::OnDblclkListSessions(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	CString strSess(m_sessions.GetItemText(iItemPos,0));
//	ZQ::MPF::utils::URLStr strUrl(strSess);
//	strSess = strUrl.getVar("id");

	CPopSessionDetail *pSessionWindow = new CPopSessionDetail(strSess);
	pSessionWindow->Create(IDD_DLG_SESSIONDETAIL, this);

	// register to get info from thread
	InfoQueryThread *taskDetailThrad = new InfoQueryThread(gHost,gPort, true);
	taskDetailThrad->AddUpdateDlg(pSessionWindow, GETINFO_METHOD);
	taskDetailThrad->SetNotifer(pSessionWindow);
	taskDetailThrad->start();
