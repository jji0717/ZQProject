...
extern int CALLBACK ListCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort);
/////////////////////////////////////////////////////////////////////////////
// MN_WorkNodes dialog
...
void MN_WorkNodes::OnColumnclickListWorknode(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	m_worknode.SortItems(ListCompare, (LPARAM)&m_worknode); 
...
void MN_WorkNodes::UpdateDlgList(RpcValue& _result)
{
	...
	{
		m_worknode.InsertItem(i, (_result[i])[INFO_NODEID_KEY].ToString(buf,1024) );
		m_worknode.SetItemText(i, 1, (_result[i])[INFO_IP_KEY].ToString(buf,1024) );
		m_worknode.SetItemText(i, 2, (_result[i])[INFO_PORT_KEY].ToString(buf,1024) );
		m_worknode.SetItemText(i, 3, (_result[i])[INFO_MPFVERSION_KEY].ToString(buf,1024) );
		m_worknode.SetItemText(i, 4, (_result[i])[INFO_LASTHB_KEY].ToString(buf,1024) );
		m_worknode.SetItemText(i, 5, (_result[i])[INFO_LEASETERM_KEY].ToString(buf,1024) );
		CString strHardware((_result[i])[INFO_HARDWARE_KEY].ToString(buf,1024));
