...
sub main()
{
    ...
    %key = (
	  	"HKLM\\SOFTWARE\\SeaChange\\TelVod\\CurrentVersion\\Services\\SMSVODService" => "HKLM\\SOFTWARE\\SeaChange\\TelVod\\CurrentVersion\\Services\\ModemGateway",
	  	"HKLM\\SOFTWARE\\SeaChange\\TelVod\\CurrentVersion\\Services\\SMSVODService_shell" => "HKLM\\SOFTWARE\\SeaChange\\TelVod\\CurrentVersion\\Services\\ModemGateway_shell",
	  	"HKLM\\SOFTWARE\\SeaChange\\Management\\CurrentVersion\\Services\\SMSVODService" => "HKLM\\SOFTWARE\\SeaChange\\Management\\CurrentVersion\\Services\\ModemGateway",
	  	"HKLM\\SOFTWARE\\SeaChange\\Management\\CurrentVersion\\Services\\SMSVODService_shell" => "HKLM\\SOFTWARE\\SeaChange\\Management\\CurrentVersion\\Services\\ModemGateway_shell",
	  	"HKLM\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\SMSVODService" => "HKLM\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\ModemGateway",
	  	"HKLM\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\SMSVODService_shell" => "HKLM\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\ModemGateway_shell",
	  	"HKLM\\SYSTEM\\CurrentControlSet\\Services\\SMSVODService" => "HKLM\\SYSTEM\\CurrentControlSet\\Services\\ModemGateway",
	 	);  
...
if($installstate)
{
	     ...
	     {
	     	$installs=system ("instserv SMSVODservice status >temp.txt");
	     	$installn=system ("instserv ModemGateway status >>temp.txt");
	        if($installs eq "0")
	        {
	 	while (($key_SMSVODS, $key_ModemG) = each (%key))
	 	{
	 		my $cmd = "reg copy $key_SMSVODS $key_ModemG /s /f";
    ...
    $directory=prompt ("Input the directory for install ModemGateway","C:\\ModemGateway");
    ...
    {
    printf CMDFILE "instserv ModemGateway \"$EXEDIR\"\\srvshell.exe local $SVCACCOUNT $SVCPASSWD auto own\n";
    printf CMDFILE "instserv ModemGateway displayname=\"ModemGateway\"\n";
    }
    if($installs eq "0")
    {
    printf CMDFILE "instserv SMSVODService remove\n";
   # printf CMDFILE "if exist \"%ITVEXEROOT%\"\\srvshell.exe instserv ModemGateway \"%ITVEXEROOT%\"\\srvshell.exe local $SVCACCOUNT $SVCPASSWD auto own\n";
    printf CMDFILE "instserv ModemGateway \"$EXEDIR\"\\srvshell.exe local $SVCACCOUNT $SVCPASSWD auto own\n";
    printf CMDFILE "instserv ModemGateway displayname=\"ModemGateway\"\n";
    }
    ...
    if ($installstate eq "U")
    {
    	while (($key_SMSVOD, $key_Modem) = each (%key))
	 	{
	 		printf CMDFILE "reg delete $key_SMSVOD /f 1>>temp.txt 2>>error.txt\n";
	 	}
    }
