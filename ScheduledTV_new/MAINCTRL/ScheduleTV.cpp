...
// Revision History:
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/ScheduledTV_new/MAINCTRL/ScheduleTV.cpp $
// 
...
//	pGlog = new ScLog(m_wszLogPath, Log::L_DEBUG, 4*1024*1024);
	...
	try
	{
		...
		_pTheRtspMan = new RtspConnectionManager(30, (bool)m_dwRtspTrace);
		if(m_dwRtspTrace>=2)
		{
			RtspDaemon::setTrace(true);
		}
...
{
	int err;
	err = _pThePlaylistMan->OnGetAElist(chnlID, OutAEList);
	if(err!=STVSUCCESS)
	{
		glog(ZQ::common::Log::L_DEBUG, "ScheduleTV::OnGetAElist()  Can not get AEList on channel %06ld : %s", chnlID, GetSTVErrorDesc(err));
	}
	return err;
}
...
{
	int err;
	err = _pThePlaylistMan->OnFreeAElist(chnlID);
	if(err!=STVSUCCESS)
	{
		glog(ZQ::common::Log::L_DEBUG, "ScheduleTV::OnGetAElist()  Can not free AEList on channel %06ld : %s", chnlID, GetSTVErrorDesc(err));
	}
	return err;
}
...
HRESULT ScheduleTV::OnSendAEStatus(DWORD chnlID, SSAENotification& status)
{
	...
	if(err!=STVSUCCESS) 
	{
		glog(ZQ::common::Log::L_DEBUG, "ScheduleTV::OnSendAEStatus()  Can not update AE status on channel %06ld : %s", chnlID, GetSTVErrorDesc(err));
		return err;
	}
...
HRESULT ScheduleTV::OnStartStream(DWORD ChnlID, const char* schNO, int ieIndex)
{
	...
	if(pCh->getLastFeedback(missingNoti))
	{
		...
		{	// last operation is start, with no stop, so send stop
			...
			OnSendAEStatus(ChnlID, missingNoti);
		}
	}
