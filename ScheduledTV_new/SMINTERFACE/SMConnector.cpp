...
extern ZQ::common::ScLog*	pGSdTrace;
#define	SDTrace (*pGSdTrace)
...
{	
	...
	while(_bWorking)
	{
		glog(Log::L_INFO, "[%X]SMConnector:run()  bind ip: %s, Connect to ip: %s  port:%d",GetCurrentThreadId(), _sBindIP, _sSMIP, _nPort);
		
		nRet = ConnectSM(_sBindIP, _sSMIP, _nPort);

		int val=100*1024;
		int vallen=4;
		setsockopt(_so, SOL_SOCKET, SO_RCVBUF, (char*)&val, vallen);
		if (nRet != errSuccess)
		{
			switch(nRet) 
			{
		...
		while(_bWorking)
		{
			while(_bWorking)
			{
					...
					else
					{
						...
						if (!_qSendData.empty())
						{
							// get out message
							{
								...
								ZQ::common::MutexGuard	tmpGd(_mutexSendQueue);
				...
				{
					glog(Log::L_ERROR, "SMConnector::run()  got exception while handle pending list : %s!", excp.getString());
				}
				catch (...) 
				{
					glog(Log::L_ERROR, "SMConnector::run()  got unknown exception while handle pending list!");
				}
			}
			...
			try
			{
				...
				if (nRet > 0)
				{
					/// added by Bernie Zhao, to trace socket input
					if(_trace)
					{
						int len=nRet, offset=0;
						do
						{
							char	tmpBuff[896];
							memset(tmpBuff, 0, 896);
							int copylen = min(len, 895);
							strncpy(tmpBuff, _sXmlBuf + nRead +offset, copylen);
							if(offset==0)
								SDTrace(ZQ::common::Log::L_DEBUG, "[SOCKET %d Begin=%ld Byte=%ld]:\n%s", _so, nRead, nRet, tmpBuff);
							else
								SDTrace(ZQ::common::Log::L_DEBUG, "[...]:\n%s", tmpBuff);
							len		-= copylen;
							offset	+= copylen;
						} while(len>0);
						...
						if (pPtr<pEnd)
						{
							*pPtr++ = '\0';		// terminate this string
							...
							if (pBegin[0])
							{
								...
								{
									glog(Log::L_ERROR, "SMConnector::run()  got exception in xmlproc : %s!", excp.getString());
								}
								catch (...) 
								{
									glog(Log::L_ERROR, "SMConnector::run()  got unknown exception in xmlproc!");
								}
							}
			...
			{
				glog(Log::L_ERROR, "SMConnector::run()  got exception while receiving data : %s!", excp.getString());
			}
			catch (...) 
			{
				glog(Log::L_ERROR, "SMConnector::run()  got unknown exception while receiving data!");
			}
		}
...
void CSMConnector::SendStatusFeedback(const char* sContent)
{
	...
	memset(sBuf, 0, MAX_SEND_XML_BUF);
