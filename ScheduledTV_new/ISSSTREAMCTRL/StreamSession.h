...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/ScheduledTV_new/ISSSTREAMCTRL/StreamSession.h $
// 
...
public:
	/// Initialize the ISSAPI to prepare for the comunication between AS and SS.
	BOOL	Initialize(const wchar_t* wchXmlPath);
...
public:
	...
	// these following process the idetified type CallBack
...
private:
	/// the asychronization API processing result returned.
	static void IssComplCallBack(LPITVSTATUSBLOCK pItvSb);
	/// the call back dispather
