...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/ScheduledTV_new/ISSSTREAMCTRL/StreamData.h $
// 
...
public:
	bool IsValidFirstAE(DWORD dwPreEUID);
...
private:
	...
	vector<CStreamData>::iterator QueryBySID(const STREAMID& sid);

	/// Query the CStreamData object by Purchase ID
	/// @param sid - the Purchase ID
	/// @return - the iterator of the specified CStreamData object.
	vector<CStreamData>::iterator QueryByPurchaseID(DWORD dwPurchaseID);

	/// the list to store the CStreamData objects.
	vector<CStreamData> m_lstStreamDatas;

	/// lock for stream data list
