...
#ifdef _DEBUG
	#pragma comment(lib, "SysMonLib_d.lib")
#else
	#pragma comment(lib, "SysMonLib.lib")
#endif
...
DWORD BaseSchangeServiceApplication::m_dwLogFileSize = SCREPORTER_DEFAULT_BUFSIZE;
DWORD BaseSchangeServiceApplication::m_dwLogBufferSize = SCREPORTER_DEFAULT_FILESIZE;
...
HRESULT BaseSchangeServiceApplication::init(void)
{
	...
	DWORD dwError = 0;
	MANSTATUS manStatus = ManOpenSession((LPCTSTR) m_sServiceName.c_str(), m_dwPortNumber, &m_hManPkg, &dwError);
...
//	getConfigValue(_T("Type"),&m_typeinst->s.dwType,m_typeinst->s.dwType,true,true);
	...
	int sysmonStatus = m_pSysMon->Initialize(*m_typeinst,m_pCIB,0,NULL,SysMonCallBack,m_hCfg);
	if (sysmonStatus != SYSMON_SUCCESS && sysmonStatus != SYSMON_ALREADY_INITIALIZED)
	{
	...
	// Resolve the string node name in FQDN or dotted notation to address in DWORD(NT byte order) 

	DWORD dwIPAddr=INADDR_NONE;
	int dwTcpErrCode;
	m_pCIB->PrimaryIp = INADDR_NONE;
	m_pCIB->SecondaryIp = INADDR_NONE;	
	if(MtCommResolveNodeName(host->h_addr_list[0], &dwIPAddr, &dwTcpErrCode) == TCPSTATE_SUCCESS)
	{
		m_pCIB->PrimaryIp = dwIPAddr;
	}		
	if(MtCommResolveNodeName(host->h_addr_list[1], &dwIPAddr, &dwTcpErrCode) == TCPSTATE_SUCCESS)
	{
...
HRESULT BaseSchangeServiceApplication::getConfigValue(wchar_t*attr_name,DWORD* value,DWORD defaultVal,bool bReport,bool bMan)
{
	DWORD dwType = 0;
	...
	{		
		DWORD dwError = 0;
		MANSTATUS ManStatus =  ManManageVar(m_hManPkg, attr_name, MAN_INT, (DWORD)value, TRUE, &dwError);	
		if(ManStatus != MAN_SUCCESS)
		{
			memset(buf,0,512);
...
HRESULT BaseSchangeServiceApplication::getConfigValue(wchar_t*attr_name,wchar_t* value,wchar_t* defaultVal,DWORD *dwSize,bool bReport,bool bMan)
{
	DWORD dwType = 0;
	HRESULT hr = S_OK;
	CFGSTATUS  status = CFG_GET_VALUE(m_hCfg,(WCHAR*)attr_name,(BYTE*)value,dwSize,&dwType);	
	...
	{		
		DWORD dwError = 0;
		MANSTATUS manStatus = ManManageVar(m_hManPkg, attr_name, MAN_STR, (DWORD)value, TRUE, &dwError);	
		if(manStatus != MAN_SUCCESS )
		{
			memset(buf,0,512);
...
HRESULT BaseSchangeServiceApplication::getConfigValue(wchar_t*attr_name,bool* value,bool defaultVal,bool bReport,bool bMan)
{
	HRESULT hr = S_OK;
	DWORD dwType = 0;
	...
	{		
		DWORD dwError = 0;
		MANSTATUS manstatus = ManManageVar(m_hManPkg, attr_name, MAN_BOOL, (DWORD)value, TRUE, &dwError);	
		if (manstatus != MAN_SUCCESS)
		{
			memset(buf,0,512);
			wsprintf(buf,_T("Error when set manual manage variable %s"),attr_name);		
