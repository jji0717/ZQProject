...
void JmsMsgSender::SetLastLostTime(long lTime)
{
	...
	glog(Log::L_DEBUG,"Last connection lost time is %ld",m_lConnectionLostTime);
}
...
bool JmsMsgSender::init(InitInfo& initInfo, const char* szSessionName)
{
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrieve jms naming context");
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrive jms server IP and Port");
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrieve jms destination name");
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrieve jms connection factory");
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrieve jms re-connect count,set it to default =5");
		m_iReconnectCount=5;
	}
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrieve jms re-connect interval,set it to default=10*1000");
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrieve storage file path,set it to c:\\storageFile.txt");
		m_strMsgStoreFile="c:\\storageFile.txt";
	}
	if(m_strMsgStoreFile.empty())
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init(): storage file path is empty,set tit to c:\\storageFile.txt");
		m_strMsgStoreFile="c:\\storageFile.txt";
	}
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrieve flush count,set it to 100");
	...
	{
		glog(Log::L_DEBUG,"JmsMsgSender::init():error when retrieve keep alive time,set it to 0");
		lKeepAliveTime=0;
	}
...
bool JmsMsgSender::InitializeJMS()
{
	...
	if(!m_pJmsContext )
	{
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::InitilizeJMS()::can't initialize JmsContext with server address is %s and server namingContext is %s and ErrCode is %d",m_strServerAddress.c_str(),m_strNamingContext.c_str(),getLastJmsError());
		return false;
	}
	...
	if( !m_pJmsContext->_context)
	{
		...
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::InitilizeJMS()::can't initialize JmsContext with server address is %s and server namingContext is %s",m_strServerAddress.c_str(),m_strNamingContext.c_str());
		return false;
	}
...
bool JmsMsgSender::ConnectToServer()
{
	...
	{		
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::ConnetToHost()::can't create connnectionfactory with factory=%s And errCode=%d","ConnectionFactory",getLastJmsError());
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::ConnetToHost()::Can't create connection using JMSConnectionFactory and ErrCode=%d",getLastJmsError());
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::ConnetToHost()::Can't create session using JMSConnection and ErrCode=%d",getLastJmsError());
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::ConnetToHost()::Can't create Destination with destinaition name=%s and ErrCode=%d",m_strDestinationName.c_str(),getLastJmsError());
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::ConnetToHost()::Can't create producer using JMS destination and ErrCode=%d",getLastJmsError());
		return false;
	}

	if(!m_JmsConnection.start())
	{
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::ConnetToHost()::JMSCOnnection start fail and errCode=%d",getLastJmsError());
		return false;
	}
	
	if(!m_JmsSession.textMessageCreate("",m_JmsTxtMessage))
	{
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::ConnetToHost()::Create text message fail and ErrCode=%d",getLastJmsError());
		return false;
	}
	if(!CreateMessageProperty())
	{
		glog(Log::L_DEBUG,"BOOL JmsMsgSender::ConnetToHost()::## Can't create message property");
		return false;
	}
...
bool JmsMsgSender::CreateMessageProperty()
{
	...
	{
		glog(Log::L_DEBUG,"bool JmsMsgSender::CreateMessageProperty()## text message instance is not availble");
		return false;
	}
	InitInfo	ini;
	if(!ini.init(m_strIniFilePath.c_str()))
	{
		glog(Log::L_DEBUG,"bool JmsMsgSender::CreateMessageProperty()## can't init InitInfo with file path =%s",m_strIniFilePath.c_str());
		return false;
	}
	...
	{
		glog(Log::L_DEBUG,"bool JmsMsgSender::CreateMessageProperty()## can't get property count");
		return false;
	}	
...
#define		VALUE_DESC		"value"	
			...
			{
				glog(Log::L_DEBUG,"None support type %s",strType.c_str());
				return false;
			}
#undef	VALUE_DESC	
		}
...
{
	glog(Log::L_DEBUG,"VOID JmsMsgSender::ConnectionMonitor()##Some exception was thowed out,Maybe network issue!");
	switch(errType)
	{
	case 0:
	...
	else
	{
		...
		for(it=pMessage->begin();it!=pMessage->end();it++)
		{
			if(!stricmp(it->key.c_str(), KD_KN_OUTPUT))
			{
				if(!m_bConnectionOK)
				{
					glog(Log::L_DEBUG,"void JmsMsgSender::OnMessage()## connection is not available");
				...
				{
					glog(Log::L_DEBUG,"void JmsMsgSender::OnMessage()## can't send the message,So push it into none send message queue");
...
#define		BUF_SIZE	4097
			...
			{
				glog(Log::L_DEBUG,"bool JmsMsgSender::HasNoneSendMessage()##Delete read log fail");				
			}
			return true;
#undef		BUF_SIZE
		}
...
bool JmsMsgSender::HasNoneSendMessageInFile()
{
	...
	if (INVALID_HANDLE_VALUE==hFile)
	{
		glog(Log::L_DEBUG, "bool JmsMsgSender::StoreNoneSendMessage():Fail to open file %s and errcode is %d",m_strMsgStoreFile.c_str(),GetLastError());
		return false;
	}
	else
	{
...
bool JmsMsgSender::InternalSendMessage(std::string strMsg)
{
	...
	if(!m_bConnectionOK)
	{
		glog(Log::L_DEBUG,"JmsMsgSender::InternalSendMessage()##connection lost.");
		return false;
	}	
	if(!m_JmsProducer.send(&m_JmsTxtMessage,&m_MsgOption))
	{
		glog(Log::L_DEBUG,"JmsMsgSender::InternalSendMessage():can't send message");		
		return false;				
	}
...
bool JmsMsgSender::AddLogContent(std::deque<std::string>& vContent,bool bFront)
{
	...
	else
	{
		...
		{
			CloseHandle(hFile);
			glog(Log::L_DEBUG,"bool JmsMsgSender::AddLogContent()## can't open temp file %s",strNewFile.c_str());
			return false;
		}
		...
		if(!CopyFileContent(hFile,0,hNewFile,pos))
		{
			...
			CloseHandle(hFile);
			glog(Log::L_DEBUG,"bool JmsMsgSender::AddLogContent()## copy file content fail");
			return false;
		}
		CloseHandle(hNewFile);
		CloseHandle(hFile);
		...
		{
			glog(Log::L_DEBUG,"bool JmsMsgSender::AddLogContent()## rename from %s to %s fail and errno=%d",strNewFile.c_str(),m_strMsgStoreFile.c_str(),errno);
			return false;
		}
...
bool JmsMsgSender::DeleteLogContent(long pos,bool bFront)
{
	...
	if(hNewFile==INVALID_HANDLE_VALUE)
	{
		glog(Log::L_DEBUG,"bool JmsMsgSender::DeleteLogContent()## can't open file %s",strNewFile.c_str());
		return false;
	}
	SetFilePointer(hNewFile,0,0,FILE_BEGIN);
	SetEndOfFile(hNewFile);
	if(bFront)
	{
		if(!CopyFileContent(hFile,pos,hNewFile,0))
		{
			...
			glog(Log::L_DEBUG,"bool JmsMsgSender::DeleteLogContent()## copy file content fail");
			return false;
		}
		CloseHandle(hFile);
		CloseHandle(hNewFile);
		...
		if(0!=rename(strNewFile.c_str(),m_strMsgStoreFile.c_str()))
		{
			glog(Log::L_DEBUG,"bool JmsMsgSender::DeleteLogContent()##rename from %s to %s fail and errno=%d",strNewFile.c_str(),m_strMsgStoreFile.c_str(),errno);
			return false;
		}
	}
	else
...
bool JmsMsgSender::CopyFileContent(HANDLE hSrc,long posSrc,HANDLE hDst,long posDst)
{
	...
	{
		glog(Log::L_DEBUG,"bool JmsMsgSender::CopyFileContent()## invalid handle pass in");
		return false;
	}
...
#define		BUF_SIZE	4097
	...
	do 
	{
		...
		{		
			...
			{
				glog(Log::L_DEBUG,"bool JmsMsgSender::CopyFileContent()## read file content fail");
				return false;
			}
			if(!WriteFile(hDst,szBuf,lRead,&lWrite,NULL))
			{
				glog(Log::L_DEBUG,"bool JmsMsgSender::CopyFileContent()## write file content fail");
				return false;
			}
		}
		catch (...)  
