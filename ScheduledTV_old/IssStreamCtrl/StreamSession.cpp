...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/ScheduledTV_old/IssStreamCtrl/StreamSession.cpp $
// 
...
// 
// 21    05-01-19 22:02 Ken.qian
// 
// 20    05-01-19 16:59 Bernie.zhao
// fixed a log bug
// 
// 19    05-01-19 16:26 Ken.qian
// 
// 18    05-01-07 14:43 Ken.qian
// 
// 17    04-12-23 17:50 Ken.qian
// 
// 16    04-12-16 14:56 Bernie.zhao
// 
// 15    04-12-02 20:32 Ken.qian
// 
// 14    04-12-01 17:38 Ken.qian
// 
// 13    04-11-29 14:46 Ken.qian
// 
// 12    04-11-26 17:04 Bernie.zhao
// 
// 11    04-11-24 17:27 Bernie.zhao
// 
// 10    04-11-24 14:21 Ken.qian
// 
// 1     04-11-24 11:43 Ken.qian
// 
// 9     04-11-22 17:07 Bernie.zhao
// 
// 8     04-11-17 14:42 Bernie.zhao
// 
// 3     04-10-14 18:08 Ken.qian
// 
// 2     04-10-12 9:54 Ken.qian
// 
...
BOOL CStreamSession::Initialize(const wchar_t* wchXmlPath)
{
	...
	// initialize the ISS
	glog(Log::L_INFO, _T("IssInit with parameter: TypeInst.type %x, TypeInst.Inst %d, dwAppUid %d, CallBackMask %d, StateChangeMask %d"),
	...
	{
		glog(Log::L_ERROR, _T("CStreamSession::Initialize() Fail! Status = %x"), status);
		return FALSE;
	}

	glog(Log::L_INFO, _T("CStreamSession::Initialize() successfully"));
	return TRUE;
}
...
BOOL CStreamSession::UnInitialize()
{
	...
	if(ITV_SUCCESS == sts)
		glog(Log::L_INFO, ("CStreamSession::UnInitialize() Succeed"));
										  ...
										  CALLBACKTYPE CbType, LPCBKBLOCK	pCbBlk)
{
	...
	case CB_NEWSTREAM:
		glog(Log::L_INFO, _T("SID=%x:\t Enter CStreamSession::IssCallBackDispather, CALLBACK --- TYPE:%d(%s)"), 
...
BOOL CStreamSession::OnNewStream(STREAMID sid, LPNEWSTREAM pNewStream)
{
	...
	glog(Log::L_DEBUG, _T("SID=%x:\t SS -> AS: AssetId = %x, PurchaseID = %d, NodeGroupID = %d, NewStreamType = %d, CurrentState = %d"), 
	...
	case START:
		{
			...
			{
				glog(Log::L_DEBUG, _T("SID=%x:\t Cannot get any UserParam, cosz pUserParam == NULL"), sid.dwStreamIdNumber);
			}
			else
			{
...
*/
			...
			glog(Log::L_DEBUG, _T("SID=%x:\t PURCHASEID(DEVID)=%d"), sid.dwStreamIdNumber, dwPurchaseId);

			
			glog(Log::L_DEBUG, _T("SID=%x:\t AS -> SS: Began AcceptStream Operation"), sid.dwStreamIdNumber);
...
//				CStreamData newStreamData(dwPurchaseId, sid, pNewStream->dwAssetId);
				...
				CStreamData newStreamData(dwHomeId, sid, pNewStream->dwAssetId);
				
				m_streamDatas.Add(newStreamData);

				glog(Log::L_DEBUG, _T("SID=%x:\t AcceptStream Operation SUCCEED"), sid.dwStreamIdNumber);
			}
			else
				glog(Log::L_DEBUG, _T("SID=%x:\t AcceptStream Operation FAILED"), sid.dwStreamIdNumber);
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t SS -> AS: OnAssetReady"), sid.dwStreamIdNumber);

	// log the original AEList
	glog(Log::L_DEBUG, _T("SID=%x:\t Original AEList returned by Ready Asset, AssetUID = %x, AECount = %d"), 
	...
	{
		glog(Log::L_DEBUG, _T("SID=%x:\t NO %d Element: AEUID = %x"), 
			sid.dwStreamIdNumber, i+1, (*pOrgAEs)[i].AEUID);
	}
		
	glog(Log::L_DEBUG, _T("SID=%x:\t AS -> SS: Began AcceptReadyAsset Operation"), sid.dwStreamIdNumber);
	...
	{
		glog(Log::L_DEBUG, _T("SID=%x:\t Cannot get Asset Elements according to PurchaseID = %d, stream will be broadcasting dummy asset"), dwPurchaseID);
...
//		pOriginalAEList->AELlist = NULL;
	...
	else 
	{
		...
		if(pCpyAEs != NULL)
		{
			glog(Log::L_DEBUG, _T("SID=%x:\t NEW AEList Count = %d"), sid.dwStreamIdNumber, lNewAECount);

			pOriginalAEList->AECount = lNewAECount;
			pOriginalAEList->AELlist = (PAEARRAY)pCpyAEs;

			// clear the AEIDs from arrray
			...
			for(int i=0; i<lNewAECount; i++)
			{
				...
				glog(Log::L_DEBUG, _T("SID=%x:\t NEW Asset Element ID = %x"), sid.dwStreamIdNumber, pCpyAEs[i].AEUID);
			}
		}
	...
	if(bRet)
		glog(Log::L_DEBUG, _T("SID=%x:\t AS -> SS: AcceptReadyAsset Operation Succeed"), sid.dwStreamIdNumber);
	else {
		if(m_streamDatas.Remove(sid))
		{
			StreamOpCtrl termCtrl(m_instance, sid);
			termCtrl.TerminateStream();
			glog(Log::L_DEBUG, _T("SID=%x:\t AS -> SS : AcceptReadyAsset Operation Failed"), sid.dwStreamIdNumber);
		}
	}
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t SS -> AS: OnAETransition"), sid.dwStreamIdNumber);
	glog(Log::L_DEBUG, _T("SID=%x:\t AE Transiton: PreAEUID = %x, NextAEUID = %x "), 
...
BOOL CStreamSession::OnStateChanged(STREAMID sid, LPSTATECHG stateChg)
{
	...
	{
	case DNETRDY:	
		glog(Log::L_DEBUG, _T("SID=%x:\t CStreamSession::OnStateChanged(), LPSTATECHG::STATECHGTYPE = %d(%s)"), 
...
//			glog(Log::L_DEBUG, _T("SID=%x:\t CStreamSession::OnStateChanged(), TerminateStream After STRMABORT"), 
...
//			gSTV.OnSendAEStatus(abortNoti.dwPurchaseID, abortNoti);
			...
			// Terminate the stream;
			glog(Log::L_DEBUG, _T("SID=%x:\t CStreamSession::OnStateChanged(), TerminateStream After STEAMTERM"), 
				sid.dwStreamIdNumber);
			StreamOpCtrl strmOpCtrl(m_instance, sid);
			strmOpCtrl.TerminateStream();
		}
	...
	case STRMPLAYING:
		break;
	default:
		glog(Log::L_DEBUG, _T("SID=%x:\t CStreamSession::OnStateChanged(), LPSTATECHG::STATECHGTYPE = %d"), 
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t OnLoadChanged"), sid.dwStreamIdNumber);
	return TRUE;
}

BOOL CStreamSession::OnPrimeVideoStore(STREAMID sid, DWORD dwOpSeqnum, OPCTL ExpectedAction)
{
	glog(Log::L_DEBUG, _T("SID=%x:\t Enter OnPrimeVideoStore"), sid.dwStreamIdNumber);
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t Enter OnPlayStream"), sid.dwStreamIdNumber);
	if (SOPCTL_APPROVE == ExpectedAction)
	{
		assert(m_instance != NULL);
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t Enter OnFFStream"), sid.dwStreamIdNumber);
	if (SOPCTL_APPROVE == ExpectedAction)
	{
		assert(m_instance != NULL);
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t Enter OnRewStream"), sid.dwStreamIdNumber);
	if (SOPCTL_APPROVE == ExpectedAction)
	{
		assert(m_instance != NULL);
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t Enter OnPauseStream"), sid.dwStreamIdNumber);
	if (SOPCTL_APPROVE == ExpectedAction)
	{
		assert(m_instance != NULL);
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t Enter OnResumeStream"), sid.dwStreamIdNumber);
	if (SOPCTL_APPROVE == ExpectedAction)
	{
		assert(m_instance != NULL);
...
{
	glog(Log::L_DEBUG, _T("SID=%x:\t Enter OnStopStream"), sid.dwStreamIdNumber);
	if (SOPCTL_APPROVE == ExpectedAction)
	{
		assert(m_instance != NULL);
