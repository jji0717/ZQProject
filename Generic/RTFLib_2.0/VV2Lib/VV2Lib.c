...
#define VV2_EOD_RTE_STRLEN			9
#define VV2_EOD_RTE_STR				"EOD_RTE\r\n"
...
EXPORT int vv2WriteIndexRecord( VV2_INDEX_CONTEXT *pCtxt )
{
	...
	do
	{
		...
		// is this realtime mode?
		if( pCtxt->mode == VV2_INDEX_MODE_REALTIME )
		{
			// yes - attach a temporary end-of-data marker
			if( bufcat( pCtxt, VV2_EOD_RTE_STR ) < 0 )
			{
				PRINTF( "index entry too large in vv2WriteIndexRecord\n" );
				result = -1;
				break;
			}
		}
		...
		if( pCtxt->mode == VV2_INDEX_MODE_REALTIME )
		{
			// yes - indicate that the next write should rewind
			// to just before the temporary end-of-data marker
			pCtxt->nextWriteOffset = -VV2_EOD_RTE_STRLEN;
		}
