// implementation file for rtfCod class MPEG-2 sub-class
...
static RTF_RESULT rtfGetMpeg2StartCodeType( RTF_COD *pCod, unsigned long code, RTF_STARTCODE_TYPE_MPEG2 *pType )
{
...
static RTF_RESULT rtfParseMpeg2SeqHdr( RTF_COD *pCod )
{
	...
	do {		 // error escape wrapper - begin
		...
		result = rtfSesStartSeq( pCod->hSes, FALSE );
		RTF_CHK_RESULT;
		// get the position of the first byte of the start code
		result = rtfWinGetFirstByteInfo( pCod->hWin, &hFirstBytePacket, &firstByteOffset );
		RTF_CHK_RESULT;
		...
		result = rtfWinSmallAdvance( pCod->hWin, 3, &data );
		RTF_CHK_RESULT;
		// extract the vertical and horizontal sizes
		pCod->width  = (unsigned short)( data >> 12 ) & 0x0FFF;
		pCod->height = ( (unsigned short)data & 0xFFF );
		// advance to the byte containing the end of the bitRate field (11th byte)
		result = rtfWinSmallAdvance( pCod->hWin, 4, &data );
		RTF_CHK_RESULT;
		// extract the frame rate code field
		pCod->frameRateCode = (unsigned char)( ( data >> 24 ) & 0x0F );
		// extract the bit rate value field
		pCod->bitRate = ( ( data >> 6 ) & 0x3FFFF ) * 400;
		...
		{
			// yes - advance past it
			result = rtfWinAdvance( pCod->hWin, 64, &data );
			RTF_CHK_RESULT;
		}
...
static RTF_RESULT rtfParseMpeg2ExtHdr( RTF_COD *pCod )
{
	...
	do {		 // error escape wrapper - begin
		...
		result = rtfWinGetFirstByteInfo( pCod->hWin, &hFirstWinPkt, &firstWinOffset );
		RTF_CHK_RESULT;
...
static RTF_RESULT rtfParseMpeg2PicHdr( RTF_COD *pCod, unsigned long code )
{
	...
	do {		 // error escape wrapper - begin
		...
		result = rtfWinSmallAdvance( pCod->hWin, 2, &data );
		RTF_CHK_RESULT;
		...
		if( pictureCodingType == 1 )
		{
			...
			if( picCount != 1 )
			{
				...
				result = rtfSesStartGop( pCod->hSes, TRUE );
				RTF_CHK_RESULT;
			}
		}
		else
...
// MPEG-2 CODEC specific public functions ***********************************************
...
RTF_RESULT rtfCodMpeg2ProcessStartCode( P_RTF_COD pCod, unsigned long code )
{
	...
	do {		 // error escape wrapper - begin
		...
		result = rtfGetMpeg2StartCodeType( pCod, code, &type );
		RTF_CHK_RESULT;
		switch( type )
		{
		case RTF_STARTCODE_TYPE_MPEG2_PICTURE:
			...
			RTF_CHK_RESULT;
			result = rtfParseMpeg2PicHdr( pCod, code );
			RTF_CHK_RESULT;
			break;
		case RTF_STARTCODE_TYPE_MPEG2_GROUP:
			result = rtfSesStartGop( pCod->hSes, FALSE );
			RTF_CHK_RESULT;
			break;
		case RTF_STARTCODE_TYPE_MPEG2_SEQHEADER:
			result = rtfParseMpeg2SeqHdr( pCod );
			RTF_CHK_RESULT;
			break;
		case RTF_STARTCODE_TYPE_MPEG2_SEQEXTENSION:
			result = rtfParseMpeg2ExtHdr( pCod );
			RTF_CHK_RESULT;
			break;
...
void rtfCodMpeg2BuildNCFrames( P_RTF_COD pCod )
{
	...
	bitAppendMpeg2MBAI( pSlice, pCod->width>>4, &pSliceBits );
	...
	// append the necessary macroblock address increment codes
	bitAppendMpeg2MBAI( bSlice, pCod->width>>4, &bSliceBits );
	...
	memcpy(pCod->noChangePFrame, zeroMotionPPesData, pLen);
	bLen = sizeof(zeroMotionBPesData);
	memcpy(pCod->noChangeBFrame, zeroMotionBPesData, bLen);
	// for each verical macroblock (slice)
	for (i = 0 ; i < ( pCod->height + 15 )/16 ; i++)
	{
		// copy in the slice data
		memcpy(pCod->noChangePFrame+pLen, pSlice, pSliceBytes);
		memcpy(pCod->noChangeBFrame+bLen, bSlice, bSliceBytes);
		// adjust the row numbers
		pCod->noChangePFrame[pLen+3] = i + 1;
		pCod->noChangeBFrame[bLen+3] = i + 1;
		// bump the lengths
		pLen += pSliceBytes;
		bLen += bSliceBytes;
	}
	// record the length of the no-change frames
	pCod->noChangePFrameBytes = pLen;
	pCod->noChangeBFrameBytes = bLen;
	// set the valid flags
	pCod->noChangePFrameValid = TRUE;
	pCod->noChangeBFrameValid = TRUE;
}

#endif // #ifdef DO_CODEC_MPEG2
