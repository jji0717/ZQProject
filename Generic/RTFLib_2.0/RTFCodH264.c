// implementation file for rtfCod class H.264 sub-class
...
#define UEV_NEXT_BIT { if( --nextBit < 0 ) { result = rtfFillBitBucket( pCod->hWin, &data, &nextBit ); RTF_CHK_RESULT; } }
...
// NOTE: CONTAINS DSM TRICK MODE FIELD - MASK FOR VVX INDEXER?
...
static void byteAppendH264( unsigned long src, unsigned char *pDst,
						    unsigned char appendByteCount, int *pTotalBitCount )
{
	...
	{
		pDst[ nextByte + i ] = (unsigned char)( src >> shift );
		shift -= 8;
	}
...
static void bitAppendH264( unsigned long src, unsigned char *pDst,
						   unsigned char appendBitCount, int *pTotalBitCount )
{
	int i, totalBitCount;
	int srcBit;
	int dstByte, dstBit;
...
static RTF_RESULT rtfCheckH264ParamSets( RTF_COD *pCod, unsigned long ppsId, BOOL *pNotFound )
{
	RTF_FNAME( "rtfCheckH264ParamSets" );
	RTF_OBASE( pCod );
	RTF_RESULT result = RTF_PASS;
	RTF_CODEC_PPS_H264 *pPPS;
	RTF_CODEC_SPS_H264 *pSPS;
	int i;
...
static RTF_RESULT rtfParseH264AccessUnitDelimiter( RTF_COD *pCod )
{
	RTF_FNAME( "rtfParseH264AccessUnitDelimiter" );
	RTF_OBASE( pCod );
	RTF_RESULT result = RTF_PASS;
	unsigned long data;
	RTF_CODEC_PPT_H264 type;

	do {		 // error escape wrapper - begin

		// get the next byte ( primary_picture_type plus rbsp_trailing_bits() )
		result = rtfWinSmallAdvance( pCod->hWin, 1, &data );
		RTF_CHK_RESULT;
		// parse the primary picture type
		switch( ( ( data >> 5 ) & 0x07 ) )
		{
		case 0:
			type = RTF_CODEC_PPT_H264_I;
			break;
		case 1:
			type = RTF_CODEC_PPT_H264_IP;
			break;
		case 2:
			type = RTF_CODEC_PPT_H264_IPB;
			break;
		case 3:
			type = RTF_CODEC_PPT_H264_SI;
			break;
		case 4:
			type = RTF_CODEC_PPT_H264_SISP;
			break;
		case 5:
			type = RTF_CODEC_PPT_H264_ISI;
			break;
		case 6:
			type = RTF_CODEC_PPT_H264_ISIPSP;
			break;
		case 7:
			type = RTF_CODEC_PPT_H264_ISIPSPB;
			break;
		}
		pCod->codecInfo.h264.ppTyp[ 1 ] = pCod->codecInfo.h264.ppTyp[ 0 ];
		pCod->codecInfo.h264.ppTyp[ 0 ] = type;
		pCod->codecInfo.h264.isKeyframe[ 1 ] = ( type == RTF_CODEC_PPT_H264_I ) ? TRUE : FALSE;
...
static RTF_RESULT rtfParseH264SliceHdr( RTF_COD *pCod, unsigned long code, RTF_STARTCODE_TYPE_H264 type )
{
	RTF_FNAME( "rtfParseH264SliceHdr" );
	RTF_OBASE( pCod );
	RTF_RESULT result = RTF_PASS;
	RTF_CODEC_INFO_H264 *pCodInfo;
	RTF_SEQ_HANDLE hSeq;
	RTF_GOP_HANDLE hGop;
	RTF_PIC_HANDLE hPic;
	...
	do {		 // error escape wrapper - begin
		...
		pCodInfo = &pCod->codecInfo.h264;
		// clone the parsing window so that exploring won't disturb context
		result = rtfWinCopyState( pCod->hWin, pCod->hWinTmp );
		RTF_CHK_RESULT;
		// get the next 4 bytes
		result = rtfWinSmallAdvance( pCod->hWinTmp, 4, &data );
		RTF_CHK_RESULT;
		nextBit = 31;
		// parse the first_mb_in_slice address
		result = rtfUevDecode( pCod->hWinTmp, &data, &nextBit, &ulTemp );
		RTF_CHK_RESULT;
		...
		result = rtfUevDecode( pCod->hWinTmp, &data, &nextBit, &ulTemp );
		RTF_CHK_RESULT;
		pCodInfo->ppsID[ 1 ] = pCodInfo->ppsID[ 0 ];
		pCodInfo->ppsID[ 0 ] = ulTemp;
		// check for the referenced parameter sets
		result = rtfCheckH264ParamSets( pCod, pCodInfo->ppsID[ 0 ], &notFound );
		RTF_CHK_RESULT;
		// do we have them?
		if( notFound == TRUE )
		{
			// no - flush the current content
			result = rtfSesFlush( pCod->hSes );
			RTF_CHK_RESULT;
			break;
		}
		// parse the frame_num
		result = rtfUevDecode( pCod->hWinTmp, &data, &nextBit, &ulTemp );
		RTF_CHK_RESULT;
		pCodInfo->frNum[ 1 ] = pCodInfo->frNum[ 0 ];
		pCodInfo->frNum[ 0 ] = ulTemp;
		// is this a new picture?
		if( ( pCodInfo->frNum[ 0 ] != pCodInfo->frNum[ 1 ] ) ||
			( pCodInfo->ppTyp[ 0 ] != pCodInfo->ppTyp[ 1 ] ) || 
			( pCodInfo->ppsID[ 0 ] != pCodInfo->ppsID[ 1 ] ) ) 
		{
			// yes - advance all of the above indicators so as not to get false positives
			pCodInfo->frNum[ 1 ] = pCodInfo->frNum[ 0 ];
			pCodInfo->ppTyp[ 1 ] = pCodInfo->ppTyp[ 0 ];
			pCodInfo->ppsID[ 1 ] = pCodInfo->ppsID[ 0 ];
			// advance the keyframe flags
			pCodInfo->isKeyframe[ 0 ] = pCodInfo->isKeyframe[ 1 ];
			pCodInfo->isKeyframe[ 1 ] = FALSE;
			// get the current picture info from the session
			result = rtfSesGetCurrentPicInfo( pCod->hSes, &hPic, &picCount );
			RTF_CHK_RESULT;
			// is there a current picture?
			if( hPic != (RTF_PIC_HANDLE)NULL )
			{
				// yes - get the packet array info from the current picture
				result = rtfPicGetPacketArrayInfo( hPic, &pktCount, &phPkt, &firstOff, &lastOff );
				RTF_CHK_RESULT;
				// are there already packets in the current picture?
				if( pktCount > 0 )
				{
					// yes - close the current picture; add it to the current group
					result = rtfSesEndPic( pCod->hSes );
					RTF_CHK_RESULT;
				}
			}
			// is the new frame a keyframe?
			if( pCodInfo->isKeyframe[ 0 ] == TRUE )
			{
				// yes - tell the session to start a new sequence
				// back where we started exploring
				result = rtfSesStartSeq( pCod->hSes, TRUE );
				RTF_CHK_RESULT;
				// record some info in the sequence object
				result = rtfSesGetSequence( pCod->hSes, &hSeq );
				RTF_CHK_RESULT;
				result = rtfSeqSetVideoInfo( hSeq, pCod->width, pCod->height, pCod->bitRate );
				RTF_CHK_RESULT;
			}
			// tell the session to start a new picture
			// back where we started exploring
			result = rtfSesStartPic( pCod->hSes );
			RTF_CHK_RESULT;
			// get the updated current picture info from the session
			result = rtfSesGetCurrentPicInfo( pCod->hSes, &hPic, &picCount );
			RTF_CHK_RESULT;
			// record some info in the new picture object
			result = rtfPicSetH264Info( hPic, pCodInfo->isKeyframe[ 0 ], pCodInfo->ppsID[ 0 ], pCodInfo->frNum[ 0 ] );
			RTF_CHK_RESULT;
			// is this a keyframe?
			if( pCodInfo->isKeyframe[ 0 ] == TRUE )
			{
				// yes - bump the keyframe counter
				RTF_INC_STAT( pCod->totalKeyframeCount );
			}
			else
			{
				// not a keyframe - is this the first picture of a group?
				if( picCount == 1 )
				{
					// yes - get the group info from the session
					result = rtfSesGetCurrentGopInfo( pCod->hSes, &hGop, &gopCount );
					RTF_CHK_RESULT;
			...
			} // if( pCodInfo->isKeyframe[ 0 ] == TRUE ) ; else
		} // if( ( pCodInfo->frNum[ 0 ] != pCodInfo->frNum[ 1 ] ) || ...
...
static RTF_RESULT rtfParseH264SPS( RTF_COD *pCod )
{
	...
	do {		 // error escape wrapper - begin
		...
		// presence of an SPS marks next frame as a keyframe
		pCod->codecInfo.h264.isKeyframe[ 1 ] = TRUE;
		...
		result = rtfWinStartRecord( pCod->hWin, pSPS->pkt, TRANSPORT_MAX_PAYLOAD_BYTES+3 );
		RTF_CHK_RESULT;
		// skip the start code plus the first 3 bytes of the SPS
		// get the 4 bytes beyond that
		result = rtfWinAdvance( pCod->hWin, 7, &data );
		RTF_CHK_RESULT;
		nextBit = 31;
		// parse and record the seq_parameter_set_id field
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &pSPS->spsID );
		RTF_CHK_RESULT;
		// parse the log2_max_frame_num_minus4 field
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &ulTemp );
		RTF_CHK_RESULT;
		pSPS->log2MaxFrameNum = ulTemp + 4;
		// parse the pic_order_cnt_type field
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &pSPS->picOrderCntType );
		RTF_CHK_RESULT;
		...
		else if( pSPS->picOrderCntType == 1 )
		{
			...
			pSPS->deltaPicOrderAlwaysZeroFlag = ( ( data >> nextBit ) & 0x01 );
			UEV_NEXT_BIT;
			// parse the offset_for_non_ref_pic field
			result = rtfSevDecode( pCod->hWin, &data, &nextBit, &lTemp );
			RTF_CHK_RESULT;
		...
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &ulTemp );
		RTF_CHK_RESULT;
		// skip the gaps_in_frame_num_value_allowed_flag field
		UEV_NEXT_BIT;
		...
		pSPS->frameMbsOnlyFlag = ( ( ( data >> nextBit ) & 0x01 ) == 0 ) ? FALSE : TRUE;
		UEV_NEXT_BIT;
		...
		pSPS->picHeightInMbs = ( pCod->codecInfo.h264.fieldPicFlag == 0 ) ? frameHeightInMbs : frameHeightInMbs>>1;
		// record pic height
		...
		{
			// record the mb_adaptive_frame_field_flag
			pSPS->mbAdaptiveFrameFieldFlag = ( ( ( data >> nextBit ) & 0x01 ) == 0 ) ? FALSE : TRUE;
			UEV_NEXT_BIT;
		}
		// set the default frame rate code !!! FIX ME !!! COMPUTE THIS FROM TIME STAMPS? !!!
		pCod->frameRateCode = RTF_DEFAULT_FRAME_RATE_CODE;
		// set the bit rate
		result = rtfSesGetInputBitrate( pCod->hSes, &pCod->bitRate );
		RTF_CHK_RESULT;
		...
		result = rtfWinStopRecord( pCod->hWin, &byteCount );
		RTF_CHK_RESULT;
		...
		pPkt[ 1 ] = (unsigned char)( pCod->vidPid >> 8 );
		pPkt[ 2 ] = (unsigned char)( pCod->vidPid );
		if( adaptLen == 0 )
		{
			pPkt[ 3 ] = 0x10;
		}
...
static RTF_RESULT rtfParseH264PPS( RTF_COD *pCod )
{
	...
	do {		 // error escape wrapper - begin
		...
		// got one. start recording
		result = rtfWinStartRecord( pCod->hWin, pPPS->pkt, TRANSPORT_MAX_PAYLOAD_BYTES+3 );
		RTF_CHK_RESULT;
		// skip the start code - get the 4 bytes beyond that
		result = rtfWinAdvance( pCod->hWin, 4, &data );
		RTF_CHK_RESULT;
		nextBit = 31;
		// parse and record the pic_parameter_set_id field
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &pPPS->ppsID );
		RTF_CHK_RESULT;
		// parse and record the seq_parameter_set_id field
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &pPPS->spsID );
		RTF_CHK_RESULT;
		// record entropy_coding_mode_flag field
		pPPS->entropyCodingModeFlag = ( ( ( data >> nextBit ) & 0x01 ) == 0 ) ? FALSE : TRUE;
		UEV_NEXT_BIT;
		// record pic_order_present_flag field
		pPPS->picOrderPresentFlag = ( ( ( data >> nextBit ) & 0x01 ) == 0 ) ? FALSE : TRUE;
		UEV_NEXT_BIT;
		// record num_slice_groups_minus_1 field
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &pPPS->numSliceGroupsMinus1 );
		RTF_CHK_RESULT;
		if( pPPS->numSliceGroupsMinus1 > 0 )
		{
			// record slice_group_map_type
			result = rtfUevDecode( pCod->hWin, &data, &nextBit, &pPPS->sliceGroupMapType );
			RTF_CHK_RESULT;
			...
			case 5:
				...
				result = rtfUevDecode( pCod->hWin, &data, &nextBit, &pPPS->sliceGroupChangeRate );
				RTF_CHK_RESULT;
				++pPPS->sliceGroupChangeRate;
				break;
			case 6:
				// record pic_size_in_map_units_minus1
				result = rtfUevDecode( pCod->hWin, &data, &nextBit, &picSizeInMapUnitsMinus1 );
				RTF_CHK_RESULT;
				sliceGroupIdBits = 0;
		...
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &pPPS->numRefIdxL0ActiveMinus1 );
		RTF_CHK_RESULT;
		// parse num_ref_idx_l1_active_minus1
		result = rtfUevDecode( pCod->hWin, &data, &nextBit, &ulTemp );
		RTF_CHK_RESULT;
		// record the weighted_pred_flag
		pPPS->weighted_pred_flag = ( data >> nextBit ) & 0x01;
		UEV_NEXT_BIT;
		...
		pPPS->deblockingFilterControlPresentFlag = ( data >> nextBit ) & 0x01;
		UEV_NEXT_BIT;
		// skip constrained_intra_pred_flag
		UEV_NEXT_BIT;
		// record redundant_pic_cnt_present_flag
		pPPS->redundantPicCntPresentFlag = ( ( ( data >> nextBit ) & 0x01 ) == 0 ) ? FALSE : TRUE;
		UEV_NEXT_BIT;
...
static RTF_RESULT rtfGetH264StartCodeType( RTF_COD *pCod, unsigned long code, RTF_STARTCODE_TYPE_H264 *pType )
{
...
static void rtfInitCABAC( P_RTF_COD pCod )
{
	// !!! FIX ME !!! CABAC CODING NOT YET SUPPORTED !!!
}
...
static void rtfGenerateNCSliceHeader( P_RTF_COD pCod, unsigned char *pBuf, unsigned long *pBits )
{
	RTF_CODEC_INFO_H264 *pCodInfo;
	RTF_CODEC_SPS_H264 *pSPS;
	RTF_CODEC_PPS_H264 *pPPS;
	...
	UEV_ENCODE( pCodInfo->ppsID[0], pBuf, pBits );
	// encode current frame number
	bitAppendH264( pCodInfo->frNum[0], pBuf, (unsigned char)pSPS->log2MaxFrameNum, pBits );
	if( pSPS->frameMbsOnlyFlag == FALSE )
	{
		// insert field_pic_flag;
		bitAppendH264( pCodInfo->fieldPicFlag, pBuf, 1, pBits );
		if( pCodInfo->fieldPicFlag != 0 )
		{
			// insert bottom_field_flag;
			bitAppendH264( pCodInfo->bottomFieldFlag, pBuf, 1, pBits );
		}
	}
	...
	if( pSPS->picOrderCntType == 0 )
	{
		...
		bitAppendH264( pCodInfo->picOrderCnt, pBuf, (unsigned char)pSPS->log2MaxPicOrderCntLsb, pBits );
	}
	if( ( pPPS->picOrderPresentFlag != 0 ) && ( pCodInfo->fieldPicFlag == 0 ) )
	{
		// insert the delta_pic_order_cnt_bottom field
		SEV_ENCODE( pCodInfo->deltaPicOrderCntBottom, pBuf, pBits );
	}
	if( ( pSPS->picOrderCntType == TRUE ) && ( pSPS->deltaPicOrderAlwaysZeroFlag == FALSE ) )
	{
		// insert delta_pic_order_cnt[ 0 ] = 0
		SEV_ENCODE( 0, pBuf, pBits );
		if( ( pPPS->picOrderPresentFlag == TRUE ) && ( pCodInfo->fieldPicFlag == FALSE ) )
		{
			// insert delta_pic_order_cnt[ 1 ] = 0
			SEV_ENCODE( 0, pBuf, pBits );
		}
	}
	if( pPPS->redundantPicCntPresentFlag == TRUE )
	{
	...
	bitAppendH264( 0, pBuf, 1, pBits );
	// slice type is P
	if( pPPS->weighted_pred_flag == TRUE )
	{
		// pred_weight_table ********************
	...
	// note: slice type is P
	if( pPPS->entropyCodingModeFlag == TRUE )
	{
	...
	if( pPPS->deblockingFilterControlPresentFlag == TRUE )
	{
...
static void rtfGenerateNCSliceDataNEC( P_RTF_COD pCod, unsigned char *pBuf, unsigned long *pBits )
{
	RTF_CODEC_INFO_H264 *pCodInfo;
	RTF_CODEC_SPS_H264 *pSPS;
	RTF_CODEC_PPS_H264 *pPPS;
...
static void rtfGenerateNCSliceDataEC( P_RTF_COD pCod, unsigned char *pBuf, unsigned long *pBits )
{
	RTF_CODEC_INFO_H264 *pCodInfo;
	RTF_CODEC_SPS_H264 *pSPS;
	RTF_CODEC_PPS_H264 *pPPS;
...
static void rtfGenerateNCSlice( P_RTF_COD pCod, unsigned char *pBuf, unsigned long *pBits )
{
	// generate an NC slice header
	rtfGenerateNCSliceHeader( pCod, pBuf, pBits );
	// generate the slice data
	if( pCod->codecInfo.h264.pps[ 0 ].entropyCodingModeFlag == TRUE )
	{
		rtfGenerateNCSliceDataEC( pCod, pBuf, pBits );
	}
	else
	{
		rtfGenerateNCSliceDataNEC( pCod, pBuf, pBits );
	}
...
// H.264 Codec specific public functions ************************************************

// initialize the H.264 codec
void rtfCodH264Init( P_RTF_COD pCod )
{
...
RTF_RESULT rtfCodH264ProcessStartCode( P_RTF_COD pCod, unsigned long code )
{
...
RTF_RESULT rtfCodH264BuildNCFrame( P_RTF_COD pCod, RTF_CODEC_PPT_H264 pictureType,
								   unsigned char *pBuf, unsigned long *pBufBits )
{
	RTF_FNAME( "rtfCodH264BuildNCFrame" );
	RTF_OBASE( pCod );
	RTF_RESULT result = RTF_PASS;
	void *pDat;
	int len;
	...
	do {		 // error escape wrapper - begin
		...
		if ( ( pictureType == RTF_CODEC_PPT_H264_IPB     ) ||
			 ( pictureType == RTF_CODEC_PPT_H264_ISIPSPB ) )
		{
		...
		memcpy(pCod->noChangePFrame, pDat, len);
		*pBufBits = ( len << 3 );
		// generate a single slice with all skipped macroblocks
		rtfGenerateNCSlice( pCod, pBuf, pBufBits );
		// record the length of the no-change frame and set the valid flag
		switch( pictureType )
		{
		case RTF_CODEC_PPT_H264_IP:
			pCod->noChangePFrameBytes = ( *pBufBits + 7 ) >> 3;
			pCod->noChangePFrameValid = TRUE;
			break;
		case RTF_CODEC_PPT_H264_IPB:
			pCod->noChangeBFrameBytes = ( *pBufBits + 7 ) >> 3;
			pCod->noChangeBFrameValid = TRUE;
			break;
		case RTF_CODEC_PPT_H264_I:
		case RTF_CODEC_PPT_H264_SI:
		case RTF_CODEC_PPT_H264_ISI:
		case RTF_CODEC_PPT_H264_ISIPSP:
		case RTF_CODEC_PPT_H264_ISIPSPB:
		default:
