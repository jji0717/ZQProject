...
#endif
		// is the buffer handle non-null?
		if( pPkt->hBuf != (RTF_BUF_HANDLE)NULL )
		{
			// yes - increment the reference count of the buffer that contains this packet
...
RTF_RESULT rtfPktRemoveReference( RTF_PKT_HANDLE handle, RTF_HANDLE hRefHolder )
{
	...
	do {		 // error escape wrapper - begin
		...
		if( pPkt->hBuf != (RTF_BUF_HANDLE)NULL )
		{
			// yes - decrement the reference count of the buffer that contains this packet
