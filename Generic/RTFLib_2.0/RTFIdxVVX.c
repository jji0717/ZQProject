...
static RTF_RESULT rtfInitFilesVVX( RTF_IDX *pIdx )
{
	...
	do {		 // error escape wrapper - begin
		...
		if( pIdx->pProfile->videoSpec.pid != TRANSPORT_INVALID_PID )
		{
			...
			// !!! FIX ME !!! WHAT HAPPENS IF REGENERATE IS TRUE? !!!
			result = rtfVcdGetNCPFrame( pIdx->hVcd, &pFrame, &pCtxt->lenZeroMotionP, &junkBool );
			RTF_CHK_RESULT;
			memcpy( (void *)pCtxt->zeroMotionP, (void *)pFrame, pCtxt->lenZeroMotionP );
			// get a copy of the no-change B frame from the codec
			result = rtfVcdGetNCBFrame( pIdx->hVcd, &pFrame, &pCtxt->lenZeroMotionB, &junkBool );
			RTF_CHK_RESULT;
		...
		pCtxt->useIndicatorRecords = ( pIdx->mode == RTF_INDEX_MODE_REALTIME ) ? 1 : 0;
...
static RTF_RESULT rtfSetTimeCodeVVX( RTF_IDX *pIdx )
{
	...
	do {
		...
		rtfGopGetTimecode(hGop, &timeCode);

		pTimeCode->hours	= (unsigned char)((timeCode & 0xff000000) >> 24);
		pTimeCode->minutes	= (unsigned char)((timeCode & 0x00ff0000) >> 16);
		pTimeCode->seconds	= (unsigned char)((timeCode & 0x0000ff00) >> 8);
		pTimeCode->frames	= (unsigned char)timeCode;
#if 0
...
RTF_RESULT rtfIdxInitIndexVVX( P_RTF_IDX pIdx )
{
	...
	do {		 // error escape wrapper - begin
		...
		// set the sequence boundary location option
