...
static RTF_RESULT rtfParseMpeg2GopHdr( RTF_VCD *pVcd )
{
	RTF_FNAME( "rtfParseMpeg2GopHdr" );
	RTF_OBASE( pVcd );
	RTF_RESULT result = RTF_PASS;
	unsigned long data;
	...
	do {		 // error escape wrapper - begin
		result = rtfWinSmallAdvance( pVcd->hWin, 4, &data );
		RTF_CHK_RESULT;
		timeCode =	((data & 0x7c000000) >> 2) |		// 5 bits 
					((data & 0x03f00000) >> 4) |		// 6 bits
					((data & 0x0007e000) >> 5) |		// 6 bits
					((data & 0x00001f80) >> 7);			// 6 bits
		result = rtfSesGetCurrentGopInfo( pVcd->hSes, &hGop, &gopCount );
		RTF_CHK_RESULT;
		rtfGopSetTimecode(hGop, timeCode);
		RTF_CHK_RESULT;
...
RTF_RESULT rtfVcdMpeg2ProcessStartCode( P_RTF_VCD pVcd, unsigned long code )
{
	...
	do {		 // error escape wrapper - begin
		...
		case RTF_STARTCODE_TYPE_MPEG2_GROUP:
			...
			RTF_CHK_RESULT;
			result = rtfParseMpeg2GopHdr( pVcd );
			RTF_CHK_RESULT;
