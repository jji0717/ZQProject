...
typedef struct _RTF_SES
{
	...
	// sequence boundary location option
			...
			if( pSes->readyForStreaming != FALSE )
			{
				...
				if( pSes->mainFileBacklogProcessed == FALSE )
				{
					...
					for( i=0; i<pSes->bufCount; ++i )
					{
						...
						{
							// yes - filter the contents of this buffer to the main file copy
				...
				if( bufState == RTF_BUFSTATE_MAPPED )
				{
					...
					if( pSes->indexLibraryIntialized == FALSE )
					{
						...
						if( pSes->profile.videoSpec.pid == TRANSPORT_INVALID_PID )
						{
							...
							RTF_CHK_RESULT;
							// get the sequence boundary location option from the indexer
							result = rtfIdxGetSeqBoundOption( pSes->hIdx, &pSes->seqBoundOption );
							RTF_CHK_RESULT;
...
static RTF_RESULT rtfComputeInputBitRate( RTF_SES *pSes )
{
	...
	do {		 // error escape wrapper - begin
		...
		deltaT  = ( ( pSes->bpsPcrTime[ last ].base.ull * TRANSPORT_SCR_TO_TS_RATIO ) + pSes->bpsPcrTime[ last ].ext.us );
		deltaT -= ( ( pSes->bpsPcrTime[    0 ].base.ull * TRANSPORT_SCR_TO_TS_RATIO ) + pSes->bpsPcrTime[    0 ].ext.us );
		...
		if( pSes->profile.augmentationPidCount == 0 )
		{
			...
			for( i=1; i<last; ++i )
			{
				deltaT = ( ( pSes->bpsPcrTime[ i   ].base.ull * TRANSPORT_SCR_TO_TS_RATIO ) + pSes->bpsPcrTime[ i ].ext.us ) -
						 ( ( pSes->bpsPcrTime[ i-1 ].base.ull * TRANSPORT_SCR_TO_TS_RATIO ) + pSes->bpsPcrTime[ i-1 ].ext.us );
		...
		deltaT -= pSes->bpsPcrTime[ 0 ].base.ull * TRANSPORT_SCR_TO_TS_RATIO;
...
static RTF_RESULT rtfComputeInputFrameRate( RTF_SES *pSes )
{
	...
	do {		 // error escape wrapper - begin
		...
		fpsFix16 = (int)(((INT64)90000 * 65536) / pSes->ptsRateCalc);
...
RTF_RESULT rtfSesSetFrameRateCode( RTF_SES_HANDLE handle, int value )
{
	...
	do {		 // error escape wrapper - begin
		...
		// record this value and set the acquired flag
...
{
	RTF_FNAME( "rtfSesGetSeqBoundOption" );
	RTF_OBASE( handle );
	RTF_RESULT result = RTF_PASS;
	RTF_SES *pSes = (RTF_SES *)handle;
	...
	do {		 // error escape wrapper - begin
		...
		RTF_CHK_OBJ( pSes, RTF_OBJ_TYPE_SES );
		// make the return
		*pSeqBoundOption = pSes->seqBoundOption;
					   ...
					   int numSettings, RTF_APP_OUTPUT_SETTINGS settings[] )
{
	...
	do {		 // error escape wrapper - begin
		...
		// reset the abend flag
		pSes->abendFlag = FALSE;
		...
		pSes->seqBoundOption = RTF_SEQBOUND_OPTION_FIXED;
...
RTF_RESULT rtfSesErrorNotifier( RTF_SES_HANDLE handle, RTF_MSG msg, RTF_OBJ_HANDLE hObj )
{
	...
	do {		 // error escape wrapper - begin
		...
		RTF_CHK_OBJ( pSes, RTF_OBJ_TYPE_SES );
		// have we already responded to this error? (i.e. are we on our way out?)
		if( pSes->abendFlag != FALSE )
		{
			// yes - skip it
			break;
		}
		// no - set the abend flag to avoid recursive error notification
		pSes->abendFlag = TRUE;
		// get the object type
							   ...
							   unsigned char *pBuffer, unsigned long bytes )
{
	...
	do {		 // error escape wrapper - begin
			...
			( pSes->frameRateAcquired == FALSE ) )
		{
			...
			result = rtfBufCaptureFpsInfo( pSes->hBuf[pSes->bufCount-1]);
			RTF_CHK_RESULT;
		}
...
RTF_RESULT rtfSesStartSeq( RTF_SES_HANDLE handle, BOOL isVirtual )
{
	...
	do {		 // error escape wrapper - begin
		...
		RTF_CHK_RESULT;
		pSes->seqCount = 1;
		// does the indexer support flexible sequence boundary setting?
		if( pSes->seqBoundOption == RTF_SEQBOUND_OPTION_FLEXIBLE )
		{
			// yes - get the sequence start info from the video codec
			result = rtfVcdGetSeqStartInfo( pSes->hVcd, &hFirstBytePacket, &firstBytePacketOffset );
			RTF_CHK_RESULT;
			// does it have a valid start point recorded?
			if( hFirstBytePacket == (RTF_PKT_HANDLE)NULL )
			{
				// no - go with the current window start position
				result = rtfWinGetFirstByteInfo( pSes->hWindow, &hFirstBytePacket, &firstBytePacketOffset );
				RTF_CHK_RESULT;
			}
		}
...
RTF_RESULT rtfSyncCheck( RTF_SES *pSes, RTF_AUG_INFO *pInfo )
{
	...
	do {		 // error escape wrapper - begin
		...
		if( pInfo->buf[ pInfo->offset ] != TRANSPORT_PACKET_SYNCBYTE )
		{
			...
			if( ( pInfo->offset + TRANSPORT_PACKET_BYTES ) >= pInfo->bufOccupancy )
			{
				// since the current position is within 1 transport packet of the end of the
				// buffer and 2 successive sync bytes are required to reestablish sync
				// there's insufficient bytes remaining in this buffer for resynchronization
				// so set the current offset at the end of the buffer so another buffer is loaded
				//
				pInfo->offset = pInfo->bufOccupancy;
				break;
			}
...
// rtfSesPesPTS
//
// a new pts was captured from a pes header.  Capture enough pts's
// to determine a frame rate given that pts are not ordered in arrival 
// order i.e. they may decode more than 1 frame prior to the adjacent picture 
// e.g. PBBIBB displays BBPBBI so the difference between the pts of 
// the P and the subsequent B is 12012 tick (ntsc), but the difference 
// between the 2 B's is 3003 (ntsc).
//
// make sure enough entries are polled to cover the 3/2 pulldown case in MPEG2 where 
// pictures marked with repeat first field present for 1 1/2 field times or 4500 ticks.
//
// return scanComplete == TRUE when enough PTS have been collected to determine
// the frame rate
//
RTF_RESULT rtfSesPesPTS( RTF_SES_HANDLE handle, UINT64 pts, BOOL *scanComplete)
{
	RTF_FNAME( "rtfSesPesPTS" );
	RTF_OBASE( handle );
	RTF_RESULT result = RTF_PASS;
	RTF_SES *pSes = (RTF_SES *)handle;
	//
	// pts of -1 indicates a discontinuity so reset pts count and 
	// restart poll for frame rate
	//
	if (pts == (UINT64)-1)
	{
		pSes->ptsCount = 0;
	...
	{
		do {
			pSes->ptsRateTable[pSes->ptsCount] = pts;
			pSes->ptsCount = (++pSes->ptsCount % (sizeof(pSes->ptsRateTable) / sizeof(pSes->ptsRateTable[0])));
			if (pSes->ptsCount == 0)
			{
				int thisRateCalc = INT_MAX;
				int i;
				for (i = 1 ; i < sizeof(pSes->ptsRateTable) / sizeof(pSes->ptsRateTable[0]) ; i++)
				{
					int j;
					//
					// save the lowest difference between any two frame rates polled
					//
					for (j = i ; j < sizeof(pSes->ptsRateTable) / sizeof(pSes->ptsRateTable[0]) ; j++)
					{
						INT64 diff = (INT64)(pSes->ptsRateTable[j] - pSes->ptsRateTable[i]);
						if (diff < 0)
						{
							diff = (INT64)(pSes->ptsRateTable[i] - pSes->ptsRateTable[j]);
						}
						//
						// check for a "lower" lowest
						//
						if (diff > 0 && diff < thisRateCalc)
						{
							// save candiate lowest
							//
							thisRateCalc = (int)diff;
						}
					}
				}
				//
				// call routine to process lowest frame rate
				//
				pSes->ptsRateCalc = thisRateCalc;
				result = rtfComputeInputFrameRate( pSes );
				RTF_CHK_RESULT;
				//
				// complete scan for frame rate
				//
				pSes->frameRateAcquired = TRUE;
				*scanComplete = TRUE;
			}
