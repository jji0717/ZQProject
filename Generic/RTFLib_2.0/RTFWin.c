	...
	do {		 // error escape wrapper - begin
		...
		*pData = TRANSPORT_INVALID_DATA;
		...
		for( ;; )
		{
			...
			data <<= 8;
			pWin->contextIndex = ( pWin->contextIndex-1 ) & RTF_CONTEXT_INDEX_MASK;
			...
			*( pWin->hContextPkt + pWin->contextIndex ) = pWin->hNextBytePkt;
			*( pWin->contextPktOffset + pWin->contextIndex ) = pWin->nextBytePktOffset++;
#ifdef DO_NATIVE64
...
RTF_RESULT rtfWinGetPcrTime( RTF_WIN_HANDLE handle, INT64 *pPcrTime )
{
	...
	do {		 // error escape wrapper - begin
		...
		byteCount *= TRANSPORT_TS_TICKS_PER_SECOND * 8;
