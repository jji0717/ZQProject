...
#define DebugLevel ZQ::common::Log::L_DEBUG
	...
	WatchDog::WatchDog(SrvrLoadEnv& env)
		: _env(env)
		, _bExit(false)
		, _nextWakeup(0)
		, _event(NULL)
	...
	void WatchDog::watch(std::string ident, __int64 milliSec)
	{
		...
		if (milliSec <= 1)
			milliSec = 1;
		__int64 tmNow = _env.now();
		__int64 expiration = milliSec + tmNow;

		ZQ::common::MutexGuard lk(_lock);

		glog(DebugLevel, CLOGFMT(WatchDog, "session(%s) required watch %I64dms"), ident.c_str(), milliSec);
		...
		for (std::list<WatchObject>::iterator lstItor = _watchList.begin(); lstItor != _watchList.end(); lstItor ++)
		{
	...
	{
		glog(DebugLevel, CLOGFMT(WatchDog, "unwatch session(%s)"), ident.c_str());
		ZQ::common::MutexGuard lk(_lock);
	...
	int WatchDog::run()
	{
		while (false == _bExit)
		{
			...
			__int64 sleepTime;
			{
				...
				glog(DebugLevel, CLOGFMT(WatchDog, "%d session under watch now"), _watchMap.size() + _watchList.size());
	...
	void WatchDog::onTimer(const std::vector<std::string>& idents)
	{
		...
		{
		_env._pWatchDog->watch(idents[i], _env._scanInterval);
		
		static __int64 sequence = _env.getWatchListSize() + 1;
		WatchXMLInfo xmlinfo;

		if (false == _env.getXMLInfo(idents[i], xmlinfo))
			continue;

		if (false == _env.needReload(xmlinfo))
			continue;

		if (false == _env.setXMLInfo(xmlinfo))
			continue;

		ParseXMLData* pParseReq = new ParseXMLData(xmlinfo._fileName, _env, *_env._pThreadPool, sequence ++, xmlinfo._properties);
		pParseReq->start();
