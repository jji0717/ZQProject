...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/Generic/JMSCppLib/JMSCpp/MapMessage.h $
// 
...
// Created.
// 
// 3     07-08-23 10:51 Hongquan.zhang
// 
...
public:
   ...
   /// get first key name
   ///@return the first key name if success,NULL if failed
   const char* getFirstKey(char* buf,int size);

   /// get next key name
   ///@return the next key name if success,NULL if failed
   const char* getNextKey(char* buf,int size);

   
private:
	JmsEnumeration* m_pEnumeration;
};
}
}
