...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/Generic/JMSCppLib/JMSCpp/Connection.h $
// 
...
// Created.
// 
// 5     08-01-04 14:28 Ken.qian
// change callback routine prototype
// 
// 4     06-11-30 12:18 Hongquan.zhang
// 
...
/// @param strErr exception error description
typedef void (*ConnExceptionCallBack) (int errType, void * lpData);
typedef void (*ConnExceptionCallBack2) (int errType, const char* errMsg, void * lpData);
...
{
public:
	Connection() : _connection(NULL),_pCallBack(NULL),_pCallBack2(NULL) {};
	~Connection();
	...
	void SetConnectionCallback2(ConnExceptionCallBack2 pCallBack,void* lpData);

	/// Fire the connection exception call back,SYSTEM WILL CALL THIS FUNCTION
	void fireConnCallback(int errType,const std::string& errMsg);
...
public:
	...
	ConnExceptionCallBack2	_pCallBack2;
	void*					_lpData;
