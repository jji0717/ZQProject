...
// --------------------------------------------------------------------------------------------
// Revision History: 
// $Header: /ZQProjs/Generic/ContentProcess/NTFSFileIORender.h 1     10-11-12 15:58 Admin $
// $Log: /ZQProjs/Generic/ContentProcess/NTFSFileIORender.h $
// 
...
protected:
	...
	{
		public:
			SubFile(int fileNo, NTFSFileIORender& ntfsIoRender, Graph& graph, bool pacing=false);
			virtual ~ SubFile();
		...
		public:
			...
			bool                 _pacing;
			bool                 _isIndexFile;
			std::string          _tempIndexFileName;
			const void*          _pacingIndexCtx;
			LARGE_INTEGER        _posIndexWrite;  // pos of index writing by trick generation
			LARGE_INTEGER        _posIndexRead;   // pos of index read for pacing
		    char                 _tmpbuffer[MAX_READ_SIZE];
