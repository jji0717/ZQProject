...
// --------------------------------------------------------------------------------------------
// Revision History: 
// $Header: /ZQProjs/Generic/ContentProcess/GraphFilter.cpp 1     10-11-12 15:58 Admin $
// $Log: /ZQProjs/Generic/ContentProcess/GraphFilter.cpp $
// 
...
//			}
			...
			writeLog(ZQ::common::Log::L_INFO, id(), "Current progress: processed stuff %I64d, total stuff %I64d, %d ms elapsed", 
	...
	while(_bContinue)
	{
		...
		if(pRequest->_stopByEndTime && pRequest->_endTime > tnow)
		{
			...
			writeLog(ZQ::common::Log::L_INFO, id(), 
					"stop time (%I64d.000) current time (%I64d.%d) duration (%ums)", 
				     getEndTime(), 
					 tnow, 
					 st.wMilliseconds, 
					 expectedExecTime);
