...
static struct fuse_opt fuse_ll_opts[] = {
	...
	{ "max_threads=%u", offsetof(struct fuse_ll, max_threads), 0},
...
"    -o max_threads         set max concurrent thread count\n");
}
					      ...
					      size_t op_size, void *userdata)
{
	...
	if (f->debug) {
		...
		fprintf(stderr, "configured max_thread %d\n", f->max_threads);
	}
	
	set_max_threadcount( f->max_threads );
