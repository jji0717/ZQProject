        ...
        if not iscdmi and method in ("get", "delete"):
            if objecttype == "unknown":
                if not io.exists( path ):
                    return self.send_error( 404, "non-cdmi and unknown object type, and  path(%s) does not exist" % (path) )
                if objecttype == "unknown":
                    self.request.objecttype = objecttype = io.objecttype( path)
        if iscdmi and objecttype == "unknown":
            if not io.exists( path ):
                return self.send_error( 404, "cdmi and unknown object type, and path(%s) does not exist"%(path) )
            self.request.objecttype = objecttype = io.objecttype( path)
