...
#    under the License.
"""
export volume and delete export commandline
Typical location:
cinder client:
    /usr/lib/python2.6/site-packages/cinderclient/v1/shell.py
...
MAPPING = {
    ...
    'cinder.volume.driver.ISCSIDriver':
    'cinder.volume.drivers.fuse.FuseISCSIDriver',
...
def locked_snapshot_operation(f):
    ...
    lock is free. This is used to protect concurrent operations on the same
    snapshot e.g. delete SnapA while create volume VolA from SnapA is in progress.
    """
...
class VolumeManager(manager.SchedulerDependentManager):
    ...
    def export_volume(self, context, volume_id):
        ...
        if volume['status'] != "available" and volume['status'] != "export" :
            msg = _("status must be available or export")
            raise exception.InvalidVolume(reason=msg)
        ...
        model_update = None
        try:
            LOG.debug(_("Volume %s: creating export start"), volume_id)
            utils.require_driver_initialized(self.driver)
            model_update = self.driver.create_export(context, volume)
            if model_update:
                volume = self.db.volume_update(context, volume_id,
                                               model_update)
            LOG.debug(_("Volume %s: creating export end"), volume_id)
        except exception.CinderException as ex:
            if model_update:
            ...
            with excutils.save_and_reraise_exception():
                LOG.exception(_("Volume %s: creating export caught exception"), volume_id)
                self.db.volume_update(context, volume_id,
                                      {'status': 'available'})
