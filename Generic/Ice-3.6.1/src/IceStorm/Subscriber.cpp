...
public:
    ...
    /*PerSubscriberPublisherI(const InstancePtr& instance) :
        _instance(instance)
    */
	PerSubscriberPublisherI(const InstancePtr& instance, const std::string& topicName) :
        _instance(instance),
		_tracePtr(_instance->traceLevels()),
               ...
               const Ice::Current& current)
    {
        ...
        {
            std::string current_con = current.con->toString();
            Ice::Trace out(traceLevels->logger, traceLevels->topicCat);
            //if(current.operation != "ping" && current.operation != "post")
            if(current.operation != "ping")
            {
                std::string sender = current_con.substr(current_con.rfind("remote"));
                out << sender << " sent A event: " << " Topic: " << _subscriber->record().topicName << " ReqId: "  << current.requestId << " Operation: " << current.operation;
            }
        }
    ...
    _obj(obj)
{
	...
	try
	{
		_clientName = _instance->communicator()->proxyToString(obj);
    ...
    const SubscriberRecord& rec)
{
    ...
    {
        PerSubscriberPublisherIPtr per = new PerSubscriberPublisherI(instance,rec.topicName);
        Ice::Identity perId;
        ...
        out << "create Subscriber: " << "Category:" << perId.category << "..Name:" << perId.name ;
        return subscriber;
    }
}
...
Subscriber::error(bool dec, const Ice::Exception& e)
{
    ...
    else if(_state < SubscriberStateError)
    {
        ...
        if(traceLevels->subscriber > 0)
        {
            Ice::Trace out(traceLevels->logger, traceLevels->subscriberCat);
            //out << _instance->communicator()->identityToString(_rec.id);
            out << _instance->communicator()->identityToString(_rec.id)<<" client ["
				<< _instance->communicator()->proxyToString(_proxy) <<"]"
...
void Subscriber::traceEventData( const IceStorm::EventDataPtr data )
{
	if( !_traceLevels->traceEventFlush )
		return;
	Ice::Trace out(_traceLevels->logger, _traceLevels->traceEventFlushCat);
	out << "send to ["<<_clientName <<"] event: op["<< data->op<<"]";
}
