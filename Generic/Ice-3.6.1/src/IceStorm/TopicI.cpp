               ...
               const Ice::Current& current)
    {
        ...
        if(traceLevels->topic > 0)
        {
			...
			if(current.operation != "ping")
            {
                ...
                out << sender << " sent A event: " << " Topic: " << _topic->getName() << " ReqId: "  << current.requestId << " Operation: " << current.operation;
            }
        }
        return true;
    }
...
TopicImpl::subscribeAndGetPublisher(const QoS& qos, const Ice::ObjectPrx& obj)
{
    ...
    if(traceLevels->topic > 0)
    {
        ...
        out << "subscribeAndGetPublisher: " << "[" << _name << "] " << obj->ice_toString();
...
TopicImpl::unsubscribe(const Ice::ObjectPrx& subscriber)
{
    ...
    if(traceLevels->topic > 0)
    {
        ...
        out << "unsubscribe: " << "[" <<  _name << "] " << subscriber->ice_toString();
...
{
    if( _traceLevels->traceEventPublish )
    {
        EventDataSeq::const_iterator it = events.begin();
        for( ; it != events.end() ; it ++ )
        {
            const IceStorm::EventDataPtr data = *it;
            Ice::Trace out(_traceLevels->logger, _traceLevels->traceEventPublishCat);
            out << "["<< _name <<"] got event: op["<< data->op<<"]";
        }
    }
    ...
    Ice::IdentitySeq reap;
    {
        ...
        {
            Ice::Trace out(_traceLevels->logger, _traceLevels->traceEventPublishCat);
            out << "Subscriber: " << (*p)->id().name << ".." << (*p)->id().category << " ["<< _name <<"]";
