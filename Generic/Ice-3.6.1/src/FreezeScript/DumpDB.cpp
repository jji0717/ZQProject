    ...
    const FreezeScript::CompactIdResolverIPtr& resolver)
{
    ...
    {
        cerr << __LINE__ << ":" << appName << ": " << e.reason << endl;
        usage(appName);
        return EXIT_FAILURE;
    }
    ...
    if(opts.isSet("c"))
    {
        if(args.empty())
        {
            cerr << __LINE__ << ":" << appName << ": no database environment specified." << endl;
            usage(appName);
            return EXIT_FAILURE;
        }
        else if(args.size() > 2)
        ...
        try
        {
            ...
            else
            {
                ...
                if(p == catalog.end())
                {
                    cerr << __LINE__ << ":" << appName << ": database `" << args[1] << "' not found in environment `" << args[0] << "'."
                         << endl;
                    return EXIT_FAILURE;
                }
                else
        ...
        {
            cerr << __LINE__  << ":" << appName << ": " << ex.reason() << endl;
            return EXIT_FAILURE;
        }
    }
    if(opts.isSet("D"))
    ...
    {
        cerr << __LINE__ << ":" << appName << ": an input file cannot be specified with --select" << endl;
        return EXIT_FAILURE;
    }
    ...
    //
    string descriptors;
    cout << "db info -> " << dbEnvName << " " << dbName << "  input file"<<inputFile<<endl;
    if(inputFile.empty())
    {
        ...
        {
            cerr << __LINE__ << ":" << appName << ": a key type and a value type must be specified" << endl;
            usage(appName);
            return EXIT_FAILURE;
        }
        else if(valueTypeName.empty())
        {
            try
            {
                cout << "CatalogDataMap dbEnvName[" << dbEnvName<<"]"<<endl;
                ...
                if(p == catalog.end())
                {
                    cerr << __LINE__ << ":" << appName << ": database `" << dbName << "' not found in catalog." << endl;
                    cerr << __LINE__ << ":" << "Current catalog databases:" << endl;
                    for(p = catalog.begin(); p != catalog.end(); ++p)
                    {
                        cerr << __LINE__ << ":" << "  " << p->first << endl;
                    }
                    return EXIT_FAILURE;
                }
                else
        ...
        if(l.empty())
        {
            cerr << __LINE__ << ":" << appName << ": unknown key type `" << keyTypeName << "'" << endl;
            return EXIT_FAILURE;
        }
        keyType = l.front();
        ...
        if(l.empty())
        {
            cerr << __LINE__ << ":" << appName << ": unknown value type `" << valueTypeName << "'" << endl;
            return EXIT_FAILURE;
        }
        valueType = l.front();
        ...
        if(!outputFile.empty())
        {
            ...
            if(!of.good())
            {
                cerr << __LINE__ << ":" << appName << ": unable to open file `" << outputFile << "'" << endl;
                return EXIT_FAILURE;
            }
            of << descriptors << endl;
            of.close();
    ...
    try
    {
        ...
        //
        {
            ...
            std::cout << "open db with env[" << dbEnvName <<"]"<<std::endl;
        ...
        try
        {
            ...
            if(evictor)
            {
                ...
                for(vector<string>::iterator p = dbNames.begin(); p != dbNames.end(); ++p)
                {
                    ...
                    Db db(&dbEnv, 0);
					std::cout <<"open db [" << dbName << "] name["<<name <<"]"<<std::endl;
    ...
    {
        cerr << __LINE__ << ":" << appName << ": database error: " << ex.what() << endl;
        status = EXIT_FAILURE;
    }
    catch(...)
    {
...
#endif
{
    ...
    {
        cerr << __LINE__ << ":" << appName << ": " << ex.what() << endl;
        status = EXIT_FAILURE;
    }
    catch(...)
    {
        cerr << __LINE__ << ":" << appName << ": unknown error" << endl;
        return EXIT_FAILURE;
    }
