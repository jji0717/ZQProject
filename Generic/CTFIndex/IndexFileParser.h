...
namespace ZQ
{
    namespace IdxParser
    {
        ...
        class IndexData
        {
            ...
            std::string                         assetIngestTime;
            std::string                         idxcreationtime;
            std::string                         programversion;
            std::string                         assetId;
            std::string                         sourcefilename;
            //	std::string                         filewrappertype;
            ...
            uint32								subFileCount;
            uint32                              videobyterate;
            uint32                              videorateticks;
            uint32                              videoratetimescale;
            uint32                              maxGOPsize;
            uint32                              fixedPTSInterval;
            uint32                              initdecodingdelay;
            uint32                              scte35count;
            //uint64                              mainFileSize;

            uint32                              programnumber;
            uint8                              filewrappertype;
            uint16                              PCRPkgInterval;
            uint8                               transstreamcount;
            uint8                               elemstreamcount;
            uint8                               PTSCalculation;
            uint8                               transportpacketsize;
            uint8                               syncbyteoffset;
            uint8                               transstreamtype;
        ...
        class IndexRecord 
        {
            ...
            uint64	findNextIFrame( uint64& offset, int& rateIdx, float speed = 0.0) const;
...
//          uint32  getPlayTime(float& speed);
            ...
            struct RecordTrickInfo{
                ...
                {
                    return (float)(in1.speed.numerator / in1.speed.denominator)  < (float)(in2.speed.numerator / in2.speed.denominator);
                }
...
#pragma pack(1)
                ...
                typedef struct _CsicoIndexHeaderTag
                {
                    uint32       Tag;
                    uint16       Length;
                }CsicoIndexHeaderTag;
...
#pragma pack()
            ...
            protected:
                ...
                bool			parseSubFiles( FileReader* reader  , IndexData& idxData, uint32 len);

                bool            parseZeroMotion( FileReader* reader, IndexData& idxData, uint32 len );

                bool            parseTransStreamInfo( FileReader* reader, IndexData& idxData, uint32 len);

                bool            parseElemStreamInfo( FileReader* reader, IndexData& idxData, uint32 len);

                bool            parseRandomIndexInfo( FileReader* reader, IndexData& idxData, uint32 len);

                bool            parsePSIInfo( FileReader* reader, IndexData& idxData, uint32 len);

                //bool            parsePSIAssociation( FileReader* reader, IndexData& idxData);

                bool			parseAssetInfo( FileReader* reader , IndexData& idxData, uint32 len );

                bool            parseIdxHeaderInfo( FileReader* reader, IndexData& idxData);

                bool			parseSection(  FileReader* reader , IndexData& idxData , int len);
                ...
                uint32 read3byteInt( FileReader* reader )
                {
                    memset(_tmpBuffer,0,10);
                    if ( reader->read( _tmpBuffer, 3) != 3 )
                    {
                        throw "not enough data";
                    }
                    return *((uint32*)_tmpBuffer);
                }
                uint32 read6byteInt( FileReader* reader )
                {
                    memset(_tmpBuffer,0,10);
                    if ( reader->read( _tmpBuffer, 6) != 6 )
                    {
                        throw "not enough data";
                    }
                    return *((uint32*)_tmpBuffer);
                }
                uint32 read8byteInt( FileReader* reader )
                {
                    memset(_tmpBuffer,0,10);
                    if ( reader->read( _tmpBuffer, 8) != 8 )
                    {
                        throw "not enough data";
                    }
                    return *((uint64*)_tmpBuffer);
