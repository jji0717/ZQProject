...
void rtfInitializeTrickSpecMPEG2Main( RTF_TRICK_SPEC *pSpec )
{
	...
	pSpec->insertPAT					= FALSE;
	pSpec->insertPMT					= FALSE;
	...
	pSpec->ttsDconAccessUnit			= FALSE;
	pSpec->ttsDconPcrPacket				= FALSE;
	pSpec->ttsDconAFPackets				= FALSE;
...
void rtfInitializeTrickSpecMPEG2VVX( RTF_TRICK_SPEC *pSpec )
{
	...
	// packet insert / replace / suppress fixups 
	pSpec->insertPES					= FALSE;
	pSpec->insertPAT					= TRUE;
	pSpec->insertPMT					= TRUE;
	pSpec->insertPCR					= FALSE;
...
#ifdef DO_DEBUGIO
static int   rtfWriteCount[20] = {0};
static int   rtfWriteSize [20] = {0};
static int   rtfReadCount [20] = {0};
static int   rtfReadSize  [20] = {0};
static int   rtfSeekCount [20] = {0};
static INT64 rtfSeekOffset[20] = {0};
static int   rtfDumpWriteFd    = -1;  // Set under debugger control to fd of interest...

EXPORT int rtfDebugOpen( const char *pFileName, int oFlag, int pMode )
{
	return RTF_NATIVE_OPEN( pFileName, oFlag, pMode );
}

EXPORT int rtfDebufClose( int fd )
{
	return RTF_NATIVE_CLOSE( fd );
}
...
EXPORT int rtfDebugWrite( int fd, const void *pBuffer, unsigned int size )
{
	...
	retSize = RTF_NATIVE_WRITE ( fd, pBuffer, size );
	printf( "F=%2d, W=%6d, L=%4d, T=%7d, R=%6d\n", fd, size, rtfWriteCount[fd],  rtfWriteSize[fd], retSize );
	rtfWriteCount[fd]++;
	rtfWriteSize[fd] += size;
	if( fd == rtfDumpWriteFd )
	{
		for( i=0; i<(int)size; i+=32 )
		{
			printf( "0x%08x: ", (ULONG)(rtfSeekOffset[fd]+i) );
			a = MIN( 32, size-i );
			for( j=i; j<i+a; j+=4 )
			{	
				b = MIN(4, size-j);
...
EXPORT int rtfDebugRead ( int fd, void *pBuffer, unsigned int size )
{
	...
	retSize = RTF_NATIVE_READ( fd, pBuffer, size );
	printf( "F=%2d, R=%6d, L=%4d, T=%7d, retSize=%6d\n", fd, size, rtfReadCount[fd], rtfReadSize[fd], retSize );
	rtfReadCount[fd]++;
	rtfReadSize[fd] += size;
	return retSize;
}
...
{
	return RTF_NATIVE_LSEEK( fd, offset, origin );
}
...
EXPORT INT64 rtfDebugLSeek64( int fd, INT64 offset, int origin )
{
	...
	retSize = RTF_NATIVE_LSEEK64( fd, offset, origin );
	rtfSeekCount[fd]++;

	rtfSeekOffset[fd] = retSize;
	printf( "F=%2d, S=%6lld, L=%4d, C=%7lld, retSize=%6lld\n", fd, offset, rtfSeekCount[fd], rtfSeekOffset[fd], retSize );
	return retSize;
}
