...
RTF_RESULT rtfVcdUpdateBoundaryInfo( RTF_VCD_HANDLE handle )
{
	...
	do {		 // error escape wrapper - begin
		...
		RTF_CHK_OBJ( pVcd, RTF_OBJ_TYPE_VCD );
		RTF_CHK_STATE_EQ( pVcd, RTF_VCDSTATE_OPEN );
		// is the sequence start info already filled in?
		if( pVcd->hSeqStartPkt == NULL )
		{
			// no - record this location
			result = rtfWinGetFirstByteInfo( pVcd->hWin, &pVcd->hSeqStartPkt,
											 &pVcd->seqStartPktOff );
			RTF_CHK_RESULT;
			result = rtfWinGetPriorByteInfo( pVcd->hWin, &pVcd->hPicEndPkt,
											 &pVcd->picEndPktOff );
			RTF_CHK_RESULT;
		}
