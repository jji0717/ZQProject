		...
		for( i=0; i<pBuf->activePacketCount; ++i )
		{
			...
			if( ( flags & RTF_PKT_PESHDRPRESENT ) != 0 )
			{
				...
				if( ( pStorage[ pesOff + 7 ] & 0x80 ) != 0 )
				{
					...
					if( *pFpsPicCount != 0 )
					{
						// yes - bump the picture count
						++(*pFpsPicCount);
					}
				} // if( ( pStorage[ hdrOff + 7 ] & 0x80 ) != 0 )
				// does the PES header contain a DTS?
				if( ( pStorage[ pesOff + 7 ] & 0x40 ) != 0 )
				{
					// yes - a DTS indicates a non-reordered picture
					// is this the first picture with a DTS?
					if( *pFpsPicCount == 0 )
					{
						// yes - record this picture's PTS as the first
						*pFpsFirstPicTime = pts;
						// start counting pictures
						*pFpsPicCount = 1;
					}
					// update the record of the last PTS seen
					*pFpsLastPicTime = pts;
				} // if( ( pStorage[ hdrOff + 7 ] & 0x40 ) != 0 )
			} // if( ( flags & RTF_PKT_PESHDRPRESENT ) != 0 )
			// escape if the target picture count has been reached
			if( *pFpsPicCount >= maxFpsPicCount )
			{
				break;
			}
		} // for( i=0; i<pBuf->activePacketCount; ++i )
