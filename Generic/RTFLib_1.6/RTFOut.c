...
RTF_RESULT rtfOutLogSettings( RTF_OUT_HANDLE handle )
{
	...
	do {		 // error escape wrapper - begin
		...
		RTF_LOG_INFO4( RTF_MSG_INF_STATS, "  appOutHandle=0x%x pBufGetFunc=0x%x pBufPutFunc=0x%x pOutCloseFunc=0x%x",
			pSettings->hAppOutFile, pSettings->pBufferGetFunction,
			pSettings->pBufferPutFunction, pSettings->pOutputCloseFunction );
		...
		RTF_LOG_INFO4( RTF_MSG_INF_STATS, "  Insert: PES=%s PAT=%s PMT=%s PCR=%s",
			pBoolStr[pSpec->insertPES], pBoolStr[pSpec->insertPAT],
			pBoolStr[pSpec->insertPMT], pBoolStr[pSpec->insertPCR] ); 
		RTF_LOG_INFO3( RTF_MSG_INF_STATS, "  Replace: PES=%s PAT=%s PMT=%s",
			pBoolStr[pSpec->replacePES], pBoolStr[pSpec->replacePAT],
			pBoolStr[pSpec->replacePMT] ); 
		...
		RTF_LOG_INFO3( RTF_MSG_INF_STATS, "  clearDAI=%s suppressPTSDTS=%s restampPTSDTS=%s",
			pBoolStr[pSpec->clearDAI], pBoolStr[pSpec->suppressPTSDTS],
			pBoolStr[pSpec->restampPTSDTS] );
		...
		RTF_LOG_INFO3( RTF_MSG_INF_STATS, "  restampPCR=%s clearDCON=%s remapPIDs=%s",
			pBoolStr[pSpec->restampPCR], pBoolStr[pSpec->clearDCON],
			pBoolStr[pSpec->remapPIDs] );
