...
// Revision History:
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/Generic/Alarm/SNMPAlarm/ini.cpp $
// 
...
{
	char* strSection = new char[MAX_INI_SECTION_LIST_LENGTH];
	memset(strSection, 0, MAX_INI_SECTION_LIST_LENGTH);

	DWORD dwGet = GetPrivateProfileSectionNames(strSection, MAX_INI_SECTION_LIST_LENGTH, GetFileName().c_str());
	GetStringList(strSection, MAX_INI_SECTION_LIST_LENGTH, arrLists);
...
{
	char* strKey = new char[MAX_INI_KEY_LIST_LENGTH];
	memset(strKey, 0, MAX_INI_KEY_LIST_LENGTH);

	DWORD dwGet = GetPrivateProfileSection(strSection, strKey, MAX_INI_KEY_LIST_LENGTH, GetFileName().c_str());
	GetStringList(strKey, MAX_INI_KEY_LIST_LENGTH, arrLists);
...
{
	char strGet[MAX_INI_VALUE_LENGTH] = {0};

	GetPrivateProfileString(strSection, strKey, "", strGet, MAX_INI_VALUE_LENGTH, GetFileName().c_str());
