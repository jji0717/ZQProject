...
#define ALARM_SOFTWARE_REG_PATH "Software\\ZQ\\ISAAlarm"

class ISAAlarmService : public ZQ::common::BaseSchangeServiceApplication
{
private:
...
RegistryEx_T ISAAlarmService::m_reg(ALARM_SOFTWARE_REG_PATH);
IniFile ISAAlarmService::m_ini;

UINT ISAAlarmService::setupUtilRegistry(WCHAR* pCmds, WCHAR** pstrBuffer, DWORD* pLength)
{
	WORD wCommand;
...
UINT ISAAlarmService::setupUtilIniFile(WCHAR* pCmds, WCHAR** pstrBuffer, DWORD* pLength)
{
	WORD wCommand;
	swscanf(pCmds, L"%c\t", &wCommand);
	if (wCommand == MAN_FREE)
	...
	{
		return MAN_BAD_PARAM;
		//throw std::exception("[ISAAlarmService::setupUtilIniFile] can not get "Main" section in ini file");
	}
...
HRESULT ISAAlarmService::OnInit(void)
{
	...
	if (!m_reg.LoadStr(REG_KEY_CONFIG_FILE, strConfigFile, MAX_PATH-1))
	{
		delete m_pAlarm;
		return -1;
		//throw std::exception("[ISAAlarmService::OnInit] can not get config file setting in registry");
	}
	if (!m_reg.LoadStr(REG_KEY_CONFIG_FILE, strImplementation, MAX_PATH-1))
	{
		delete m_pAlarm;
		return -1;
		//throw std::exception("[ISAAlarmService::OnInit] can not get implemenetation file setting in registry");
	}
...
HRESULT ISAAlarmService::OnUnInit(void)
{
	if (NULL != m_pAlarm)
	{
		delete m_pAlarm;
...
HRESULT ISAAlarmService::OnStart(void)
{
	if (!m_pAlarm->startTrigger())
	{
		Log(LogHandler::L_ERROR, "can not start trigger");
		return -1;
	}
...
HRESULT ISAAlarmService::OnStop(void)
{
	if (!m_pAlarm->stopTrigger())
	{
		Log(LogHandler::L_ERROR, "can not stop trigger");
		return -1;
	}
...
ISAAlarmService gApplicationService;
