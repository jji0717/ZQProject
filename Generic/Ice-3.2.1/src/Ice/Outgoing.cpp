...
IceInternal::Outgoing::invoke()
{
    ...
    switch(_reference->getMode())
    {
        case Reference::ModeTwoway:
        {
		  ...
		  // take multiple sleep()s instead of one deep-sleep in order to wake up in time
		  // -andy
		  if (_state == StateInProgress && timeout <0)
		  {
			while (_state == StateInProgress)
				_monitor.timedWait(IceUtil::Time::milliSeconds(100)); // minimal 100msec
		  }
		  else
		  {
		  	while (_state == StateInProgress && timeout >0)
			{
				Int nextSleep = 100; // minimal 100msec
				if (timeout <= nextSleep)
				{
					nextSleep= timeout;
					timeout =0;
				}
				else timeout -= nextSleep;
				_monitor.timedWait(IceUtil::Time::milliSeconds(timeout));
		  	}
		  }

		  timedOut = (_state == StateInProgress);
            }
