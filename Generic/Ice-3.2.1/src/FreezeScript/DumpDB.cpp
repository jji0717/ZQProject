...
usage(const char* n)
{
    cerr << __LINE__ << ":" << "Usage: " << n << " [options] [dbenv [db]]\n";
    cerr << __LINE__ << ":" <<
        "Options:\n"
...
run(int argc, char** argv, const Ice::CommunicatorPtr& communicator)
{
    ...
    {
        cerr << __LINE__ << ":" << argv[0] << ": " << e.reason << endl;
        usage(argv[0]);
        return EXIT_FAILURE;
    }
    ...
    if(opts.isSet("c"))
    {
        if(args.empty())
        {
            cerr << __LINE__ << ":" << argv[0] << ": no database environment specified." << endl;
            usage(argv[0]);
            return EXIT_FAILURE;
        }
        else if(args.size() > 2)
        ...
        try
        {
            ...
            else
            {
                ...
                if(p == catalog.end())
                {
                    cerr << __LINE__ << ":" << argv[0] << ": database `" << args[1] << "' not found in environment `" << args[0] << "'."
                         << endl;
                    return EXIT_FAILURE;
                }
                else
        ...
        {
            cerr << __LINE__ << ":" << argv[0] << ": " << ex.reason() << endl;
            return EXIT_FAILURE;
        }
    }
    if(opts.isSet("D"))
    ...
    {
        cerr << __LINE__ << ":" << argv[0] << ": an input file cannot be specified with --select" << endl;
        return EXIT_FAILURE;
    }
    ...
    if(inputFile.empty())
    {
        ...
        {
            cerr << __LINE__ << ":" << argv[0] << ": a key type and a value type must be specified" << endl;
            usage(argv[0]);
            return EXIT_FAILURE;
        }
        else if(!evictor && keyTypeName.empty() && valueTypeName.empty())
        {
            try
            {
                ...
                if(p == catalog.end())
                {
                    cerr << __LINE__ << ":" << argv[0] << ": database `" << dbName << "' not found in catalog." << endl;
        ...
        if(l.empty())
        {
            cerr << __LINE__ << ":" << argv[0] << ": unknown key type `" << keyTypeName << "'" << endl;
            return EXIT_FAILURE;
        }
        keyType = l.front();
        ...
        if(l.empty())
        {
            cerr << __LINE__ << ":" << argv[0] << ": unknown value type `" << valueTypeName << "'" << endl;
            return EXIT_FAILURE;
        }
        valueType = l.front();
        ...
        if(!outputFile.empty())
        {
            ...
            if(!of.good())
            {
                cerr << __LINE__ << ":" << argv[0] << ": unable to open file `" << outputFile << "'" << endl;
                return EXIT_FAILURE;
            }
            of << descriptors << endl;
            of.close();
    ...
    {
        cerr << __LINE__ << ":" <<": "<<argv[0] << ": database error: " << ex.what() << endl;
        status = EXIT_FAILURE;
    }
    catch(...)
    {
