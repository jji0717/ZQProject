...
if ($installstate eq "U")
    ...
    $directory=prompt ("Input the directory for install $servicename","C:\\PlaylistAS") if ($install);
    if ($upgrade)
    {
    	...
    	$directory=prompt ("Input the directory for install $servicename","$InstallfilePath[2]");
    }
...
sub usage()
{
    print <<EOF;
Usage:
    PlaylistAS <--install | --upgrade> [--copy | --uncopy]
    PlaylistAS --help
options
    --install
        install the service
    ...
    --servicename=<servicename>
        specify servicename, default is \"PlaylistAS\"
    --help
        display this screen
