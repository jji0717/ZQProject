...
sub main()
{
    ...
    {
    	print "Please specify Install or Upgrade MODPlugin by using \"--install\" or \"--upgrade\" and WFES mode by using \"--mode=single\" or \"--mode=multiple\".For more detail, please using \"--help\"";
    	exit 0;
    }
...
if ($installstate && $ins_mode)
{
	  ...
	  while($sss<$count1)
	  {
		...
		while($ii<$cc)
		{
			if(($$ins_servicesname[$ii][0] eq $$ins_servicesname[$a][0])&&($$ins_servicesname[$ii][2] eq $a[0]))
			{
				...
				{
					system ("reg delete \"HKLM\\$$ins_servicesname[$ii][0]\" /v AppSitesConfigurationFile /f>temp.txt");
				        unlink "temp.txt";
				}
			}
    ...
    $directory=prompt ("Input the directory for install $servicename","C:\\TVBS\\MODPlugin") if ($install);
    if ($upgrade)
    {
    	$key="LMachine/SoftWare/SeaChange/ITV Applications/Currentversion/Services/Movies On Demand";
        $Registry->Delimiter("/");
    	$key = $Registry->{$key};
    	$ImagePath = $key->GetValue(AuthorizationDllPath);
    	$directory=prompt ("Input the directory for install $servicename","$ImagePath");
    }
    ...
    printf CMDFILE "xcopy /fycsr bin\\MultiverseAuthorization.dll $directory\n";
    if ($ins_mode =="multiple")
    {
    	printf CMDFILE "xcopy /fycsr bin\\MultiverseAuthorization_Sample.xml $directory\n";
    }
	...
	if ($$ins_servicesname[$i][3] =~ m/^\%/)
	{
        ...
        if (($$ins_servicesname[$i][2] eq "SOAPWsdlPath")||($$ins_servicesname[$i][2] eq "AppSitesConfigurationFile"))
        {
        if ($ins_mode eq "single")
        {
        ...
        {
        	if (!($$ins_servicesname[$i][2] eq "SOAPWsdlPath"))
        	{
        	$value = prompt("$$ins_servicesname[$i][1]","$$ins_servicesname[$i][3]") if ($$ins_servicesname[$i][5]);
	        $value = $$ins_servicesname[$i][3] if !($$ins_servicesname[$i][5]);
        	$value="\\\\" if ($value eq "\\");
...
sub usage()
{
    print <<EOF;
Usage:
    MODPlugin <--install | --upgrade>
    MODPlugin <--mode=single or multiple>
    MODPlugin --help
options
    --install
        install ModPlugin
...
sub choise()
{
    print "Do you want to install the ModPlugin and commit the changes {Y|N}? ";
    my $choose = <stdin>;
    $choose = trim($choose);
    return $choose;
}
