...
if ($installstate eq "U")
    ...
    $directory=prompt ("Input the directory for install $servicename","C:\\SNMPSvcs") if ($install);
    if ($upgrade)
    {
...
sub usage()
{
    print <<EOF;
Usage:
    SNMPSvc <--install | --upgrade> [--copy | --uncopy]
    SNMPSvc --help
options
    --install
        install the service
    ...
    --servicename=<servicename>
        specify servicename, default is \"SNMPSvc\"
    --help
        display this screen
