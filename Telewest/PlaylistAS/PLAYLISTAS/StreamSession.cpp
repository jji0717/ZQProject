...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/Telewest/PlaylistAS/PLAYLISTAS/StreamSession.cpp $
// 
...
void CStreamSession::LogSoapError(wchar_t* pMessage, const _com_error& Error)
{
	std::wstring Message;
	
	// Message starts off with the provided string.
	Message = L"[Proxy] Error:";
	Message += pMessage;

	// If the _com_error object contains a description, append it
	// to Message.
	if( (wchar_t*)Error.Description() != L"" )
	{
		Message += L" ";
		Message += Error.Description();
	}

	// If the _com_error object specifies a failure HRESULT,
	// other then the generic E_FAIL, get the error message 
	// that goes with that HRESULT and append it to Message. 
	// Also append the HRESULT value formatted as a number.
	if (FAILED(Error.Error()) && (Error.Error() != E_FAIL))
	{
		...
		wchar_t buff[32];
		swprintf(buff,L"0x%08lX", Error.Error());

		Message += buff;
	}

	// Display the error message.
	glog(Log::L_ERROR, Message.c_str());
...
BOOL CStreamSession::Initialize()
{
	...
	{
		glog(Log::L_ERROR, _T("CStreamSession::Initialize() Failed! Status = %x"), status);
		return FALSE;
	}
...
BOOL CStreamSession::UnInitialize()
{
	...
	if(ITV_SUCCESS == sts) {
		glog(Log::L_DEBUG, ("CStreamSession::UnInitialize() Succeed"));
	}
	else {
		glog(Log::L_DEBUG, ("CStreamSession::UnInitialize() Failed! Status = %x"),sts);
	}
...
BOOL CStreamSession::OnAssetReady(STREAMID sid, PAELIST pOriginalAEList, PAELIST pNewAes)
{
	...
	PlaylistSoapProxy tmpSoapProxy(m_SoapWSDLFilePath, m_SoapWSMLFilePath, m_SoapServiceName, m_SoapPort, m_SoapNamespace);
	tmpSoapProxy.setTimeout(m_SoapTimeout);

	try
	{
		tmpSoapProxy.Initialize();
		
		NewAEList = tmpSoapProxy.OnSetupSoapCall(buffHomeID, buffDeviceID, buffPurchaseID, buffStreamID);
	}
	catch( _com_error Error )
	{
		glog(Log::L_DEBUG, _T("Error when SETUP: COM error"));
		LogSoapError( _T("Error when SETUP:"), Error );
		if(m_streamDatas.Remove(sid))
		{
			StreamOpCtrl strmOpCtrl(m_instance, sid);
			strmOpCtrl.TerminateStream();
	...
	if(bRet == ITV_SUCCESS)
		...
		if(m_streamDatas.Remove(sid))
		{
			...
			{
				LogSoapError( _T("Error when TEARDOWN:"), Error );
			}
...
BOOL CStreamSession::OnStateChanged(STREAMID sid, LPSTATECHG stateChg)
{
	...
	case STRMPLAYING:
		...
		{	// is the first time PLAY message got
			...
			{
				LogSoapError( _T("Error when PLAY:"), Error );
				return TRUE;
			}
