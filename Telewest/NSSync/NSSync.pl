...
if ($installstate eq "U")
    ...
    $directory=prompt ("Input the directory for install $servicename","C:\\NSSync") if ($install);
    if ($upgrade)
    {
...
sub usage()
{
    print <<EOF;
Usage:
    NSSync <--install | --upgrade> [--copy | --uncopy]
    NSSync --help
options
    --install
        install the service
    ...
    --servicename=<servicename>
        specify servicename, default is \"NSSync\"
    --help
        display this screen
