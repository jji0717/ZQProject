...
INT32 CSCTCPSocket::Send(CHAR* pBuffer, INT32& iLength) const
{
	if(m_hSocket == INVALID_SOCKET)
	{
		Clog( LOG_DEBUG, _T("Send(), Socket == INVALID_SOCKET: error "));
		return SCS_FAILED;
	}
    ...
    while(left)
    {
        ...
        {
			Clog( LOG_DEBUG, _T("Send(), data error SOCKET_ERROR: errorcode :%d "), GetLastError());
            return SCS_FAILED;
        }
...
INT32 CSCTCPSocket::Receive(CHAR* pBuffer, INT32& iLength) const
{
	if(m_hSocket == INVALID_SOCKET)
	{
		Clog( LOG_DEBUG, _T("Receive(), Socket == INVALID_SOCKET: error "));
		return SCS_FAILED;
	}
    ...
    while(left)
    {
        ...
        if(SOCKET_ERROR == result)
        {
			Clog( LOG_DEBUG, _T("recv(), data error SOCKET_ERROR: errorcode :%d "), GetLastError());
            return SCS_FAILED;
        }
...
INT32 CSCTCPSocket::ReceiveData(CHAR* pBuffer, INT32& iLength) const
{
	if(m_hSocket == INVALID_SOCKET)
	{
		Clog( LOG_DEBUG, _T("ReceiveData(), Socket == INVALID_SOCKET: error "));
		return SCS_FAILED;
	}
