...
BOOL FileCopy(CString SrcFileName, CString DstFileName) 
{
    CString Message;	
	FILE *fpin, *fpout;	
	fpin = fopen(SrcFileName,"rb");
	if (!fpin)
	{
		Message="OPen"+SrcFileName+"error!";
	//	AfxMessageBox(Message);
		return false;
	}
	fseek(fpin, 0, SEEK_END);
	unsigned long size = ftell(fpin);
	fseek(fpin, 0, SEEK_SET);
	
	BYTE *pBuffer=(BYTE*)malloc(size); 
	if (!pBuffer)
	{
	//	AfxMessageBox("Memory lake!");
		return false;
	}
	
	if (fread(pBuffer,1,size,fpin)!=size)
	{
		Message="Read file"+SrcFileName+"error";
	//	AfxMessageBox(Message);
		return false;
	}
	
	fpout = fopen(DstFileName,"wb");
	if (!fpout)
	{
		Message="Open file"+DstFileName+"error!";
		//AfxMessageBox(Message);
		return false;
	}

	if (fwrite(pBuffer,1,size,fpout)!=size)
	{
		Message="Write"+DstFileName+"error!";
	//	AfxMessageBox(Message);
		return false;
	}
