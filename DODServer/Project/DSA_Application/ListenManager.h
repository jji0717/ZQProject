...
public: //attribute
	...
	DWORD				m_dwCheckThreadID;
	HANDLE				m_CheckStatusThread;		
	static bool			m_bCheckStatusThreadAlive;	// control heartbeat thread.
	static int			m_iInterval_Del_New;		// specify a interval for waiting from delete notification to new notification with same file.
	...
	///@param pSocket(IN) - socket which is previous put into
	void FreeOneSocket(CSCTCPSocket *pSocket);
...
private:
	...
	// Check File Status List thread function.
	static DWORD WINAPI	CheckStatusThread(LPVOID pParam);
