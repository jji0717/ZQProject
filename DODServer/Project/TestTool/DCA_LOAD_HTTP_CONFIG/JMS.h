...
public:
	...
	/// @Function .Add a queue ot topic to JMS client. 
	/// @return true on success,false on failure
	/// if return failure,The queue or topic name may not regedit in JMS server.
	...
	BOOL AddQueueOrTopic(BOOL bIsQueue,char *queueName,int nFomrat,CJMSParser *Inparse=NULL);

	/// @Function :the current client will connect JMS server.
	/// @return true on success,false on failure
	/// if return failure, now ,there is not automatic after
	/// @param :no
	BOOL StartConnect(CDODClientAgent *pSource=NULL);
...
public:
	...
	//zhenan add Currentconnect states 2006-02-23
	BOOL m_bConnectionOK;

	CDODClientAgent *m_pSource;
