...
static void WINAPI MiniDumpCallback(DWORD ExceptionCode, PVOID ExceptionAddress)
{
	...
	Clog(LOG_DEBUG,  "Crash exception callback called,ExceptonCode 0x%08x, ExceptionAddress 0x%08x, Current Thread ID: 0x%04x",
		ExceptionCode, ExceptionAddress, dwThreadID);
}

UINT DODRunhread(LPVOID pParam)
{
	g_pDCAagent = new CDODClientAgent;
...
int RunThread(void)
{
	...
	g_bStop=FALSE;
	g_bRealExit=FALSE;
	HANDLE m_Thread = AfxBeginThread(DODRunhread,THREAD_PRIORITY_NORMAL);
...
{
	CString        strLogFilename,strConfigFilename,strCurDir;
	...
	strLogFilename=strCurDir+"\\"+"DODClientAgent.log";	

	CMarkup m_XmlDOM;   

	strConfigFilename=strCurDir+"\\"+"dca.xml";
	DWORD dwLogLevel = LOG_DEBUG;
	if(m_XmlDOM.Load(strConfigFilename))
	{
		if(m_XmlDOM.FindElem("DCAConfiguration"))
		{
			m_XmlDOM.IntoElem();

			if(m_XmlDOM.FindElem("log"))
			{		
				CString sTmp = m_XmlDOM.GetAttrib("Loglevel");
				if (sTmp.GetLength() > 0)
				{
					dwLogLevel = atoi(sTmp.GetBuffer(0));

					if ((dwLogLevel < LOG_NORECORD) || (dwLogLevel > LOG_All))
						dwLogLevel = LOG_All;
				}
			}
		}
	...
	ClogEstablishSettings(strLogFilename, dwLogLevel, LOGMAXSIZE);
}
...
int main(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	TCHAR wszSafestorePath[256] = _T("c:\\");
	// initialize the mini dump
	g_minidump.setDumpPath(wszSafestorePath);
...
// ------------------------------------------------------ Modified by zhenan_ji at 2005年10月21日 10:20:58
		...
		else
		{
			...
			_tprintf(TEXT("CreateService failed - errorcode = %d ,ErrorDescription = %s\n"),nError, strError);
			nRet=1;
		}
	...
	else
	{
		...
		GetErrorDescription(nError, strError);
		_tprintf(TEXT("OpenSCManager failed - errorcode = %d, ErrorDescription = %s \n"),nError, strError);
		nRet=1;
	}
	return nRet;
}
	...
	if(schSCManager)
	{
		...
		if(schService)
		{
			...
			else
			{
				...
				_tprintf(TEXT("DeleteService failed - errorcode =%d, ErrorDescription = %s. \n"), nError, strError);
			}
			CloseServiceHandle(schService);
		}
		else
		{   
			...
			GetErrorDescription(nError, strError);
			_tprintf(TEXT("OpenService failed - errorcode = %d, ErrorDescription = %s \n"),nError, strError);
		}
		CloseServiceHandle(schSCManager);
	}
	else
