...
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
BOOL FileCopy(CString SrcFileName, CString DstFileName) 
{

	//Clog(LOG_DEBUG,_T("copy to dstFn=%s"),DstFileName);
// ------------------------------------------------------ Modified by zhenan_ji at 2005年9月2日 16:47:43
	BOOL bb= CopyFile(SrcFileName,DstFileName,FALSE);
	if (!bb)
	{
		Clog( LOG_DEBUG,_T("CopyFile(SrcFileName(%s),DstFileName(%s)); error"),SrcFileName,DstFileName);
	}
	return bb;
}

bool DeleteDirectory(char* DirName, bool isDel)
{
	if (strlen(DirName) <= 0)
		return FALSE;
	...
	if (strlen(DirName) >= 256)
	{
		strncpy(Dir_Name,DirName,255); //DirNameBak
		Clog( LOG_DEBUG,_T("DeleteDirectory(%s) path_len is more than 256,return "),DirName);
		return FALSE;
	}
	else
	...
	try
	{
		sprintf(tempFileFind,"%s\\*.*",Dir_Name);
		BOOL IsFinded = tempFind.FindFile(tempFileFind);
		BOOL bDelFileOK = TRUE;
		while (IsFinded)
		{
			...
			if (!tempFind.IsDots())
			{
				...
				{
					char tempDir[256];
					sprintf(tempDir,"%s\\%s",Dir_Name,foundFileName);
					DeleteDirectory(tempDir,true);
					//Clog( LOG_DEBUG,_T("Deletedir::%s "),tempDir);
				}
				else
				{
					char tempFileName[256];
					sprintf(tempFileName,"%s\\%s",Dir_Name,foundFileName);
					bDelFileOK = DeleteFile(tempFileName);
					if (bDelFileOK == FALSE)
					{   
						int nError = GetLastError();
						char strError[500];
						GetErrorDescription(nError, strError);
						Clog( LOG_DEBUG,_T("DeleteFile::%s errorcode= %d,ErrorDescription = %s "),tempFileName,nError, strError);
					}
				}
			}
		}
		...
		if(isDel)
		{
			if(!RemoveDirectory(Dir_Name))
			{
				...
				Clog( LOG_DEBUG,_T("RemoveDirectory::path(%s),errorcode= %d, ErrorDescription = %s "),Dir_Name,nError,strError);
				return FALSE;
			}
	...
	catch (...) 
	{
		...
		Clog( LOG_DEBUG,_T("DeleteDirectory(%s) ::GetLastErrorcode = %d ,ErrorDescription = %s"),Dir_Name,nError,strError);
		return FALSE;
	}
	return TRUE;
} 
...
BOOL DirectoryExist(LPCTSTR lpszpathame)
{
	...
	{
//		Clog(LOG_DEBUG,"find first file error path=%s errorcode=%d",szWildpath,GetLastError());
		return FALSE;
	}
	...
	critClog.Lock();
		...
		if (!g_logfile_initialized)			// Do it a second time in case two threads try to init
			...
			if (g_cStdioFile.Open(g_stLogFileName,uiOpenFlags))
				{
				LONGLONG dwLength = g_cStdioFile.GetLength();
				if (dwLength == 0)
					{
...
void  GetErrorDescription(int nErrorcode, char *StrError)
{
	 ...
	 ::FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,0, nErrorcode, 
