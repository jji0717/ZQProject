...
bool DeleteDirectory(char* DirName)
{
	...
	try
	{
		...
		while (IsFinded)
		{
			...
			if (!tempFind.IsDots())
			{
				...
				else
				{
					...
					if (bDelFileOK==FALSE)
					{
						Clog( LOG_DEBUG,_T("DeleteFile::%s error"),tempFileName);
						int nError=GetLastError();
						Clog( LOG_DEBUG,_T("DeleteFile::GetLastError=%d error"),nError);
					}
					//	else
					//		Clog( LOG_DEBUG,_T("DeleteFile::%s ok"),tempFileName);
				}
			}
		}
		tempFind.Close();
		...
		{
			Clog( LOG_DEBUG,_T("RemoveDirectory::%s error"),DirName);
			int nError=GetLastError();
			Clog( LOG_DEBUG,_T("RemoveDirectory::GetLastError=%d error"),nError);
			return FALSE;
		}
	}
	catch (...) 
	{
		int nError=GetLastError();
		Clog( LOG_DEBUG,_T("DeleteDirectory(char* DirName) ::GetLastError=%d error"),nError);
		return FALSE;
	}
	return TRUE;
} 

BOOL DirectoryExist(LPCTSTR lpszpathame,LPCTSTR lpszFilename)
{
	TCHAR szWildpath[MAX_PATH];		//260
	_tcscpy( szWildpath, lpszpathame );
	_tcscat( szWildpath, "\\" );
	_tcscat( szWildpath, lpszFilename);
...
static void Clog_wrapped( CString& stWrkBuf )
{
	...
	GetLocalTime(&St);
	stTimestamp.Format(_T("%02d/%02d %02d:%02d:%02d"),St.wMonth,St.wDay,St.wHour,St.wMinute,St.wSecond);
