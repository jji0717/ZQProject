...
CDCATestDlg::CDCATestDlg(CWnd* pParent /*=NULL*/)
	...
	, m_strIPPort(_T("192.168.80.49:1199"))
{
...
void CDCATestDlg::OnBnClickedButton1()
{
	...
	sTmp.Format("queue/queue_testSep");
	if(m_jms->AddQueueOrTopic(TRUE,sTmp.GetBuffer(0),INT_ONLY_SEND,m_Parse)==FALSE)
	{
		Clog( LOG_ERROR,_T("m_jms.AddOneSendQueue (%s) error"), sTmp);
		delete m_Parse;
...
void CDCATestDlg::OnBnClickedButton5()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	sTmp.Format("queue/queue_DOD");
	if (m_jms->SendMsg(TRUE,sTmp.GetBuffer(0),str.GetBuffer(0),"MESSAGECODE",3006,"MESSAGECLASS","NOTIFICATION")==FALSE)
	{
		Clog( LOG_DEBUG,_T("PtopSend( send share folder mssage, error "));
		AfxMessageBox("send share folder mssage, error");
		return ;
...
void CDCATestDlg::OnBnClickedButton6()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	sTmp.Format("queue/queue_DOD");
	if (m_jms->SendMsg(TRUE,sTmp.GetBuffer(0),str.GetBuffer(0),"MESSAGECODE",3005,"MESSAGECLASS","NOTIFICATION")==FALSE)
	{
		Clog( LOG_DEBUG,_T("PtopSend( send share folder mssage, error "));
		AfxMessageBox("send FullData with Payload in Message, error");
		return ;
	}

	Clog( LOG_DEBUG,_T("PtopSend :send FullData with Payload in Message ok"));
	AfxMessageBox("FullData with Payload in Message ok");
}

void CDCATestDlg::OnBnClickedButton7()
{
	// TODO: 在此添加控件通知处理程序代码
	if (m_jms == NULL)
	{
