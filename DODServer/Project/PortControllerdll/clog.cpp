...
void glog(ISvcLog::LogLevel level, const char* fmt, ...)
{
	...
	static const char prefix[] = "[PortConterller]";
...
void Clog( int level, const char* fmt, ...)
{
	...
	if (service_log == NULL) {
		printf("[PortController]Clog():\tservice_log is NULL\n");
		assert(false);
		return;
	}
...
{
	TRACE("\n %s",pStrFmt);
// ------------------------------------------------------ Modified by zhenan_ji at 2005Äê4ÔÂ4ÈÕ 11:31:28
	...
	critClog.Lock();
		...
		if (!g_logfile_initialized)			// Do it a second time in case two threads try to init
			...
			if (g_cStdioFile.Open(g_stLogFileName,uiOpenFlags))
				{
				DWORD dwLength = (DWORD)g_cStdioFile.GetLength();
				if (dwLength == 0)
					{
