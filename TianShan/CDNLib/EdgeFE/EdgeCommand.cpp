...
#define	PLFMT(x,y) 	"%s/%08X/EdgeCommand[%s]\t"##y,mStreamCtx.id().c_str(),GetCurrentThreadId(),#x	
#elif defined ZQ_OS_LINUX
#define	PLFMT(x,y) 	"%s/%08X/EdgeCommand[%s]\t"y,mStreamCtx.id().c_str(),(unsigned int)pthread_self(),#x
#endif	
...
int32	CdnSsCommand::doUdpPlay( const std::string& streamId, int64 timeOffset, float scale, StreamParams& ctrlParams) {
	...
	if( iRet != 200 ) {
		ZQTianShan::_IceThrow<TianShanIce::ServerError>( ENVLOG , "EdgeFE" , iRet , PLFMT(doPlay,"failed to play stream[%s]"), streamId.c_str());
	} else {
...
int32 CdnSsCommand::doUdpPause( const std::string& streamId, StreamParams& ctrlParams ) {
	...
	int32 iRet = C2Streamer::cUdpRun( request, response );
	if( iRet != 200 ) {
		ZQTianShan::_IceThrow<TianShanIce::ServerError>( ENVLOG , "EdgeFE" , iRet , PLFMT(doPlay,"failed to pause stream[%s]"), streamId.c_str());
	} else {
		ctrlParams.mask = MASK_TIMEOFFSET | MASK_SCALE;
		ctrlParams.timeoffset = response->timeOffset;
		ctrlParams.scale = response->scale;
...
int32 CdnSsCommand::doUdpCommit( const ZQ::StreamService::PlaylistItemSetupInfos& items,
	...
	if( iRet != C2Streamer::errorCodeOK )
	{
		...
		case C2Streamer::errorCodeContentNotFound:
			{
				ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>( ENVLOG , "EdgeFE" , TianShanIce::errcodeAssetNotfound , PLFMT(doCommit,"failed to commit[%s] because[%s]"),
		...
		default:
			{
				ZQTianShan::_IceThrow<TianShanIce::ServerError>( ENVLOG , "EdgeFE" , iRet , PLFMT(doCommit,"failed to commit[%s] because[%s]"),
						mStreamCtx.id().c_str(), response->errorText.c_str() );
			}
			break;
		}
							 ...
							 TianShanIce::SRM::ResourceMap &crResource)
{
	...
	if( upStreamBandwidth > 0 )
	{
		...
		{
			ZQTianShan::_IceThrow<TianShanIce::ServerError>(ENVLOG,"EdgeFE",400,PLFMT(doCommit,"shadow session but no session url is found"));
		}
	...
	if( iRet != C2Streamer::errorCodeOK )
	{
		...
		case C2Streamer::errorCodeContentNotFound:
			{
				ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>( ENVLOG , "EdgeFE" , TianShanIce::errcodeAssetNotfound , PLFMT(doCommit,"failed to load [%s] because[%s]"),
		...
		default:
			{
				ZQTianShan::_IceThrow<TianShanIce::ServerError>( ENVLOG , "EdgeFE" , iRet , PLFMT(doCommit,"failed to load [%s] because[%s]"),
						fileName.c_str() , response->errorText.c_str() );
			}
			break;
		}
	...
	{//shadow session
		...
		{
			ZQTianShan::_IceThrow<TianShanIce::ServerError>(ENVLOG,"EdgeFE",500,PLFMT(doCommit,"Shadow session, but no upstream session url is found"));
		}
	}
	else
	{
...
int32 CdnSsCommand::doValidateItem(TianShanIce::Streamer::PlaylistItemSetupInfo &info)
{
	...
	try
	{
		...
		{//if request the index file , then remember to query content's metadata to find out the index file name or extension
			...
			realFullPathName = getFileNameWithoutMountPath( ctntPrx , METADATA_FILENAME_INDEX, &bitrate );
		}
		else if( stricmp( strSubType.c_str() , CDN_SUBTYPE_NormalForward ) == 0  )
		{
			bMainFile = true;
			realFullPathName = getFileNameWithoutMountPath( ctntPrx , METADATA_FILENAME_MAIN, &bitrate );
			TianShanIce::Properties props = ctntPrx->getMetaData();
			ZQTianShan::Util::getPropertyDataWithDefault( props, METADATA_FILENAME_INDEX, "", strIndexFilePathname );
		}
		...
		{//not index file or main file, so get the extension name
			...
			ZQTianShan::Util::getPropertyDataWithDefault( props,  METADATA_EXTNAME_MAIN, "", mainfileExtName );
