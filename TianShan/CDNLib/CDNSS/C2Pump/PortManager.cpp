...
void PortManager::unregisterSession( const std::string& ip , const std::string& sessId, int64 requestBW )
{
	...
	mSess2Port.erase(sessId);
	MLOG(ZQ::common::Log::L_INFO,CLOGFMT(PortManager,"unregisterSession() session[%s] unregistered : capacity[%ld] usedBW[%ld] sessionCount[%ld] ip[%s] portName[%s]"),
...
std::vector<std::string> PortManager::getAvailPorts(int64 bw, const std::string& sessId) const
{
	...
	for( ; it != mPorts.end(); it ++)
	{
		...
		{
			MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(PortManager,"[%s] getAvailPorts(), do not select [%s] due to no ip address"),
					sessId.c_str(), it->first.c_str() );
			continue;
		}
		...
		{
			MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(PortManager,"[%s] getAvailPorts(), do not select [%s] due to:status[%s] capacity[%ld] usedBW[%ld] natBW[%ld] requestBW[%ld]"),
					sessId.c_str(), it->first.c_str(), attr.bUp?"UP":"DOWN", attr.capacity, attr.usedBW, natBW, bw);
			continue;
		}
		PortAvailInfo info;
		info.portName = ip;
		info.weight = (attr.capacity - attr.usedBW) * 1000 /attr.capacity;
		MLOG.debug(CLOGFMT(PortManager,"[%s] getAvailPorts(), select [%s] as candidate, port weight[%d]"),
			sessId.c_str(), info.portName.c_str(), info.weight );
...
bool PortManager::registerSession( const std::string& ip , const std::string& sessId , int64 requestBW )
{
	...
	mSess2Port.insert(std::make_pair<std::string,std::string>(sessId,portName));
	int64 natBW = 0;
	int64 capacity = 0;
	int64 usedBW = 0;
	int64 usedCount = 0;
	...
	MLOG(ZQ::common::Log::L_INFO, CLOGFMT(PortManager,"registerSession() new session [%s] portName[%s] requestBW["FMT64"]: capacity["FMT64"] usedBW["FMT64"] natBW["FMT64"] sessionCount[%ld] ip[%s] "),
...
std::string PortManager::getIpPort(const std::string& sessId, std::string& ip)
{
	std::string portname = mSess2Port[sessId];
	ip = portNameToIp(portname);
	return portname;
}
