...
namespace StreamService{
    ...
    bool GetRequest::process()
    {
        ...
        getMsgPtr->header(C2CLIENT_Range, std::string("bytes=") + _params.range);
        getMsgPtr->header(C2CLIENT_UserAgent, "EdgeFE/2.8");
        ...
        _bBodySend = false;
		std::string sendingMsg = getMsgPtr->toRaw();
		removeCRLF(sendingMsg);
        _log(ZQ::common::Log::L_DEBUG, CLOGFMT(GetRequest, "[%s/%s] bufReqId[%ld] process(), begin send get request endpoint[%s:%d]: %s"), 
				_instanceKey.c_str(), _xSessionID.c_str(), _bufferReqID, _params.getAddr.c_str(), _params.getPort, sendingMsg.c_str());
