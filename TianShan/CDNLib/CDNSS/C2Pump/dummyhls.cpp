...
bool HLSServer::init() {
	uninit();
	const ConfOldHls_obsolete& conf = mEnv.getConfig().oldHls;
	...
	if( !mAuth->loadKeyFile( conf.mKeyFile5I.c_str() ) ) {
		delete mAuth;
		return false;
	}
...
int32 HLSServer::process(HLSRequestParamPtr request, HLSResponseParamPtr response ) {
	...
	if(!request->subLevelName.empty()) {
		...
		MLOG(ZQ::common::Log::L_DEBUG,CLOGFMT(HLSServer,"process() export master M3U8 with: sessId[%s] addr[%s]"),
				sessId,addr);
		if(!hls.exportMasterM3u8(strContent,1, sessId, addr , mEnv.getConfig().oldHls.mServerHostUrl.c_str() ) ) {
			bOK = false;
		}
	}
	if(!bOK) {
