...
namespace StreamService{
    ...
    {
        _requestInstID =      _atomicID.inc();

		/*
        std::ostringstream oss;
        oss<<"C2Request "<<_requestInstID;
    ...
    bool C2ReadFile::startRequest()
    {
		...
		if(_instanceKey.empty()) {
			std::ostringstream oss;
			oss << "C2Request "<<_requestInstID<<"|"<<(*beginIter)->sessionId();
			_instanceKey = oss.str();
		}
    ...
    void C2ReadFile::onLocate(const LocateResponseData& resp)
    {
		...
		_log(ZQ::common::Log::L_INFO, CLOGFMT(LocateClient, "OnLocate() bufReqId[%ld] Ip:Port[%s:%u] contentName[%s] filename[%s] transferID[%s]"),
		         _bufferReqID, _params.getAddr.c_str(), _params.getPort, _params.contentName.c_str(),_params.filename.c_str(), _params.transferID.c_str());
    ...
    void C2ReadFile::onData(const char* data, const size_t& size, bool error)
    {
        ...
        if (error)
        {
            ...
            _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2ReadFile, "[%s] bufReqId[%ld] onData(), useful data[%ld], align abandon data[%ld]"), _instanceKey.c_str(), _bufferReqID, alignLength, size - alignLength);
        }
    ...
    {
        _phase = ZQ::StreamService::phaseInit;
        _queryIndexInstID = _atomicID.inc();

        std::ostringstream oss;
        oss<<"QueryIndex "<< _queryIndexInstID<<"|"<<attr->sessionId();
        _instanceKey = oss.str();
        _reqID = attr->reqId();
    }
