...
namespace ZQ
{
	namespace StreamService
	{
        ...
        bool RequestHandle::onHttpMessage( const LibAsync::HttpMessagePtr msg)
        {
			...
			std::string recvedMsg = msg->toRaw();
			removeCRLF(recvedMsg);

            _log(ZQ::common::Log::L_DEBUG, CLOGFMT(RequestHandle, "[%s/%s] bufReqId[%ld] onHttpMessage(), received http header: %s"), 
					_instanceKey.c_str(), _xSessionID.c_str(), _bufferReqID, recvedMsg.c_str() );
            return true;
        }
		...
		static char remove_crlf(char c) {
			if(c == '\r' || c == '\n') {
				return '.';
			}
			return c;
		}
		void removeCRLF(std::string& data) {
			std::transform(data.begin(), data.end(), data.begin(), remove_crlf);
		}
	}
}
