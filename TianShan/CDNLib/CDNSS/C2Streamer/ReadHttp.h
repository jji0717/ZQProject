...
public:
	  ...
	  ReadHttp(IReaderCallback* callBack, const std::string& proxyUrl, const std::string& segmenterUrl, ZQ::common::Log& log, const int timeout = 10*1000, int retrytime = 0);
	  ~ReadHttp();
public:
...
public:
	virtual void onRead(Buffer* buf, bool retry = false);
