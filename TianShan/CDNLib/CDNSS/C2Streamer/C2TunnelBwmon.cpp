...
void C2IpConnTackMon::compareInfos()
{
	...
	for( ; it != mLocalPortBWUsage.end() ; it ++ )
	{
		...
		MLOG(ZQ::common::Log::L_DEBUG,CLOGFMT(C2IpConnTackMon,"upstream [%s] bw usage: bw[%lld] bytes[%lld], delta[%lld]"),
	...
	for( ; it != mClientBWUsage.end() ; it ++ )
	{
		BWINFO& info = it->second;
		info.usedBW = info.usedBytes * 8000 / delta;
		MLOG(ZQ::common::Log::L_DEBUG,CLOGFMT(C2IpConnTackMon,"downstream [%s] bw usage: bw[%lld] bytes[%lld]"),
...
{
	const PORTSPEEDCONF& portAttr = mEnv.getConfig().portSpeed;
	PORTSPEEDCONF::const_iterator it = portAttr.begin();
	for( ; it != portAttr.end(); it ++ )
	{
		const SERVEPORTATTR& sa = it->second;
