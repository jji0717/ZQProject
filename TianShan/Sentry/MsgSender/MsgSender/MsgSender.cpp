...
void OnIceMessage(const MSGSTRUCT& msgStruct, const MessageIdentity& mid, void* ctx)
{
    if(pMagSender == NULL)
        return;

    pMagSender->iceMessage(msgStruct, mid, ctx);
}

void OnTextMessage(const MSGSTRUCT& msgStruct, const MessageIdentity& mid, void* ctx)
{
    if(pMagSender == NULL)
        return;

    pMagSender->textMessage(msgStruct, mid, ctx);
}
...
static 
void OnSnapshotMessage(const MSGSTRUCT& msgStruct, const MessageIdentity& mid, void* ctx)
{
...
//        curSnapshot.swap(buf.str());
    ...
    try
    {
        ...
        { // the real 'now'
			...
			gettimeofday(&nowtm,(struct timezone*)NULL);
#endif
            char utcbuf[40] = {0};
