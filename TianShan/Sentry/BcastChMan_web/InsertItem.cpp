...
namespace BcastWebPage
{
	...
	InsertItem::InsertItem(IHttpRequestCtx* pHttpRequestCtx) : BasePage(pHttpRequestCtx)
	{
	}
	...
	bool InsertItem::get()
	{
		...
		snprintf(szBuf, sizeof(szBuf) - 1, "<H2>Insert item before [%s#%s]</H2>", chnlName.c_str(), itemName.c_str());
		responser<<szBuf;
		...
		snprintf(szBuf, sizeof(szBuf) - 1, "<form id=\"InsertItem\" method=\"post\" action=\"%s\">", String::getRightStr(url.generate(), "/", false).c_str());
		responser<<szBuf;
...
*/
		// write submit button
		responser<<"<input type=\"submit\" value=\"Insert item\"/>";
	...
	bool InsertItem::post()
	{
		...
		try
		{
			...
			pubBcastPrx->insertItem(itemName, chnlItem);
		}
		catch (const TianShanIce::InvalidParameter& ex)
		{
			snprintf(szBuf, sizeof(szBuf) - 1, "insert [%s#%s] before [%s] caught %s:%s<br>", 
				chnlName.c_str(), content.c_str(), itemName.c_str(), ex.ice_name().c_str(), ex.message.c_str());
			setLastError(szBuf);
			glog(ErrorLog, CLOGFMT(InsertItem, "%s"), getLastError());
			LinkToShowChannelPageError;
			LinkSpaceError;
		...
		{
			snprintf(szBuf, sizeof(szBuf) - 1, "insert [%s#%s] before [%s] caught %s<br>", 
				chnlName.c_str(), content.c_str(), itemName.c_str(), ex.ice_name().c_str());
			setLastError(szBuf);
			glog(ErrorLog, CLOGFMT(InsertItem, "%s"), getLastError());
			LinkToShowChannelPageError;
			LinkSpaceError;
