...
namespace CodWebPage
{
	...
	bool InsertItem::get()
	{
		...
		_snprintf(szBuf, sizeof(szBuf) - 1, "<H2>Insert item before [%s#%s]</H2>", chnlName.c_str(), itemName.c_str());
		responser<<szBuf;
		...
		_snprintf(szBuf, sizeof(szBuf) - 1, "<form id=\"InsertItem\" method=\"post\" action=\"%s\">", String::getRightStr(url.generate(), "/", false).c_str());
		responser<<szBuf;
	...
	bool InsertItem::post()
	{
		...
		try
		{
			...
			chnlPPT->insertItem(itemName, chnlItem);
		}
		catch (const TianShanIce::InvalidParameter& ex)
		{
			_snprintf(szBuf, sizeof(szBuf) - 1, "insert [%s#%s] before [%s] caught %s:%s<br>", 
		...
		{
			_snprintf(szBuf, sizeof(szBuf) - 1, "insert [%s#%s] before [%s] caught %s<br>", 
