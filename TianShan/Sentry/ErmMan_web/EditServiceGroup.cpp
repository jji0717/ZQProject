...
{
	EditServiceGroup::EditServiceGroup(IHttpRequestCtx* pHttpRequestCtx) : BasePage(pHttpRequestCtx)
	{
	}
	...
	bool EditServiceGroup::post()
	{
		IHttpResponse& responser = _reqCtx->Response();
		return show(responser);
	}

	bool EditServiceGroup::show(IHttpResponse& responser)
	{
		std::string devName;
		...
		if (devName.empty())
		{
			...
			setLastError(szBuf);
			glog(ErrorLog, CLOGFMT(EditServiceGroup, "%s"), getLastError());
		...
		if (isAdd)
		{
			...
			{	
				...
				{
					std::string sg, freq;
					if(!_varMap["ServiceGroup"].empty())
					{
						sg = _varMap["ServiceGroup"];
						/*
...
/*
					...
					{
						groups.push_back(sg);
						if(unlink(responser, devName, portId, groups))
							link(responser, devName, portId, sg, freq);
					}
					else
					{
						link(responser, devName, portId, sg, freq);
			...
			{
				const char* names = _reqCtx->GetRequestVar("ServiceGroupsSelected");
				splitString(groups, names, ";");
				unlink(responser, devName, portId, groups);
			}
		}
		...
		{
			snprintf(szBuf, sizeof(szBuf) - 1, "get Service Group Restriction caught %s:%s<br>", 
		...
		{
			snprintf(szBuf, sizeof(szBuf) - 1, "get Service Group Restriction caught %s<br>", 
				ex.ice_name().c_str());
			setLastError(szBuf);
			glog(ErrorLog, CLOGFMT(EditServiceGroup, "%s"), getLastError());
			responser.SetLastError(getLastError());
		...
		{
			snprintf(szBuf, sizeof(szBuf) - 1, "get Service Group Restriction caught unknown error<br>");
			setLastError(szBuf);
			glog(ErrorLog, CLOGFMT(EditServiceGroup, "%s"), getLastError());
			responser.SetLastError(getLastError());
			return false;
		...
		snprintf(szBuf, sizeof(szBuf) - 1, "<H2>Servicegroups of RFPort <u><i><a href=\"%s\">%s/%s</a></i></u></H2>", String::getRightStr(url.generate(), "/", false).c_str(), devName.c_str(), portId.c_str());
		responser<<szBuf;

		url.clear();
		url.setPath(EditServiceGroupPage);
		url.setVar(TemplateKey, _varMap[TemplateKey].c_str());
		url.setVar(PublisherKey, _varMap[PublisherKey].c_str());
		url.setVar(DeviceNameKey, devName.c_str());
		url.setVar(EdgePortKey, portId.c_str());
		snprintf(szBuf, sizeof(szBuf) - 1, "<form id=\"EditServiceGroup\" method=\"post\" action=\"%s\" onsubmit=\"return true\">", String::getRightStr(url.generate(), "/", false).c_str());
		responser<<szBuf;

		responser<< "<script type='text/javascript'>\n";
		responser << "function updateSG(isAdd)\n"
			<< "{\n"
			<<      "var ServiceGroupsSelected=\"\";\n"
			...
			<<           "{\n"
			<<              "ServiceGroupsSelected+=obj[i].value;\n"
			<<              "ServiceGroupsSelected+=\";\";\n"
			<<           "}\n"
			<<      "}\n"
			<<      "}\n"
			<<      "document.getElementById(\"isAdd\").value=isAdd;\n"
			<<      "document.getElementById(\"ServiceGroupsSelected\").value=ServiceGroupsSelected;\n"
			<<      "document.getElementById(\"EditServiceGroup\").submit();\n"
			<< "}\n";
		...
		responser << "function rowSelected()\n"
			...
			<<      "document.getElementById(\"ServiceGroup\").value=cellArray[1].innerHTML;\n"
...
/*
		...
		responser<<"<tr>";
		responser<<"	<td>ServiceGroup</td>";
		responser<<"	<td>&nbsp&nbsp:<input type='text' id='ServiceGroup' name='ServiceGroup' width=\"10%\"/>";
		responser<<"</tr>";
		responser<<"<tr>";
		...
		responser<< "<input type=\"button\" value=\"unlink\" onclick=\"updateSG(0)\"/>&nbsp&nbsp\n";
		responser<< "<br><input type=\"hidden\" name=\"isAdd\" id=\"isAdd\" value=\"\"/>\n";
		responser<< "<br><input type=\"hidden\" name=\"isUpdate\" id=\"isUpdate\" value=\"\"/>\n";
		responser<< "<br><input type=\"hidden\" name=\"ServiceGroupsSelected\" id=\"ServiceGroupsSelected\"/>\n";

		responser<<"<table id='tableview' class='listTable' onclick='rowSelected()'>";

		responser<<"<tr>";
		responser<<"	<th></th>";
		responser<<"	<th><center>Service Group ID</center></th>";
		...
		{
			responser<<"<tr>";
			snprintf(szBuf, sizeof(szBuf) - 1, "<td><input type='checkbox' name='toSelect' id='toSelect' onclick='isSelected()' value='%d' style='width:auto'></td>", *iter);
			responser<<szBuf;
			...
			url.setVar(ServiceGroupKey, buf);
	...
	bool EditServiceGroup::link(IHttpResponse& responser, std::string devName, std::string portId, std::string routeName, std::string& freqs)
	{
		TianShanIce::EdgeResource::EdgeDevicePrx devicePrx;
		...
		{
			snprintf(szBuf, sizeof(szBuf) - 1, "link Service Group Restriction caught %s:%s<br>", 
				ex.ice_name().c_str(), ex.message.c_str());
			setLastError(szBuf);
			glog(ErrorLog, CLOGFMT(EditServiceGroup, "%s"), getLastError());
			responser.SetLastError(getLastError());
		...
		{
			snprintf(szBuf, sizeof(szBuf) - 1, "link Service Group Restriction caught %s<br>", 
				ex.ice_name().c_str());
			setLastError(szBuf);
			glog(ErrorLog, CLOGFMT(EditServiceGroup, "%s"), getLastError());
			responser.SetLastError(getLastError());
		...
		{
			snprintf(szBuf, sizeof(szBuf) - 1, "link Service Group Restriction caught unknown error<br>");
			setLastError(szBuf);
			glog(ErrorLog, CLOGFMT(EditServiceGroup, "%s"), getLastError());
			responser.SetLastError(getLastError());
			return false;
	...
	bool EditServiceGroup::unlink(IHttpResponse& responser, std::string devName, std::string portId, const TianShanIce::StrValues& routeNames)
	{
		TianShanIce::EdgeResource::EdgeDevicePrx devicePrx;
		TianShanIce::IValues currentGroups;
		try
		...
		for(TianShanIce::StrValues::const_iterator iterStr = routeNames.begin(); iterStr != routeNames.end(); iterStr++)
		{
			...
			{
				snprintf(szBuf, sizeof(szBuf) - 1, "unlink Service Group caught %s<br>", 
					ex.ice_name().c_str());
				setLastError(szBuf);
				glog(ErrorLog, CLOGFMT(EditServiceGroup, "%s"), getLastError());
				responser.SetLastError(getLastError());
			...
			{
				snprintf(szBuf, sizeof(szBuf) - 1, "unlink Service Group caught unknown error<br>");
				setLastError(szBuf);
				glog(ErrorLog, CLOGFMT(EditServiceGroup, "%s"), getLastError());
				responser.SetLastError(getLastError());
				return false;
