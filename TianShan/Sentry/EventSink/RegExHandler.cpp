...
void RegExpHandler::handle(const char* msg, int len, const MessageIdentity& mid)
{
	...
	try {
		...
		std::ostringstream postedbuf;
		postedbuf << "stampUTC[" << evnt.stampUTC << "]";
		for(Event::Properties::const_iterator it = evnt.params.begin();it != evnt.params.end();++it)
		{
			postedbuf << ", " << it->first << "[" << it->second << "]";
		}		
		int64 stampPost = ZQ::common::now();
		_log(ZQ::common::Log::L_INFO, ReLOGFMT(RegExpHandler, "Event posted [%d/%d]: %s"), (int) (stampMatched - stampStart), (int) (stampPost - stampStart), postedbuf.str().c_str());
	}
