// SessionCtrl_web.cpp : Defines the entry point for the DLL application.
//
...
extern "C"
{
	...
	__EXPORT bool service(IHttpRequestCtx *pHttpRequestCtx)
    {
        if(NULL == pHttpRequestCtx)
            return false;

        CpcServiceController ctrler(pHttpRequestCtx);
	...
	__EXPORT bool method(IHttpRequestCtx *pHttpRequestCtx)
	{
		...
		CpcMethodController mctl(pHttpRequestCtx);
		return mctl.process();
	}
