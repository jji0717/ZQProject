...
bool NgodResourceManager::initResourceManager( bool bNeedWarmup )
{
	...
	if( bNeedWarmup )
	{
		...
		for( ; itIter != streamerIters.end() ; itIter++ )
		{
			...
			{
				MLOG(ZQ::common::Log::L_ERROR,CLOGFMT(NgodResourceManager,"initResourceManager() got [%s] when query replica for streamer [%s], endpoint[%s]"),
...
void NgodResourceManager::applyPenalty( const std::string& streamerNetId , int32 penaltyValue )
{
	ZQ::common::MutexGuard gd(*this);
	...
	{
		MLOG(ZQ::common::Log::L_WARNING, CLOGFMT(NgodResourceManager,"applyPenalty() can't find streamer with NetId[%s]"), streamerNetId.c_str() );
		return;
	}
	...
	MLOG(ZQ::common::Log::L_DEBUG,CLOGFMT(NgodResourceManager,"applyPenalty() set penalty [%d] to streamer[%s]"), penaltyValue , streamerNetId.c_str() );
}
	...
	for( ; itStreamerSymLink != streamerSet.end() ; itStreamerSymLink++ )
	{
		...
		{//only record last update time if status is OK
			...
			if(mEnv.mbPublishLog)
			{
				if ( itStreamer->second.penalty <= 0 )
				{
					MLOG(ZQ::common::Log::L_INFO, CLOGFMT(NgodResourceManager, "PenaltyManager streamer hasn't penalty netId[%s] endpoint[%s]"), itStreamer->second.netId.c_str(), itStreamer->second.endpoint.c_str());
				}
				else
				{
					MLOG(ZQ::common::Log::L_INFO, CLOGFMT(NgodResourceManager, "PenaltyManager streamer has penalty netId[%s] endpoint[%s]"), itStreamer->second.netId.c_str(), itStreamer->second.endpoint.c_str());
				}
			}
		}
...
bool NgodResourceManager::getStreamersFromSopAndVolume( const StreamerResourcePara& para , ResourceStreamerAttrExS& rets )
{
	...
	{// walk streamers and try to find the available streamer to create stream on
		...
		MLOG(ZQ::common::Log::L_INFO,REQFMT(NgodResourceManager,"getStreamersFromSopAndVolume() select streamer[%s] as candidate: rquestBW[%d]  usedBW[%lld] totalBW[%lld] usedSess[%lld] totalSess[%lld] weight[%d] "),
				ret.netId.c_str(),para.requestBW , streamerAttr.usedBw , streamerAttr.maxBw , streamerAttr.usedSessCount, streamerAttr.maxSessCount , weight );
...
bool NgodResourceManager::allocateResource( const StreamerResourcePara& para, const std::string& streamerNetId )
{
	...
	if( para.bNeedImportChannel )
	{
		...
		else if( para.bNeedImportChannel )
		{
			...
			oss<<"Import channel status: usedBW["<<icAttr.reportUsedBW <<"] maxBW["<<icAttr.reportMaxBW<<"] usedSess["<<icAttr.reportUsedSessCount<<"] maxSess["<<icAttr.confMaxSessCount<<"]";
			icStatus = oss.str();
		}
	}
	...
	if( para.bNeedImportChannel  )
	{
		...
		else
		{
			...
			if( itIc != mImportChannles.end() )
			{
				...
				icAttr.reportUsedSessCount  --;
				icAttr.reportUsedBW			-= para.requestBW;;
				icAttr.reportUsedSessCount	= max( icAttr.usedSessCount , 0 );
				icAttr.reportUsedBW			= max( icAttr.usedBw , 0 );
				std::ostringstream oss;
...
void NgodResourceManager::processStreamerReplica( const TianShanIce::Replica& rep )
{
	...
	updateStreamerReplica( streamerId , rep.replicaState == TianShanIce::stInService);
}
			...
			for( ; itSymLink != mStreamerSymLinks.end() ; itSymLink++ )
			{
				...
				if( iterSet.size() > 0 )
				{
					...
					if( bStatus && ( lastUpdate + updateInterval ) < curTime ) 
					{
						...
						MLOG(ZQ::common::Log::L_INFO,CLOGFMT(NgodResourceManager,"streamer[%s] is not updated since [%s] ,disable it"),
							itSymLink->first.c_str() ,szTimeBuf );
