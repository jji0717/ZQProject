...
private:
	bool recvData(void);
	bool sendData(std::string& strRes, std::string& respHttpMessageBody);
	char getChar(void);
	bool beginRecv(void);
...
int HttpHandlerProc::run(void)
{
	...
	{		
		...
		msgInfo.uri = _strUri;
		msgInfo.msgBody = _strContent;
		std::string strRes;
		std::string respMessageBody;
		int code = _pSimSer->OnHttpMessage(msgInfo, respMessageBody);
		//catch return code
		switch(code)
		{
			case MSG_OK:
		...
		if(!sendData(strRes, respMessageBody))
		{
...
bool HttpHandlerProc::sendData(std::string& strRes, std::string& respHttpMessageBody)
{
	std::string strSend;
	//start line
	if(strRes.length() > 0)
	...
	if(!respHttpMessageBody.empty())
	{
		memset(buf, 0, sizeof(buf));
		sprintf(buf, "%u", respHttpMessageBody.length());
		strSend += "Content-Length: ";
		strSend +=  buf;
		strSend += "\r\n\r\n";

	    strSend += respHttpMessageBody;
	}else{
...
int SimpleHttpd::OnHttpMessage(const MsgInfo& info, std::string& contentBody)
{
