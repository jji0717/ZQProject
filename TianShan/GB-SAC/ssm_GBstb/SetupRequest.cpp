...
#define		AXIOMNODEGROUPID		"nodegroupid"
					...
					if( VALIDSTR(pNodeGroupId) )
					{
						...
						SSMLOG(InfoLevel , HandlerFmt(HandleSetup ,"replace %s with NodeGroupId in uri [%s]"),
							HeaderTianShanServiceGroup, pNodeGroupId);
...
#define PAIDSTR "ProviderAssetId="
	...
	{
		_statusCode = 500; // initialize via non-succeeded server error
		...
		if (_pSite->findClientSession(_session.c_str()))
		{
			...
			_statusCode = 455; // 455 Method Not Valid in This State
...
printf("srvrSessionId: %s\n", pSessionContext->srvrSessID.c_str());
		...
		// about the common fields in Transport
		{
			...
			for (trspStrsItor = trspStrs.begin(); trspStrsItor != trspStrs.end(); trspStrsItor ++)
			{
				...
				if (false == keyStr.empty() && false == valStr.empty())
				{
					...
					{
						serviceGroupId = atoi(valStr.c_str());
				...
				if (0 == stricmp(keyStr.c_str(), Destination))
					destIP = valStr;
				else if (0 == stricmp(keyStr.c_str(), ClientPort))
					destPort = valStr;
				else if (0 == stricmp(keyStr.c_str(), ClientMac))
					destMac = valStr;
				else if (0 == stricmp(keyStr.c_str(), BandWidth))
				{
		...
		// end of parsing request::Transport

		// NGB: enh#18135 to add "Service-Group-IP" into res[rtServiceGroup]["descriptor"]

		{
			TianShanIce::ValueMap resSvcGrpData;
			char sgip[100];
			TianShanIce::Variant vrt;
			vrt.bRange = false;

			// descriptor = Service-Group-IP
			vrt.type = TianShanIce::vtStrings;
			snprintf(sgip, sizeof(sgip)-2, "%d-%s", serviceGroupId, (destIP.length()>4 ? destIP.c_str() : peerIP.c_str()));
			vrt.strs.push_back(sgip);
			MAPSET(::TianShanIce::ValueMap, resSvcGrpData, "descriptor", vrt);

			// user = x-userID
			if (!xUserID.empty())
			{
				vrt.strs.clear();
				vrt.strs.push_back(xUserID);
				MAPSET(::TianShanIce::ValueMap, resSvcGrpData, "user", vrt);
			}

			// id = ServiceGroupId
			vrt.strs.clear();
			vrt.type = TianShanIce::vtInts;
			vrt.ints.push_back(serviceGroupId);
			MAPSET(::TianShanIce::ValueMap, resSvcGrpData, "id", vrt);
			
			try {
				_srvrSessPrx->addResource(TianShanIce::SRM::rtServiceGroup, TianShanIce::SRM::raMandatoryNonNegotiable, resSvcGrpData);
			}
			catch(...)
			{
				SSMLOG(ErrorLevel, HandlerFmt(HandleSetup, "failed to addResource(rtServiceGroup)"));
			}
		}
...
//			provison() throw exception list: InvalidResource, InvalidParameter, InvalidStateOfArt, NotSupported, ServerError;
		...
		catch (TianShanIce::InvalidParameter& ex)
		{
			...
			_statusCode = 451; // 451 Parameter Not Understood
			composeResponse(_statusCode);
			return false;
		}
		catch (TianShanIce::BaseException& ex)
...
//			serve() throw exception list: InvalidStateOfArt, InvalidResource, OutOfResource, InvalidParameter, NotSupported, ServerError;
		...
		catch (TianShanIce::SRM::OutOfResource& ex)
		{
			...
			_statusCode = 453; // 453 Not Enough Bandwidth
			composeResponse(_statusCode);
			return false;
		}
		catch (TianShanIce::InvalidParameter& ex)
...
// 			}
		...
		if (true == bQam)
		{
			...
			{
				int qamMode = rsMap[TianShanIce::SRM::rtAtscModulationMode].resourceData["modulationFormat"].ints[0];
				
				// convert to GB definition:
				switch (qamMode)
				{
				case 0x06: nQamMode =1; break; // QAM 16 modulation format
				case 0x07: nQamMode =2; break; // QAM 32 modulation format
				case 0x08: nQamMode =3; break; // QAM 64 modulation format
				case 0x0c: nQamMode =4; break; // QAM 128 modulation format
				case 0x10: nQamMode =5; break; // QAM 256 modulation format
				default:   nQamMode =3; break; 
				}
			}
...
//			ChangeQamMode(nQamMode);
			...
			{
				// frequency_number：字符格式，以MHz为单位。（例如654.123 MHz表示为654.123)
				// symbol_rate: 字符格式，采用4位BCD码表示7字符符号率值，以兆符号/s为单位。（例如6.875 MSymbol/s 表示为 6.875)
				snprintf(_szBuf, sizeof(_szBuf) - 1, "%s;programid=%d;frequency=%.3f;modulation=%d;symbol_rate=%.6f", String::getLeftStr(trspStr, ";", true).c_str(), nProgramNumber, nFrequency/1000, nQamMode, nSymbolRate/1000000);
				trspStr = _szBuf;
			}
		}
		else 
...
//		_pResponse->setHeader(HeaderTianShanTransport, _szBuf);
		...
		if (false == bQam)
		{
			...
			else if (true == bNatOpen) // replace the destination ip with the current connection's "peerIP"
			{
				...
				leftStr = String::nLeftStr(trspStr, (int)posEqualAfterDest); // gain the first n chars of a string
				rightStrTmp = String::rightStr(trspStr, (int)posEqualAfterDest); // gain the string after the specified position
				int nPos;
...
//		_pResponse->setHeader(HeaderTianShanClientTimeout, _szBuf);
	...
	bool SetupResponse::process()
	{
		if (0 == stricmp(getRequestHeader(HeaderFormatType).c_str(), SeaChangeFormat))
		{
			...
			char buf[100];
			snprintf(buf, sizeof(buf)-2, "%s;timeout=%d", getResponseHeader(HeaderSession).c_str(), _tsConfig._rtspSession._timeout);
//			std::string scSession = getResponseHeader(HeaderSession) + ";timeout=" + getResponseHeader(HeaderTianShanClientTimeout);
			_pResponse->setHeader(HeaderSession, buf);
