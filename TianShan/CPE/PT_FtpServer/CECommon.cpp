    ...
    else 
    {
        ...
        case ERROR_INTERNET_EXTENDED_ERROR:
            strErrMsg = "ERROR_INTERNET_EXTENDED_ERROR"; break;
			{
				...
				{
					strErrMsg = "ERROR_INTERNET_EXTENDED_ERROR"; 
				}
				break;
			}
...
std::string getErrMsg(uint32 errNum)
{
	char chErr[256] = {0};
	char* pStr;
	pStr = strerror_r(errNum, chErr, sizeof(chErr));
	return pStr;
}
...
{
	struct stat fStatus;
	int ret = stat(wszPath,&fStatus);
	if(ret == 0)
		return true;
	
	if(errno == ENOENT)
	{
		int result = mkdir(wszPath, 0775);
		if(result == 0)
			return true;
		else
		{			
			MOLOG(Log::L_ERROR, CLOGFMT(Common, "Failed to create directory %s Reason : %s"),
                        wszPath, getErrMsg().c_str() );
                }
			
	}
	else
		MOLOG(Log::L_ERROR, CLOGFMT(Common, "Directory %s invalidate : %s"),
                        wszPath, getErrMsg().c_str() );
