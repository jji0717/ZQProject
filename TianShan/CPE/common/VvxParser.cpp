...
#define UQUADWORD  uint64
#define QUADWORD	int64
#define BYTE_OFFSET_POINTER(Address, Offset)		((void*) (((unsigned char*)(Address)) + (Offset)))
...
bool VvxParser::ntfs_ParseVvxFile(const char* szFileName, VvxParser& parser) {
	...
	if(fd == (-1)) {
		glog(Log::L_ERROR, "(%s) failed to open: (%s)", szFileName,strerror(errno));
	}

	struct stat info;
	if(fstat(fd, &info) == (-1)) {
		glog(Log::L_ERROR, "(%s) failed to get file info: (%s)", szFileName,strerror(errno));
		return false;
	}
	...
	if (!vvxIndexHeader)
	{
		glog(Log::L_ERROR, "(%s) Fail to allocate memory, size %ld", szFileName, dwLow);
...
#elif defined(ZQ_OS_LINUX)
		if((bytesRead = read(fd, vvxIndexHeader, len)) == (unsigned long)(-1)) {
	...
	/* TODO */
		...
		if(bytesRead != len)
		{
			glog(Log::L_ERROR, "(%s) bytes read(%ld) != read len requested(%d)",
		...
		if(vvxIndexHeader->majorVersion == 5) {
			...
			*(uint64*)&parser._fileTime = vvx6Header->systemTime;
			...
			uint32 i;
			for (i = 0; i < vvx6Header->tsInformationCount; i++)
			{
			...
			for (i = 0; i < vvx6Header->subFileInformationCount; i++)
			{
				strcpy(parser._subFileExtension[i], (const char*)(subFileInformation[i].fileExtension));
				parser._subFileType[i] = subFileInformation[i].fileType;
				parser._subFileSpeed_denominator[i] = subFileInformation[i].speed.denominator;
			...
			*(uint64*)&parser._fileTime = vvx7Header->systemTime;
			...
			uint32 i;
			for (i = 0; i < vvx7Header->tsInformationCount; i++)
			{
				parser._tsProgramNumber[i] = tsInformation[i].programNumber;
				parser._tsPMTPid[i] = tsInformation[i].pmtPid;
				parser._tsPCRPid[i] = tsInformation[i].pcrPid;
			...
			for (i = 0; i < vvx7Header->subFileInformationCount; i++)
			{
				strcpy(parser._subFileExtension[i], (const char*)(subFileInformation[i].fileExtension));
				parser._subFileType[i] = subFileInformation[i].fileType;
				parser._subFileSpeed_denominator[i] = subFileInformation[i].speed.denominator;
...
//		for (uint32 i = 0; i < hdr.tsInformationCount; i++)
//		{
//			parser._tsProgramNumber[i] = tsInformation[i].programNumber;
//			parser._tsPMTPid[i] = tsInformation[i].pmtPid;
//			parser._tsPCRPid[i] = tsInformation[i].pcrPid;
...
{
	if (bNTFS)
		return ntfs_ParseVvxFile(szFilename, *this);
	else {
#ifdef NAS 
		return false;
#else
		return vsm_ParseVvxFile(szFilename, *this);	
#endif
	}
