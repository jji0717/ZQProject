...
public:
	...
	bool getSubFileSpeed(uint32 nIndex, long& numerator, uint32& denominator)
	{
	...
	bool getSubFilePlayTime(uint32 nIndex, uint32& playTimeinMs)
	{
		if (nIndex>=_nSubFileCount)
			return false;
	...
	bool getSubFileType(uint32 nIndex, SubfileType& stype)
	{
		if (nIndex>=_nSubFileCount)
			return false;
