...
#ifndef __ZQTianShan_BaseAquaRec_CPH_H__
#define __ZQTianShan_BaseAquaRec_CPH_H__
...
#include "../CPH_nPVR/TimerWatcher.h"
...
class BaseAquaRecSession : public ICPHSession, public TimerObject
{
public:
	typedef IceUtil::Handle<BaseAquaRecSession> Ptr;

	BaseAquaRecSession(BaseCPHelper& helper, const ::TianShanIce::ContentProvision::ProvisionSessionExPtr& pSess);
	virtual ~BaseAquaRecSession();
...
{
	friend class BaseAquaRecSession;
public:
...
protected:
	bool reg(const ::std::string& provisionId, BaseAquaRecSession::Ptr pSess);
	void unreg(const std::string& provisionId);
	

protected:

	typedef std::map <std::string, BaseAquaRecSession::Ptr> SessMap; // map of provsionSessId to CPHSess within a CPHO
	SessMap _sessMap;
