...
class CAsyncIo
{
    ...
    CAMEvent m_evWork;      // set when listWork is not empty
    CAMEvent m_evDone;      // set when listDone is not empty
    ...
    CAsyncRequest* GetDoneItem();                   // get an item from the done list

    HRESULT PutWorkItem(CAsyncRequest* pRequest);   // put an item on the work list

    HRESULT PutDoneItem(CAsyncRequest* pRequest);   // put an item on the done list

    void ProcessRequests(void);                     // called on thread to process any active requests

    static DWORD WINAPI InitialThreadProc(LPVOID pv)// initial static thread proc calls ThreadProc with DWORD
	{
        CAsyncIo * pThis = (CAsyncIo*) pv;          // param as this
