                ...
                DWORD dwUser)
{
    if(bAligned)
    {
        if(!IsAligned(llPos) ||!IsAligned(lLength) ||!IsAligned((LONG) pBuffer))
        {
...
CAsyncIo::Length(LONGLONG *pllTotal, LONGLONG *pllAvailable)
{
    ...
    *pllTotal = m_pStream->Size(pllAvailable);
	/*char pp[256];
	sprintf(pp,"Total=%d,Available=%d.",*pllTotal,*pllAvailable);
	logFile.Write(pp);*/
    return S_OK;
}
...
CAsyncIo::GetDoneItem()
{
    ...
    //如果完成请求数为0而且((不在清理内存)或者(在等待数据的完成))，那么应设置成没有信号
	//也就是说，如果有请求，或者在清理内存，那么设置成有信号状态
    return preq;
}
    ...
    {
        {
            ...
            m_cItemsOut++;// one more item not on the done or work list
        }// release critsec
        preq->Complete();
...
CAsyncIo::ThreadProc(void)
{
    ...
    for(;;)
    {
        ...
        {
            ProcessRequests();// requests need processing
        }
        else
        {
            return 0;// any error or stop event - we should exit
        }
    }
}
