...
bool CMarkup::Load( LPCTSTR szFileName )
{
	...
	MultiByteToWideChar(CP_UTF8,0,pBuffer,nFileByteLen,m_csDoc.GetBuffer(nWideLen),nWideLen);
	m_csError.Format(_T("%d bytes to %d wide chars"),nFileByteLen,nWideLen);
	m_csDoc.ReleaseBuffer( nWideLen );
	delete [] pBuffer;
#else
	// Read file directly
	fread( m_csDoc.GetBuffer(nFileByteLen), nFileByteLen, 1, fp );
	m_csDoc.ReleaseBuffer( nFileByteLen );
	m_csError.Format( _T("%d bytes"), nFileByteLen );
#endif
	fclose( fp );
	return x_ParseDoc();
}
...
#if defined( _UNICODE )
	// For ANSI files, replace CP_UTF8 with CP_ACP in both places
	int nMBLen = WideCharToMultiByte(CP_UTF8,0,m_csDoc,nLength,NULL,0,NULL,NULL);
	char* pBuffer = new char[nMBLen+1];
	WideCharToMultiByte(CP_UTF8,0,m_csDoc,nLength,pBuffer,nMBLen+1,NULL,NULL);
	bSuccess = ( fwrite( pBuffer, nMBLen, 1, fp ) == 1 );
	delete [] pBuffer;
	m_csError.Format( _T("%d wide chars to %d bytes"), nLength, nMBLen );
#else
	bSuccess = ( fwrite( (LPCTSTR)m_csDoc, nLength, 1, fp ) == 1 );
	m_csError.Format( _T("%d bytes"), nLength );
#endif
...
bool CMarkup::x_ParseDoc()
{
	...
	m_iPosFree = 1;
	x_AllocPosArray( m_csDoc.GetLength() / 64 + 8 );
	...
	if ( m_csError.IsEmpty() )
		...
		m_csError = csResult + _T(", ") + m_csError;
...
bool CMarkup::x_SetAttrib( int iPos, LPCTSTR szAttrib, LPCTSTR szValue )
{
	...
	else
	{
		...
		csFormat = _T(" ");
		csFormat += szAttrib;
		csFormat += _T("=") x_ATTRIBQUOTE;
		csFormat += x_TextToDoc( szValue, true );
		csFormat += x_ATTRIBQUOTE;
		csInsert = csFormat;
	}
		...
		if ( pSource[nChar] == _T('&') )
		{
			...
			if ( pSource[nChar+1] == _T('#') )
			{
				...
				while ( nCodeLen < 7 && cChar && cChar != _T(';') )
				{
					...
					nCodeLen += _tclen( &pSource[nNumericChar+nCodeLen] );
