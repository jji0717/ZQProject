...
public:
	...
	DataOnDemand::DataStreamPrx operator()(const std::string& space, 
	const std::string& name)
	{
	  return DataOnDemand::DataStreamPrx::uncheckedCast(
...
public:
	...
	{
	  return DataOnDemand::MuxItemPrx::uncheckedCast(
...
public:
	...
	DataStreamImpl(Ice::ObjectAdapterPtr& adapter, 
		DataStreamServiceImpl* parent, 
		const std::string& space);
	virtual ~DataStreamImpl();

	bool init(const ::DataOnDemand::StreamInfo& info);
    ...
    virtual ::DataOnDemand::StreamInfo getInfo(
		const Ice::Current& = Ice::Current()) const;
