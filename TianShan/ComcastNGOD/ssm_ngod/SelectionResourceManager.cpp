...
void NgodResourceManager::updateImportChannelReplica( const std::string& importChannelName , int64 bwUsed , int64 bwTotal ,int32 runningSess )
{
	...
	if( bLog)
	{
		MLOG(ZQ::common::Log::L_INFO, CLOGFMT( NgodResourceManager,"updateImportChannelReplica() update importChannle[%s] .Import channel status: bwUsed[%ld] bwMax[%ld] runningSess[%d] maxSess[%ld]"),
				importChannelName.c_str() , bwUsed, bwTotal , runningSess , itImportChannel->second.confMaxSessCount);
	}
}

#define	REQFMT(_X, _FMT) 	CLOGFMT(_X, "sess(%s)seq(%s)%s " _FMT), para.sessionId.c_str(), para.cseq.c_str(), para.method.c_str()
...
bool NgodResourceManager::getStreamersFromSopAndVolume( const StreamerResourcePara& para , ResourceStreamerAttrExS& rets )
{
	...
	{// walk streamers and try to find the available streamer to create stream on
		...
		char tmpstr[100]={0};
		snprintf(tmpstr, sizeof(tmpstr)-2, "streamer[%s] bw[%ld/%ld] sess[%ld/%ld] weight[%d<Strm%d,Imp%d]", ret.netId.c_str(), streamerAttr.usedBw, streamerAttr.maxBw,
			streamerAttr.usedSessCount, streamerAttr.maxSessCount, weight, streamerWeight, importChannelWeight);
		canddest += tmpstr;
	}

	if (rets.size() > 0)
	{
		MLOG(ZQ::common::Log::L_INFO, REQFMT(NgodResourceManager, "getStreamersFromSopAndVolume() requestBW[%d], %d candidate streamers: %s"), para.requestBW, rets.size(), canddest.c_str());
		return true;
	}

	MLOG(ZQ::common::Log::L_WARNING, REQFMT(NgodResourceManager, "getStreamersFromSopAndVolume() requestBW[%d], no candidate streamers"), para.requestBW);
	return false;
}
...
bool NgodResourceManager::allocateResource( const StreamerResourcePara& para, const std::string& streamerNetId )
{
	...
	MLOG(ZQ::common::Log::L_INFO, REQFMT(NgodResourceManager,"allocateResource() allocated resource for netId[%s] identifier[%s] requestBW[%d], "
		"now usedBW[%ld] maxBW[%ld] usedSess[%ld] maxSess[%ld] %s"),
...
void NgodResourceManager::releaseResource( const StreamerResourcePara& para , const std::string& streamerNetId  , bool bFailed , int32 penaltyValue )
{
	...
	if (0 == streamerAttr.usedSessCount)
		streamerAttr.usedBw = 0;
	...
	MLOG(ZQ::common::Log::L_INFO,REQFMT(NgodResourceManager,"releaseResource() release resource to netId[%s]%s identifier[%s] bandwidth[%ld], now usedBW[%ld] maxBW[%ld] usedSess[%ld] maxSess[%ld]"),
		streamerAttr.netId.c_str(), icStatus.c_str(), para.identifier.c_str(), para.requestBW ,
		streamerAttr.usedBw , streamerAttr.maxBw , streamerAttr.usedSessCount, streamerAttr.maxSessCount);
}
...
int32 NgodResourceManager::evaluateStreamer( const StreamerResourcePara& para , const ResourceStreamerAttr& streamerAttr )
{
	...
	if( streamerAttr.maxBw <=0 || streamerAttr.maxSessCount <=0 || (streamerAttr.maxBw < ( streamerAttr.usedBw + para.requestBW )) || (streamerAttr.maxSessCount < (streamerAttr.usedSessCount + 1 )) )
	{
		MLOG(ZQ::common::Log::L_WARNING, REQFMT(NgodResourceManager, "skip streamer[%s] due to insufficient maxbandiwdth: usedBW[%ld] totalBW[%ld] requestBW[%ld] ; usedSess[%ld] maxSess[%ld] "),
			streamerAttr.netId.c_str(), streamerAttr.usedBw , streamerAttr.maxBw, para.requestBW , streamerAttr.usedSessCount , streamerAttr.maxSessCount );
		return 0;
	}
...
bool NgodResourceManager::isImportChannelAvailable( const StreamerResourcePara& para, const std::string& importChannelName )
{
	...
	else
	{
		...
		{
			MLOG(ZQ::common::Log::L_WARNING,REQFMT(NgodResourceManager,"isImportChannelAvailable() import channel[%s], reportUsedBW[%ld] reportMaxBW[%ld] which means import channel is not available"),
...
int32 NgodResourceManager::evaludateImportChannel( const StreamerResourcePara& para , const ResourceStreamerAttr& streamerAttr )
{
	...
	if( maxBw <=0 || maxSess <=0 || ( maxBw < ( usedBw + para.requestBW )) || ( usedSess >= maxSess) )
	{
		MLOG(ZQ::common::Log::L_WARNING, REQFMT(NgodResourceManager, "evaludateImportChannel() skip [%s] due to insufficient bandwidth: requestBW[%d] usedBW[%ld] maxBW[%ld/(report)%ld/(conf)%ld] usedSess[%ld] maxSess[%ld] "),
			icAttr.netId.c_str(), para.requestBW , usedBw, maxBw, icAttr.reportMaxBW ,icAttr.confMaxBw,
			usedSess , maxSess );
		return 0;
	}
