...
#define	REQFMT(_X, _FMT) 	CLOGFMT(_X, "sess(%s)seq(%s)%s " _FMT), getSessId().c_str(), getCseq().c_str(), getMethod().c_str()
#define	COMMFMT(_X, _FMT) 	CLOGFMT(_X, "sess(%s)seq(%s)%s " _FMT), mIntention.getSessId().c_str(), mIntention.getCseq().c_str(), mIntention.getMethod().c_str()
...
bool CandidateVolumesBuilder::getElementInfo( const SelectIntentionParam& paras )
{
	...
	// associating elements of each asset for elements and their attrs such as content_name bandwidth cuein cueout	
	MLOG(ZQ::common::Log::L_DEBUG, REQFMT(CandidateVolumesBuilder, "getElementInfo(): assicating %d assets"), paras.playlist.size());
	for( ; itAsset != paras.playlist.end(); itAsset ++ )
	{
		...
		try
		{
			if( mEnv.contentInTestMode() )
			{
				...
				// fixup if there are macros of PID/PAID/SID in the content name
				std::string cnamelist;
				for (AEInfo3Collection::iterator itE = aeInfos.begin(); itE < aeInfos.end(); itE++)
				{
					std::string& cname = itE->name;
					size_t pos;
					if (std::string::npos != (pos = cname.find("${PAID}")))
						cname.replace(pos, strlen("${PAID}"), itAsset->paid);
				
					if (std::string::npos != (pos = cname.find("${PID}")))
						cname.replace(pos, strlen("${PID}"), itAsset->pid);

					if (std::string::npos != (pos = cname.find("${SID}")))
						cname.replace(pos, strlen("${SID}"), itAsset->sid);

					cnamelist += cname +", ";
				}

				MLOG(ZQ::common::Log::L_INFO, REQFMT(CandidateVolumesBuilder, "test mode associated %d items for PID[%s]PAID[%s]SID[%s]primary[%s]: %s"), aeInfos.size(), 
					itAsset->pid.c_str(), itAsset->paid.c_str(), itAsset->sid.c_str(), itAsset->primaryAsset ?"T":"F", cnamelist.c_str());
			}
			else if( mEnv.contentLibMode() )
			{
				aeInfos = getElementInfoFromContentLib( itAsset->pid, itAsset->paid, itAsset->sid );
				MLOG(ZQ::common::Log::L_INFO, REQFMT(CandidateVolumesBuilder, "ContentLib mode associated %d items for PID[%s]PAID[%s]SID[%s]primary[%s]"), aeInfos.size(), 
					itAsset->pid.c_str(), itAsset->paid.c_str(), itAsset->sid.c_str(), itAsset->primaryAsset ?"T":"F");
			}
			else
			{
				MLOG(ZQ::common::Log::L_INFO, REQFMT(CandidateVolumesBuilder, "querying LAM for PID[%s]PAID[%s]SID[%s]primary[%s]"), 
					itAsset->pid.c_str(), itAsset->paid.c_str(), itAsset->sid.c_str(), itAsset->primaryAsset ?"T":"F");
				
				int64 stampStart= ZQ::common::now();
				aeInfos = getElementInfoFromLAM( itAsset->pid, itAsset->paid, itAsset->sid );
				
				MLOG(ZQ::common::Log::L_INFO, REQFMT(CandidateVolumesBuilder, "LAM took %dmsec to associate %d items for PID[%s]PAID[%s]SID[%s]primary[%s]"), (int)(ZQ::common::now() - stampStart), aeInfos.size(), 
					itAsset->pid.c_str(), itAsset->paid.c_str(), itAsset->sid.c_str(), itAsset->primaryAsset ?"T":"F");
			}
			...
			{
				MLOG(ZQ::common::Log::L_DEBUG, REQFMT(CandidateVolumesBuilder, "got AE info of PID[%s]PAID[%s]SID[%s]: name[%s] bandwidth[%d] cueIn[%ld] cueOut[%ld] urls[%s] volumes[%s] attr{%s}"), 
		...
		for( ; itElement != aeInfos.end() ; itElement ++ )
		{
			...
			else
			{
				MLOG(ZQ::common::Log::L_DEBUG, REQFMT(CandidateVolumesBuilder, "do not use volume information from element[%s] which belong to PID[%s] PAID[%s] due to primary[%s]"), 
					itElement->name.c_str(), itAsset->pid.c_str(), itAsset->paid.c_str(), itAsset->primaryAsset?"T":"F");
			}

			//set it as primary element if the relative asset if a primary asset
			eleInfo.primaryElement = itAsset->primaryAsset;
			eleInfo.extAds		   = itAsset->extAds;
			...
			MLOG(ZQ::common::Log::L_INFO, REQFMT(CandidateVolumesBuilder, "adjusted cueIn/cueOut of content[%s]:  SETUP[%lld/%lld] LAM[%d/%d], taking cueIn[%lld] cueOut[%lld], bandwidth[%d] nasUrl[%s] volumeList[%s]"), 
				...
				itAsset->cuein, itAsset->cueout, itElement->cueIn, itElement->cueOut, eleInfo.cueIn, eleInfo.cueOut, 
				itElement->bandWidth, 
...
void PriorityVolumeSet::adjustFirstClassVolumes( const std::string& requestedVolume )
{
	...
	for( ; it != mFirstClassVolumes.end() ; it++ )
	{
		if( it->netId == requestedVolume )
		{
			...
			MLOG(ZQ::common::Log::L_DEBUG, COMMFMT(adjustFirstClassVolumes, "adjusted content attrs by requested volume[%s]"), requestedVolume.c_str());
			return;
		}
	}
...
{
	mbLastPreferredStreamer = false;
	mPreferredStreamerCount = 0;    //reset these two variable, we need these two vars to select streamers
	//if( !mResManager.hasAvailVolumes( mIntention.getParameter().identifier) ) {
	//	//could not find a valid sop name which should contain at least one valid streamer
	//	setLastError(errorcodeUnkownSopnames, "sopname[%s] is not found", mIntention.getParameter().identifier.c_str() );
	//	return false;
	//}
	//find first streamer
	...
	mResourceRequest.requestBW			= MAX( (int32)mIntention.getParameter().requestBW, (int32)mVolumesBuilder.getMaxBandwidth() );
...
void StreamerSelection::adjustWeightByAssetValue( )
{
	...
	MLOG(ZQ::common::Log::L_INFO, COMMFMT(StreamerSelection, "adjustWeightByAssetValue() adjust weight: node[%s]  assetvalue[%d]"), 
	...
	for( ResourceStreamerAttrExS::iterator  itStreamer = mCandidateStreamers.begin();  itStreamer != mCandidateStreamers.end() ; itStreamer ++ )	
	{
		...
		{
			if( itStreamer->streamerWeight >= (MAX_WEIGHT - threshold) )
			{
				itStreamer->streamerWeight = caclNewWeight( itStreamer->streamerWeight, true );
				++mPreferredStreamerCount;
			}
		}
		else
		{
...
void StreamerSelection::adjustWeight()
{
	...
	if(!mEnv.mbEnableAssetStack )
	{
		MLOG(ZQ::common::Log::L_DEBUG, REQFMT(StreamerSelection, "AssetStack disabled, skip weight adjusting"));
		return;
	}
	if( !bAllLocal )
	{
...
bool StreamerSelection::commitStreamerForResource( bool bLocalPlaylist)
{
	...
	while( !bCommitOk )
	{
		...
		MLOG(ZQ::common::Log::L_DEBUG, COMMFMT(StreamerSelection, "commitStreamerForResource() trying to get a streamer in [%s], preferStreamer[%d]"), 
			streamerNetIdsToString( mCandidateStreamers ).c_str(), mPreferredStreamerCount );

		// try to select a streamer
		
		std::sort( mCandidateStreamers.begin(), mCandidateStreamers.end(), StreamerWeightSortCmp() );
		int64 totalWeight		= 0;
		size_t involvedStreamerCount = 0;
		//sum the total weight
		{
			for( ResourceStreamerAttrExS::iterator itStreamer = mCandidateStreamers.begin() ; 
				itStreamer != mCandidateStreamers.end() ;
				itStreamer++ )
			{
				//take the smaller one of Streamer Weight and importChannel weight		
				totalWeight += itStreamer->streamerWeight;
				if( (mPreferredStreamerCount > 0) && 
					(++involvedStreamerCount >= mPreferredStreamerCount ))
					break;
			}
			if( totalWeight <= 0 )
		...
		{	
			...
			{
				--mPreferredStreamerCount;
				mbLastPreferredStreamer = mPreferredStreamerCount > 0 ;
			}
			continue;
		}
		...
		MLOG(ZQ::common::Log::L_INFO, COMMFMT(StreamerSelection, "commitStreamerForResource() committed to take streamer[%s]"), 
			itStreamer->netId.c_str());
...
void StreamerSelection::releaseResource( bool bAddPenalty, bool bSkipToNextVolume )
{
	if( mItStreamer !=mCandidateStreamers.end() )
	{
		...
		if( mbLastPreferredStreamer && mPreferredStreamerCount > 0 )
		{
			--mPreferredStreamerCount;
			mbLastPreferredStreamer = mPreferredStreamerCount > 0;
		}
