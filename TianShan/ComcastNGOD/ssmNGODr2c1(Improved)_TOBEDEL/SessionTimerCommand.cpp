...
SessionTimerCommand::SessionTimerCommand(ssmNGODr2c1& env, const ::Ice::Identity& sessIdent)
...
int SessionTimerCommand::run(void)
{
	...
	try
	{		
		NGODr2c1::ContextPrx pContextPrx = NGODr2c1::ContextPrx::uncheckedCast(_env._pContextAdapter->createProxy(_identSess));
		pContextPrx->onTimer();
		return 0;
	}
	...
	try
	{
		_env._pSessionWatchDog->watchSession(_identSess, _env._config._timeoutInterval * 1000);
	}
	catch(...)
	{
	}
