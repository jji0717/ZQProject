...
#define PENALTY_ENABLE_MASK_PLAY	  FLAG(0)
#define PENALTY_ENABLE_MASK_PAUSE	  FLAG(1)
#define PENALTY_ENABLE_MASK_GETPAR	  FLAG(2)
#define PENALTY_DISABLE_MASK_TEARDOWN FLAG(4) // note: this is a DISABLE when "1"
...
const int32 errorcodeTrickRestriction 		= 403;
...
{	
	...
	IN	std::string				groupName;			//group name, for NGOD this should be equal to sop name.For openVBO , this should be the dummy group name
...
public:
	...
	const StreamerResourcePara&					getStreamerResourcePara() const {
...
private:
	...
	size_t										mPreferredStreamerCount;
	bool										mbLastPreferredStreamer;
	
	VolumeAttrEx								mSelectedVolumeAttr;
	
	bool										mbLocalPlaylist;

	StreamerResourcePara						mResourceRequest;
