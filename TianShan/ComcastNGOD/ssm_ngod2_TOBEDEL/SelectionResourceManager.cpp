...
bool NgodResourceManager::allocateResource( const StreamerResourcePara& para, const std::string& streamerNetId )
{
	...
	MLOG(ZQ::common::Log::L_INFO, REQFMT(NgodResourceManager,"allocateResource() allocated resource for netId[%s] identifier[%s] requestBW[%ld], "
...
void NgodResourceManager::releaseResource( const StreamerResourcePara& para , const std::string& streamerNetId  , bool bFailed , int32 penaltyValue )
{
	...
	MLOG(ZQ::common::Log::L_INFO,REQFMT(NgodResourceManager,"releaseResource() release resource to netId[%s]%s identifier[%s] bandwidth[%ld], now usedBW[%lld] maxBW[%lld] usedSess[%lld] maxSess[%lld] %s"),
...
int32 NgodResourceManager::evaluateStreamer( const StreamerResourcePara& para , const ResourceStreamerAttr& streamerAttr )
{
	...
	{
		MLOG(ZQ::common::Log::L_WARNING, REQFMT(NgodResourceManager, "skip due to insufficient maxbandiwdth: usedBW[%lld] totalBW[%lld] requestBW[%ld] ; usedSess[%lld] maxSess[%lld] "),
...
int32 NgodResourceManager::evaludateImportChannel( const StreamerResourcePara& para , const ResourceStreamerAttr& streamerAttr )
{
	...
	{
		MLOG(ZQ::common::Log::L_WARNING, REQFMT(NgodResourceManager, "evaludateImportChannel() skip due to insufficient bandwidth: requestBW[%ld] usedBW[%lld] maxBW[%lld/(report)%lld/(conf)%lld] usedSess[%lld] maxSess[%lld] "),
