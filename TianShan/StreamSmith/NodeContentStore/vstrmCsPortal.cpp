...
{
	uint32 resdMask, resdFlags;
	int64 stampAsOf;
} ContentInfo;

typedef ZQ::common::LRUMap<std::string, ContentInfo> ContentInfoCache;
ContentInfoCache gContentInfos(200);
ZQ::common::Mutex glkContentInfos;
...
{	
	if( !mbEdgeServer )	 //all items should be monitored if is not in EdgeServer mode
		return false;
	...
	if( pos != std::string::npos )
	{
		...
		if( 0 == extName.find("0X") && extName!="0X0000")
			return true; // none-main file can be ignore in the edgemode
	}
...
// 		}
	...
	MLOG(ZQ::common::Log::L_INFO, PORTALFMT(populateAttrsFromFile, "mainfile[%s][%s] populate attr with specified indexFilePath [%s]"),
		mainFilePathname.c_str(),
	...
	ZQTianShan::Util::getPropertyDataWithDefault( content.metaData , METADATA_ContentInPWE, 0, bPrevPWE);
//	ZQTianShan::Util::getPropertyDataWithDefault( content.metaData , SYS_PROP(ContentPropertyParsedOK) , 0 , bParseOkByLoadAll );
	...
	if( !idxParser.ParseIndexFileFromVstrm( mainFilePathname,idxData,bnPvRCopy,strIndexFile ,
											...
											bLocalContentForVstrm != 0) )
	{		
		MLOG(ZQ::common::Log::L_ERROR, PORTALFMT(populateAttrsFromFile, "mainfile[%s][%s] failed to populate attribute with specified index file[%s]"),
			mainFilePathname.c_str(), mainFilePathnameReal.c_str(), strIndexFile.c_str());
	...
	// if PWE has recently changed, call again by forcing to allInfo
	if (bPrevPWE != idxData.isPWE())
	{
		MLOG(ZQ::common::Log::L_DEBUG, PORTALFMT(populateAttrsFromFile, "mainfile[%s][%s] PWE recently changed[%d->%d], forcing to call LoadFullAssetInfoEx() again"),
			mainFilePathname.c_str(), mainFilePathnameReal.c_str(), bPrevPWE, idxData.isPWE());
		ZQ::IdxParser::IndexData		newIdxData;
		if (!idxParser.ParseIndexFileFromVstrm(mainFilePathname, newIdxData, bnPvRCopy, strIndexFile ,
											false, bLocalContentForVstrm != 0) )
		{
			MLOG(ZQ::common::Log::L_WARNING, PORTALFMT(populateAttrsFromFile, "mainfile[%s][%s] LoadFullAssetInfoEx() failed, take the previous result"),
				mainFilePathname.c_str(), mainFilePathnameReal.c_str());
		}
		else
			idxData = newIdxData;
	}
	...
	if( bLocalContentForVstrm != 1 )//need record all metadata
	{
		...
		MLOG(ZQ::common::Log::L_DEBUG,	PORTALFMT(populateAttrsFromFile,"mainfile[%s] successfully parse the index file"),
			mainFilePathname.c_str());
		...
		if( bnPvRCopy  )
		{
			...
			{
				MLOG(ZQ::common::Log::L_DEBUG, PORTALFMT(populateAttrsFromFile,"mainfile[%s] [npvrCopy ]invalid muxRate[%d] can't get playTime"),
											...
											int64* fileSize )
{
	...
	if( findHandle == INVALID_HANDLE_VALUE )
	{
		MLOG(ZQ::common::Log::L_WARNING, PORTALFMT(checkFileCharacters, "can't find file[%s] when checkFileCharacters"), file.c_str() );
		return false;
	}
	...
	if ( findData.VstrmFileVersion.StructureLength > FIELD_OFFSET( VSTRM_FILE_VERSION, Modulus ) )
	{
		...
		MLOG(ZQ::common::Log::L_INFO, PORTALFMT(checkFileCharacters,"get file[%s]'s characters fileSize[%lld] readerCount[%d] writerCount[%d]"),
			file.c_str(),
...
uint64 vstrmCSPortalI::checkResidentialStatus( uint64 flagsToTest, ContentImpl::Ptr pContent,
												...
												const ::std::string& mainFilePathname )
{
	MLOG(ZQ::common::Log::L_DEBUG, PORTALFMT(checkResidentialStatus, "mainfile[%s] checkResidential with flag[%s]"),
		mainFilePathname.c_str(), showCheckResidentialFlag(flagsToTest).c_str() );
	{
		ZQ::common::MutexGuard g(glkContentInfos);
		ContentInfo ci;
		ci.stampAsOf = 0;
		if (gContentInfos.end() != gContentInfos.find(contentFullName))
			ci = gContentInfos[contentFullName];

		// ticket#20613 vstrm gets slow if call FindFirstFile frequently, no more frequently than once-every-10sec
		int esp = (int) (ZQ::common::now() - ci.stampAsOf);
		if ((flagsToTest == ci.resdMask) && (esp < 10000))
		{
			MLOG(ZQ::common::Log::L_INFO, PORTALFMT(checkResidentialStatus, "mainfile[%s] checkResidential with flag[%s] returning cached value[%s] asof %dms ago"),
				mainFilePathname.c_str(), showCheckResidentialFlag(flagsToTest).c_str(), showCheckResidentialFlag(ci.resdFlags).c_str(), esp);
			return ci.resdFlags;
		}
	}
	...
	if (!getFileSet( pContent, mainFilePathname, fileSet, &bAllImportant))
		MLOG(ZQ::common::Log::L_WARNING, PORTALFMT(checkResidentialStatus,"mainfile[%s] can't get file set"), mainFilePathname.c_str( ) );
	...
	if (flagsToTest & ( 1 << TianShanIce::Storage::frfAbsence ) )
	{
		...
		if (!bAllImportant)
		{
			...
			for(STRINGSET::const_iterator it = fileSet.begin(); !bAbsence && it != fileSet.end() ; it ++ )
			{
				if( checkFileExistence( *it ) )
	...
	{		
		int32	readerCount;
		int32	writerCount;		
		for(STRINGSET::const_iterator it = fileSet.begin(); it != fileSet.end() ; it ++ )
		{
			// enh per ticket#20613
			// when it is in EdgeServer mode, it is necessary to ignore those non-main and non-index files
			// especially when there are multiple trick speeds and file-io is slow
			if (mbEdgeServer && it != fileSet.begin())
			{
				std::string::size_type pos = it->find_last_of('.');
				if( pos != std::string::npos)
				{
					std::string extName = it->substr( pos + 1);
					std::transform(extName.begin(), extName.end(), extName.begin(), (int(*)(int)) std::toupper);
					if ((0 == extName.find("0X") && extName !="0X0000") || 'F' == extName[0])
						continue; // none-main file can be ignore in the edgemode
				}
			}
...

	{
		...
		ci.resdFlags      = flagToRet;
		ci.resdMask       = flagsToTest;
		ci.stampAsOf      = ZQ::common::now();
		gContentInfos[contentFullName] = ci;
	}
