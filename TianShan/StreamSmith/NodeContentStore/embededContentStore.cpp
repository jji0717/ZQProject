...
{
TianShanIce::Storage::ContentStorePrx wahahahahahahcontentstore = NULL;
...
Ice::ObjectPrx	NCSBridge::StartContentStore(  ZQADAPTER_DECLTYPE& objAdapter , const std::string confFolder ,ZQ::common::Log& log , VHANDLE vstrmHandle )
{
	...
	if( GAPPLICATIONCONFIGURATION.embededContenStoreCfg.timeoutOfPlaytimeAtProvisioning < 1000 )
		GAPPLICATIONCONFIGURATION.embededContenStoreCfg.timeoutOfPlaytimeAtProvisioning = 1000;

	try
	{
		int32 csThreadCount = gStreamSmithConfig.embededContenStoreCfg.csServiceThreadCount;
		csThreadCount = max(csThreadCount,3);
		EmbededNCSThreadPool.resize(csThreadCount);
		...
		embedContentStore->_timeoutOfPlaytimeAtProvisioning	=	GAPPLICATIONCONFIGURATION.embededContenStoreCfg.timeoutOfPlaytimeAtProvisioning;
		embedContentStore->_streamableLength				=	GAPPLICATIONCONFIGURATION.embededContenStoreCfg.streamableLength;

		bool syncAtStart = GAPPLICATIONCONFIGURATION.embededContenStoreCfg.syncAtStart >= 1;

		embedContentStore->_edgeMode = embedContentStore->_autoFileSystemSync	=	syncAtStart;
								

		ncsMainLog(ZQ::common::Log::L_INFO,"turn [%s] auto file sync",syncAtStart ? "on" : "off");
