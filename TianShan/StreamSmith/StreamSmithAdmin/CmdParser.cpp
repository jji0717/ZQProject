...
void CCmdParser::err(const char* fmt,...)
{
	char msg[2048];
	va_list args;
	...
	va_end(args);
	MutexGuard gd(m_OutputMutex);
	HANDLE hStdHanle=GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(hStdHanle,FOREGROUND_RED);
	printf("%s\n",msg);
	SetConsoleTextAttribute(hStdHanle,FOREGROUND_RED|FOREGROUND_GREEN|FOREGROUND_BLUE);
}
