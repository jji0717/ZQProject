...
BEGIN_CMDROUTE(ScriptRequest,SSAdmin)
...
:ZQ::common::ThreadRequest(pool),SSAdmin(ptr),_env(env)
{
	_iCurCommand = 0;
	m_AdminPrx = env._adminPrx;
}
...
bool ScriptRequest::CreatePlaylist( const char* destIp , const char* destPort )
{
	_strDestIp	= destIp;
	_strPort	= destPort;
	return true;
