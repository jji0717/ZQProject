...
use Win32::TieRegistry;
    ...
    $directory=prompt ("Input the directory for install RtspProxy","C:\\TianShan") if ($install);
    if ($upgrade)
    {
    	$key="LMachine/SoftWare/ZQ Interactive/TianShan/Currentversion/Services/RtspProxy_shell";
        $Registry->Delimiter("/");
    	$key = $Registry->{$key};
    	$ImagePath = $key->GetValue(ImagePath);
    	@InstallfilePath = getpath ($ImagePath);
    	$directory=prompt ("Input the directory for install RtspProxy","$InstallfilePath[2]");
    }
...
sub instservice()
{
	my $profile = shift;
        ...
        printf CMDFILE "ZQShell -i $profile TianShan \"$EXEDIR\"\\ZQShell.exe 4 $SVCACCOUNT \"$SVCPASSWORD\" \"$EXEDIR\"\\ZQShellMsgs.dll auto\n";
        printf CMDFILE "cd ..\n";
}
...
sub usage()
{
    print <<EOF;
Usage:
    RtspProxy <--install | --upgrade> [--copy | --uncopy]
    RtspProxy --help
options
    --install
        install the service
