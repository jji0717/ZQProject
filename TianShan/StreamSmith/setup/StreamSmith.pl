...
use Win32::TieRegistry;
    ...
    $directory=prompt ("Input the directory for install StreamSmith","C:\\TianShan") if ($install);
    if ($upgrade)
    {
    	$key="LMachine/SoftWare/ZQ Interactive/TianShan/Currentversion/Services/StreamSmith_shell";
        $Registry->Delimiter("/");
    	$key = $Registry->{$key};
    	$ImagePath = $key->GetValue(ImagePath);
    	@InstallfilePath = getpath ($ImagePath);
    	$directory=prompt ("Input the directory for install StreamSmith","$InstallfilePath[2]");
    }
...
sub instservice()
{
	my $profile = shift;
        ...
        printf CMDFILE "ZQShell -i $profile TianShan \"$EXEDIR\"\\ZQShell.exe 3 $SVCACCOUNT \"$SVCPASSWORD\" \"$EXEDIR\"\\ZQShellMsgs.dll auto\n";
        printf CMDFILE "cd ..\n";
}
...
sub usage()
{
    print <<EOF;
Usage:
    StreamSmith <--install | --upgrade> [--copy | --uncopy]
    StreamSmith --help
options
    --install
        install the service
