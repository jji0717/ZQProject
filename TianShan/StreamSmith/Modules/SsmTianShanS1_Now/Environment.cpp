...
#define TopicStreamClientRequestEvent  "TianShan/Event/ClientRequest"
...
RequestProcessResult Environment::doContentHandler(IStreamSmithSite* pSite, IClientRequestWriter* pReq)
{
	...
	try 
	{
		...
		TianShanIce::Properties params = pContentHandler->getEventParam();
		std::string url = pContentHandler->getUrl();
		ZQ::common::URLStr parmUrl(url.c_str());
		switch(verbCode)
		{
		case RTSP_MTHD_SETUP:
			{
				...
				char serPort[4];
				std::string  asset = "";
				itoa(pContentHandler->responseStatusCode(),respCode,10);
				itoa(parmUrl.getPort(),serPort,10);
				std::string server = std::string(parmUrl.getHost()) + ":" + std::string(serPort);
				std::map<std::string, std::string> mVar = parmUrl.getEnumVars();
				std::map<std::string, std::string>::iterator it =  mVar.find("asset");
				if( it !=  mVar.end())
				{
					asset =it->second;
				}
				MAPSET(TianShanIce::Properties, params, "sess", pContentHandler->getSession());
				MAPSET(TianShanIce::Properties, params, "verb", "SETUP");
				MAPSET(TianShanIce::Properties, params, "respCode",respCode);
				MAPSET(TianShanIce::Properties, params, "server", server);
				MAPSET(TianShanIce::Properties, params, "application-id", parmUrl.getPath());
				MAPSET(TianShanIce::Properties, params, "asset",asset);
				_streamEventSinker->publish(TopicStreamClientRequestEvent, "RTSP", 1000, "SETUP", params);
				_sessLog(ZQ::common::Log::L_NOTICE, CLOGFMT(RTSPSession, "sess[%s] SETUP[%d]:%s"), pContentHandler->getSession().c_str(), pContentHandler->responseStatusCode(),pContentHandler->ParamLine2Str(params).c_str());
		...
		case RTSP_MTHD_TEARDOWN:
			{
				...
				itoa(pContentHandler->responseStatusCode(),respCode,10);
				MAPSET(TianShanIce::Properties, params, "sess", pContentHandler->getSession());
				MAPSET(TianShanIce::Properties, params, "verb", "TEARDOWN");
				MAPSET(TianShanIce::Properties, params, "respCode",respCode);
				_streamEventSinker->publish(TopicStreamClientRequestEvent, "RTSP", 2000, "TEARDOWN",params);
				_sessLog(ZQ::common::Log::L_NOTICE, CLOGFMT(RTSPSession, "sess[%s] TEARDOWN[%d]:%s"), pContentHandler->getSession().c_str(), pContentHandler->responseStatusCode(), pContentHandler->ParamLine2Str(params).c_str());
			}
			break;
		}
...
bool Environment::getStreamPlayInfo(const TianShanIce::Streamer::PlaylistPrx& playlist, const std::string& streamPrxID, std::string& scale, Ice::Int& iCurrentPos, Ice::Int& iTotalPos)
{
	...
	{
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "%s[SsmEnv:0400] caught by stream(%s).getInfo"), ex.ice_name().c_str(), streamPrxID.c_str());
		return false;
	}
	
	if (!bInfoSucc)
	{
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "stream(%s).getInfo() failed[SsmEnv:0401]"), streamPrxID.c_str());
		return false;
	}
...
bool StreamEventSinker::publish(const std::string& topicName, const std::string& category, int eventId, const std::string& eventName, const TianShanIce::Properties& params)
{
	TianShanIce::Events::GenericEventSinkPrx eventChClient = NULL;

	{
		static GenericEventSinkPrxMap  meventChClient;
		static ZQ::common::Mutex locker;
		ZQ::common::MutexGuard gd(locker); // to mutex meventChClient

		GenericEventSinkPrxMap::iterator it= meventChClient.find(topicName);
		if(it != meventChClient.end())
			eventChClient = it->second;
		else
		{
			try
			{
				Ice::ObjectPrx topic = getTopic(TopicStreamClientRequestEvent);
				if(!topic)
					return false;

				eventChClient = TianShanIce::Events::GenericEventSinkPrx::uncheckedCast(topic);
				meventChClient.insert(GenericEventSinkPrxMap::value_type(topicName,eventChClient));
			}
			catch( const Ice::Exception& ex)
			{
				SSMLOG(ZQ::common::Log::L_ERROR,CLOGFMT(StreamEventSinker,"failed to uncheckedCast:%s due to: [%s]"),topicName.c_str() , ex.ice_name().c_str() );
			}
		}
	}
	...
	std::string stampUTC  = buf;
	try
	{
		eventChClient->post(category, eventId, eventName, stampUTC, _tsConfig._NetId, params);
		return true;
	}
	catch( const Ice::Exception& ex)
	{
		SSMLOG(ZQ::common::Log::L_ERROR,CLOGFMT(StreamEventSinker,"At publish: %s post Exception:[%s]"),topicName.c_str() , ex.ice_name().c_str() );
	}

	return false;
}

Ice::ObjectPrx StreamEventSinker::getTopic( const std::string& topicName )
{
	if( !mTopicManager )
		return NULL;
	...
	try
	{
		iceTopic = mTopicManager->retrieve( topicName );
	}
	catch (const ::IceStorm::NoSuchTopic& ) 
	{	
		try
		{
			iceTopic = mTopicManager->create( topicName );
		}
		catch( const Ice::Exception&)
		{
			iceTopic = NULL;
...
#define StreamEventThreadPoolSZ    (10)
void StreamEventDispatcher::start()
{
	mPool.resize(StreamEventThreadPoolSZ);
...
{
	if (mPool.pendingRequestSize() > StreamEventThreadPoolSZ*5 )
	{
		SSMLOG(ZQ::common::Log::L_WARNING, CLOGFMT(StreamEventDispatcher, "drop event due to too many pending ANNOUNCEs(%d) in pool[%d]"), mPool.pendingRequestSize(), mPool.size());
		return;
	}
...
void Announce::postAnnounceEndofStream(Environment& env, const ::std::string& proxy, const ::std::string& uid, const TianShanIce::Properties& props, const ::Ice::Current& ic)
{
	...
	try
	{
		...
		pServerRequest = env._pSite->newServerRequest(idents[0].name.c_str(), cltSessPrx->getProperty("ctrlConnId"));
		...
		{
			SYS::TimeStamp ts;
			snprintf(szBuf, sizeof(szBuf) -1, SC_ANNOUNCE_ENDOFSTREAM " " SC_ANNOUNCE_ENDOFSTREAM_STRING " eventdate=%04d%02d%02dT%02d%02d%02dZ \"Normal End\"" \
...
void Announce::postAnnounceBeginningOfStream(Environment& env, const ::std::string& proxy, const ::std::string& uid, const TianShanIce::Properties& props, const ::Ice::Current& ic)
{
	...
	try 
	{
		...
		{
			SYS::TimeStamp ts;
			snprintf(szBuf, sizeof(szBuf) -1, SC_ANNOUNCE_BEGINOFSTREAM " " SC_ANNOUNCE_BEGINOFSTREAM_STRING " eventdate=%04d%02d%02dT%02d%02d%02dZ", ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second);
			pServerRequest->printHeader(HeaderSeaChangeNotice, szBuf);
		}
		else // TianShan spec
		{
			...
			if (0 == stricmp(cltSessCtx.rangePrefix.c_str(), "clock"))
			{
				...
				if(!tmp.empty())
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "BcastPos=%s primaryItemNPT=%s", bcastPos.c_str(), tmp.c_str());
					snprintf(szBuf, sizeof(szBuf) -1, "BcastPos=%s;primaryItemNPT=%s", bcastPos.c_str(), tmp.c_str());
				}
				else
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "BcastPos=%s", bcastPos.c_str());
			...
			else 
			{
				...
				if(!tmp.empty())
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "npt=%d.%03d primaryItemNPT=%s", curPos /1000, curPos %1000, tmp.c_str());

					snprintf(szBuf, sizeof(szBuf) -1, "npt=%d.%03d;primaryItemNPT=%s", curPos /1000, curPos %1000, tmp.c_str());
				}
				else
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "npt=%d.%03d", curPos /1000, curPos % 1000);
            ...
            if(SPEC_NGOD_S1 == cltSessCtx.requestType) // NGOD spec
            {
                ...
                else
                {
                    ...
                    ZQTianShan::Util::getPropertyDataWithDefault( props , "streamNptPosition" , "" , pos_str );
                    SYS::TimeStamp ts;
                    snprintf(szBuf, sizeof(szBuf) -1, SC_ANNOUNCE_BEGINOFSTREAM " "  SC_ANNOUNCE_BEGINOFSTREAM_STRING " event-date=%04d%02d%02dT%02d%02d%02d.%03dZ npt=%s",
                        ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.millisecond, pos_str.c_str());
                }
...
void Announce::postAnnounceSpeedChanged(Environment& env, const ::std::string& proxy, const ::std::string& uid, const TianShanIce::Properties& props, const ::Ice::Current& ic)
{
	...
	try 
	{
		...
		if (cltSessCtx.requestType == SPEC_NGOD_SeaChange) // SeaChange spec
		{
			...
			SYS::TimeStamp ts;
			snprintf(szBuf, sizeof(szBuf) -1, SC_ANNOUNCE_SCALECHANGED " " SC_ANNOUNCE_SCALECHANGED_STRING " eventdate=%04d%02d%02dT%02d%02d%02dZ",
				ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second);
			pServerRequest->printHeader(HeaderSeaChangeNotice, szBuf);
		}
		else // TianShan spec
		{
			...
			else 
			{
				...
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "npt=%d.%03d Scale=%f",	curPos /1000, curPos %1000, currentSpeed);

					snprintf(szBuf, sizeof(szBuf) -1, "npt=%d.%03d;Scale=%f", curPos /1000, curPos %1000, currentSpeed);
				}
			}
...
void Announce::postAnnounceStateChanged(Environment& env, const ::std::string& proxy, const ::std::string& uid, const TianShanIce::Properties& props, const ::Ice::Current& ic)
{
	...
	try 
	{
		...
		if (0 == stricmp(cltSessCtx.rangePrefix.c_str(), "clock"))
		{
			...
			if(!tmp.empty())
			{
                snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "BcastPos=%s presentation_state=%s primaryItemNPT=%s", bcastPos.c_str(), stateDept.c_str(), tmp.c_str());
				snprintf(szBuf, sizeof(szBuf) -1, "BcastPos=%s;presentation_state=%s;primaryItemNPT=%s", bcastPos.c_str(), stateDept.c_str(), tmp.c_str());
			}
			else
			{
                snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "BcastPos=%s presentation_state=%s", bcastPos.c_str(), stateDept.c_str());
				snprintf(szBuf, sizeof(szBuf) -1, "BcastPos=%s;presentation_state=%s", bcastPos.c_str(), stateDept.c_str());
			}
		}
		else 
		{
        ...
        if (cltSessCtx.requestType == SPEC_NGOD_S1) // NGOD_S1 spec
        {
            ...
            {
                SYS::TimeStamp ts;
                snprintf(szBuf, sizeof(szBuf) -1, SC_ANNOUNCE_STATECHANGED " " SC_ANNOUNCE_STATECHANGED_STRING " eventdate=%04d%02d%02dT%02d%02d%02d.%03dZ", 
					ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.millisecond);
            }
...
static bool fixupDuration(std::string& dur)
{
	if (std::string::npos != dur.find('.'))
		return false;

	long i = atol(dur.c_str());
	char buf[30];
	snprintf(buf, sizeof(buf)-2, "%d.%03d", i/1000, i%1000);
	dur = buf;
	return true;
}
...
void Announce::postAnnounceItemStepped(Environment& env, const ::std::string& proxy, const ::std::string& uid, const TianShanIce::Properties& props, const ::Ice::Current& ic)
{
	...
	try 
	{
		...
		char noticestr[256];
		szBuf[sizeof(szBuf) - 1] = '\0';
		Adbuf[sizeof(Adbuf) - 1] = '\0';
		noticestr[sizeof(noticestr) - 1] = '\0';
		std::string hdrStr = "ANNOUNCE " + cltSessCtx.url + " RTSP/1.0";
		pServerRequest->printCmdLine(hdrStr.c_str());
		pServerRequest->printHeader(HeaderSession, (char*) idents[0].name.c_str());
		pServerRequest->printHeader(HeaderServer, (char*) env._serverHeader.c_str());
		...
		//add data into TianShan-NoticeParam header
		std::string					curPAssetId,curProviderId;
		std::string					prevPAssetId,prevProviderId;
		std::string					ItemCurName,ItemPrevName;
		std::string					prevCtrlNum,currentCtrlNum;
		std::string					StreamPrevItemNptPrimary;
		std::string					curflags,prevflags;
		std::string					curdur,prevdur;
		std::string					scale,ADStatus;	
		std::string					event_date;
		std::string					noticenpt;
		std::string					totalVideoDur;
		std::string					totalDuration;
		std::string					adstatus, totalADItemDur;
		int                         nptBy =0;
		std::string                 tmpstr;
		
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.currentPAID","",curPAssetId);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.prevPAID","",prevPAssetId);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.currentPID","",curProviderId);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.prevPID","",prevProviderId);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.currentItem","",ItemCurName);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.prevItem","",ItemPrevName);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.prevCtrlNum","",prevCtrlNum);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.currentCtrlNum","",currentCtrlNum);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.nptPrimary","",StreamPrevItemNptPrimary);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.currentDur","",curdur);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.prevDur","",prevdur);		
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.currentFlags","",curflags);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.prevFlags","",prevflags);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.event-date","",event_date);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.npt", "", noticenpt);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.totalVideodur","",totalVideoDur);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.totalDur","",totalDuration);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.ADStatus","",adstatus);
		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.TotalADItemDuration","",totalADItemDur);

		ZQTianShan::Util::getPropertyDataWithDefault(tempMap,"sys.nptBy", "", tmpstr); nptBy = atoi(tmpstr.c_str());
		nptBy = 1;  // force to 1 by stupid ADs interagation, ticket#19405

		// fixup msec to show as decimal sec
		fixupDuration(StreamPrevItemNptPrimary);
		fixupDuration(curdur);
		fixupDuration(prevdur);
		fixupDuration(totalVideoDur); 
		fixupDuration(totalDuration); 
		fixupDuration(totalADItemDur); 

		TianShanIce::Properties::const_iterator it;
		for(it = tempMap.begin();it!=tempMap.end();++it) 
		{
			SSMLOG(DebugLevel, CLOGFMT(PlaylistEvent,"TianShan-NoticeParam header[%s] = %s "),(it->first).c_str(),(it->second).c_str());
		}
		...
		if (cltSessCtx.requestType == SPEC_NGOD_SeaChange) // SeaChange spec
		{
			...
			SYS::TimeStamp ts;
			snprintf(szBuf, sizeof(szBuf) -1, SC_ANNOUNCE_ITEMSTEPPED " " SC_ANNOUNCE_ITEMSTEPPED_STRING " eventdate=%04d%02d%02dT%02d%02d%02dZ" " " "\"%s\"",
		...
		else // TianShan spec
		{
			...
			else 
			{
				...
				int ADNpt;
				sscanf(StreamPrevItemNptPrimary.c_str(), "%d", &ADNpt);
				int64 curDur = atol(curdur.c_str());
				float speed = (float) atof(scale.c_str());
				...
				case 1:
					...
					// when nptBy=1, force npt=nptPrimary, ticket#19405
					pNP_Ads += snprintf(pNP_Ads, Adbuf + sizeof(Adbuf) -pNP_Ads -1, "npt=%d.%03d", (ADNpt / 1000), (ADNpt % 1000));
					pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1,    "npt=%s;nptPrimary=%s", StreamPrevItemNptPrimary.c_str(), StreamPrevItemNptPrimary.c_str());
					break;

				default:
					pNP_Ads += snprintf(pNP_Ads, Adbuf + sizeof(Adbuf) -pNP_Ads -1, "npt=%d.%03d", (ADNpt / 1000), (ADNpt % 1000));
					pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1,    "npt=%d.%03d;nptPrimary=%s", curPos /1000, curPos %1000, StreamPrevItemNptPrimary.c_str());
				}

				pNP_Ads += snprintf(pNP_Ads, Adbuf + sizeof(Adbuf) -pNP_Ads -1, ";scale=%.3f;ItemPAID=%s;ADItemDuration=%d.%03d", //;TotalADItemDuration=%d.%03d",
					speed, ItemCurName.c_str(), (int)(curDur/1000), (int)(curDur%1000));

				if (!adstatus.empty())
				{
					pNP_Ads += snprintf(pNP_Ads, Adbuf + sizeof(Adbuf) -pNP_Ads -1, ";ADStatus=%s", adstatus.c_str());
					pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";adsType=%s", adstatus.c_str());
				}

				if (!totalADItemDur.empty())
				{
					pNP_Ads += snprintf(pNP_Ads, Adbuf + sizeof(Adbuf) -pNP_Ads -1, ";TotalADItemDuration=%s", totalADItemDur.c_str());
					pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";adsTimeRemained=%s", totalADItemDur.c_str());
				}
    ...
    //                snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "npt=%d.%03d prev_item=%s current_item=%s primaryItemNPT=%s",
				...
				//	snprintf(szBuf, sizeof(szBuf) -1, "npt=%d.%03d;prev_item=%s;current_item=%s;prevPAID=%s;prevPID=%s;currentPAID=%s;currentPID=%s;nptPrimary=%s;prevDur=%s;currentDur=%s;prevFlags=%s;currentFlags=%s;prevCtrlNum=%s;currentCtrlNum=%s",
				//		curPos /1000, curPos %1000, ItemPrevName.c_str(), ItemCurName.c_str(),prevPAssetId.c_str(),prevProviderId.c_str(),curPAssetId.c_str(),
				//		curProviderId.c_str(),StreamPrevItemNptPrimary.c_str(),prevdur.c_str(),curdur.c_str(),prevflags.c_str(),curflags.c_str(),prevCtrlNum.c_str(),currentCtrlNum.c_str());
				//}
				//else
				//{
    //                snprintf(noticeParamBuf, sizeof(noticeParamBuf) -1, "npt=%d.%03d prev_item=%s current_item=%s",
				...
				//	snprintf(szBuf, sizeof(szBuf) -1, "npt=%d.%03d;nptPrimary=%s;prev_item=%s;current_item=%s;prevPAID=%s;prevPID=%s;currentPAID=%s;currentPID=%s;prevDur=%s;currentDur=%s;prevFlags=%s;currentFlags=%s;prevCtrlNum=%s;currentCtrlNum=%s",
				//		curPos /1000, curPos %1000, StreamPrevItemNptPrimary.c_str(), ItemPrevName.c_str(), ItemCurName.c_str(),prevPAssetId.c_str(),prevProviderId.c_str(),curPAssetId.c_str(),
				//		curProviderId.c_str(),prevdur.c_str(),curdur.c_str(),prevflags.c_str(),curflags.c_str(),prevCtrlNum.c_str(),currentCtrlNum.c_str());
				//}

				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";currentDur=%s",     curdur.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";prevDur=%s",        prevdur.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";currentItem=%s",    ItemCurName.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";prevItem=%s",       ItemPrevName.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";currentPAID=%s",    curPAssetId.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";currentPID=%s",     curProviderId.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";prevPAID=%s",       prevPAssetId.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";prevPID=%s",        prevProviderId.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";currentFlags=%s",   curflags.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";prevFlags=%s",      prevflags.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";currentCtrlNum=%s", currentCtrlNum.c_str());
				pNP_TS += snprintf(pNP_TS, szBuf + sizeof(szBuf) -pNP_TS -1, ";prevCtrlNum=%s",    prevCtrlNum.c_str());

				char* pNP_ORIG = noticeParamBuf;
				pNP_ORIG += snprintf(pNP_ORIG, noticeParamBuf + sizeof(noticeParamBuf) -pNP_ORIG -1, "npt=%d.%03d prev_item=%s current_item=%s", 
					curPos /1000, curPos %1000, tempMap["prevItemName"].c_str(), tempMap["currentItemName"].c_str());

				if (tempMap.find("sys.primaryItemNPT") != tempMap.end())
					pNP_ORIG += snprintf(pNP_ORIG, noticeParamBuf + sizeof(noticeParamBuf) -pNP_ORIG -1, " primaryItemNPT=%s", tempMap["sys.primaryItemNPT"].c_str());
			}
			/*
			//将16进制的npt转化为对应的10进制数值
			int64 Npt;
			char chtemp[32];
			sscanf(noticenpt.c_str(),"%llx",&Npt);
			sprintf(chtemp,"%ld",Npt);
			snprintf(noticestr, sizeof(noticestr) -1, SC_ANNOUNCE_ITEMSTEPPED " \"%s\" event-date=%s npt=%d.%03d","Item Step",
				event_date.c_str(),Npt/1000,Npt%1000);
			*/
			int Npt=0, decNpt=0;
			if (sscanf(StreamPrevItemNptPrimary.c_str(),"%d.03d",&Npt, &decNpt) >1)
				Npt = Npt*1000 + decNpt;
			else Npt *=1000;
			snprintf(noticestr, sizeof(noticestr) -1, SC_ANNOUNCE_ITEMSTEPPED " \"%s\" event-date=%s npt=%d.%03d","Item Step",
				event_date.c_str(), Npt/1000, Npt%1000);
			...
			pServerRequest->printHeader(HeaderNotice,noticestr);
			if (cltSessCtx.requestType == SPEC_NGOD_S1) // NGOD_S1 spec
			{
				std::string clientSessID;
				ZQTianShan::Util::getPropertyDataWithDefault( cltSessCtx.props , HeaderClientSessionId , "" , clientSessID);
                ...
                {
                    SYS::TimeStamp ts;
                    snprintf(szBuf, sizeof(szBuf) -1, SC_ANNOUNCE_ITEMSTEPPED " " SC_ANNOUNCE_ITEMSTEPPED_STRING " eventdate=%04d%02d%02dT%02d%02d%02d.%03dZ", 
						ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.millisecond);
                }
//              std::string tianShanNoticeParam(noticeParamBuf);
//              std::string noticeStr(szBuf);
...
//              pServerRequest->printHeader(HeaderNotice, (char*)noticeStr.c_str());
				pServerRequest->printHeader(HeaderRequire, "com.comcast.ngod.c1");
				pServerRequest->printHeader(HeaderOnDemandSessionId, (char *)cltSessCtx.streamID.c_str());
				if(0 != _tsConfig._announce._notifyAds  && atoi(curflags.c_str()))	//currentFlags为非零时 &&  NGOD_S1 spec && _notifyAds 非零
				{
					pServerRequest->printHeader(HeaderADNoticeParam,Adbuf);	
					SSMLOG(DebugLevel, CLOGFMT(PlaylistEvent, "NGOD_S1 spec(Notice: %s AD-NoticeParam: %s) _notifyAds (%d) has been sent out."), noticestr, Adbuf ,_tsConfig._announce._notifyAds);
					pServerRequest->post();
				}
				else
					SSMLOG(DebugLevel, CLOGFMT(PlaylistEvent, "NGOD_S1 spec: _notifyAds [%d] equal 0 or curflags is [%s] so no post Announce with AD-NoticeParam."),_tsConfig._announce._notifyAds,curflags.c_str());
			}
			else // NGOD_TianShan spec
			{
				if(0 != _tsConfig._announce._notifyAds && atoi(curflags.c_str()))
				{
					pServerRequest->printHeader(HeaderADNoticeParam,Adbuf);	
					SSMLOG(DebugLevel, CLOGFMT(PlaylistEvent, "requestType: [%d] , _notifyAds [%d] and curflags is [%s],so post Announce with AD-NoticeParam."),cltSessCtx.requestType,_tsConfig._announce._notifyAds,curflags.c_str());
				}
				SSMLOG(DebugLevel, CLOGFMT(PlaylistEvent, "requestType: [%d] , _notifyAds [%d] equal 0 or curflags is [%s], so not post Announce with AD-NoticeParam."),cltSessCtx.requestType,_tsConfig._announce._notifyAds,curflags.c_str());
			}
		}
		if (cltSessCtx.requestType != SPEC_NGOD_S1)
			pServerRequest->post();
