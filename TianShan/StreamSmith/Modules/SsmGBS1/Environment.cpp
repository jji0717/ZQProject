...
void Environment::uninitIceRuntime()
{
	...
	{
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "destroy event Channel adapters caught(%s)"), ex.ice_name().c_str());
	}
...
bool Environment::openSafeStore()
{
	...
	_tsConfig._database._path += FNSEPS MODULE_NAME;
...
bool Environment::connectIceStorm()
{
	...
	SSMLOG(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "leave connectIceStorm()"));
	return true;
}
