...
// --------------------------------------------------------------------------------------------
// Revision History: 
// $Header: /ZQProjs/TianShan/DataTunnel/source/DataContentStore/DataContentStoreServ.cpp 2     1/02/14 3:08p Zonghuan.xiao $
// $Log: /ZQProjs/TianShan/DataTunnel/source/DataContentStore/DataContentStoreServ.cpp $
// 
// 2     1/02/14 3:08p Zonghuan.xiao
// 
...
ZQ::common::Config::Loader<DODContentStoreCfg> config("DataContentStore.xml");
...
//		_adapter = _communicator->createObjectAdapter(CONTENT_STORE_ADAPTER_NAME);
		...
		_adapter = ZQADAPTER_CREATE(_communicator, CONTENT_STORE_ADAPTER_NAME, config.endpoint.c_str(), *m_pReporter);

		_dodStoreI = new TianShanIce::Storage::DataOnDemand::DODContentStoreI(config.netId);
	
		_adapter->ZQADAPTER_ADD(_communicator, _dodStoreI, CONTENT_STORE_ADAPTER_NAME);
		_adapter->activate();
