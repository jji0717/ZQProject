...
TianShanIce::Streamer::DataOnDemand::DataStreamServicePrx getStreamService(Commander& cmder)
{
	...
	try {
		...
		TianShanIce::Streamer::DataOnDemand::DataStreamServicePrx strmSvc;
		if (obj == NULL) {
			return NULL;
		} else {
			strmSvc = TianShanIce::Streamer::DataOnDemand::DataStreamServicePrx::checkedCast(obj);
			return strmSvc;
		}
...
TianShanIce::Streamer::DataOnDemand::DataStreamPrx getStream(Commander& cmder,
										const std::string& strmName)
{
	std::string proxy;
	...
	try {
		Ice::ObjectPrx obj = iceComm->stringToProxy(proxy);
		TianShanIce::Streamer::DataOnDemand::DataStreamPrx strm;
		if (obj == NULL) {
			cmder.print("stringToProxy failed\n");
			return NULL;
		} else {
			strm = TianShanIce::Streamer::DataOnDemand::DataStreamPrx::checkedCast(obj);
			return strm;
		}
	} catch (Ice::Exception& e) {
		cmder.print("exception has occurred. exception name is: %s\n",
...
TianShanIce::Streamer::DataOnDemand::MuxItemPrx getMuxItem(Commander& cmder,
									  const std::string& strmName, 
									  const std::string& muxItemName)
{
	std::string proxy;
	...
	try {
		...
		if (obj == NULL) {
			...
			muxItem = TianShanIce::Streamer::DataOnDemand::MuxItemPrx::checkedCast(obj);
			return muxItem;
		}
	} catch (Ice::Exception& e) {
		cmder.print("exception has occurred. exception name is: %s\n",
...
int PingCmd::handler(Commander& cmder, const Arguments& args)
{
	...
	try {
		...
		TianShanIce::Streamer::DataOnDemand::DataStreamServicePrx strmSvc = getStreamService(cmder);
	...
	try {
		...
		if (args.size() == 1) {
			...
			if (strmSvc == NULL) {
				...
				std::string space;
				cmder.getSettingString(SETTING_SPACE, space);
				::TianShanIce::StrValues strs = strmSvc->listStreams(space);
				::TianShanIce::StrValues::iterator it;				

				for (it = strs.begin(); it != strs.end(); it ++) {
					TianShanIce::Streamer::DataOnDemand::DataStreamPrx strm = getStream(cmder, 
						*it);
			...
			TianShanIce::Streamer::DataOnDemand::DataStreamPrx strm = getStream(cmder, args[1]);
			if (strm == NULL) {
				cmder.print("getStream(%s) failed\n", args[1].c_str());
				r = CMD_R_FAILED;
			} else {
...
int CreateStreamCmd::handler(Commander& cmder, const Arguments& args)
{
	...
	TianShanIce::Streamer::DataOnDemand::StreamInfo strmInfo;
...
int DumpStreamCmd::dumpStream(Commander& cmder, const std::string& strmName)
{
	int r;
	try {
		TianShanIce::Streamer::DataOnDemand::DataStreamPrx strm = getStream(cmder, strmName);
		if (strm == NULL) {
			cmder.print("getStream() failed\n");
			r = CMD_R_FAILED;
		} else {
			TianShanIce::Streamer::DataOnDemand::StreamInfo strmInfo = strm->getInfo();
...
int AddMuxItemCmd::handler(Commander& cmder, const Arguments& args)
{
	...
	if (args.size() == 2) {
		...
		muxItemInfo.ctype = (TianShanIce::Streamer::DataOnDemand::CacheType )
		...
		muxItemInfo.ctype = (TianShanIce::Streamer::DataOnDemand::CacheType )atoi(args[9].c_str());
...
int RemoveMuxItemCmd::handler(Commander& cmder, const Arguments& args)
{
	...
	try {
		...
		TianShanIce::Streamer::DataOnDemand::MuxItemPrx muxItem = getMuxItem(cmder, 
			args[1], args[2]);
...
int NotifyCmd::handler(Commander& cmder, const Arguments& args)
{
	...
	try {
		TianShanIce::Streamer::DataOnDemand::MuxItemPrx muxItem = getMuxItem(
			cmder, args[2], args[3]);
								...
								const std::string& muxItemName)
{
	...
	try {
		...
		if (muxItem == NULL) {
			...
			TianShanIce::Streamer::DataOnDemand::MuxItemInfo muxItemInfo = muxItem->getInfo();
...
int TotalMuxItem::handler(Commander& cmder, const Arguments& args)
{
	...
	try {
		...
		if (strmSvc == NULL) {
			...
			for (it = strs.begin(); it != strs.end(); it ++) {
				TianShanIce::Streamer::DataOnDemand::DataStreamPrx strm = getStream(cmder, *it);
				if (strm != NULL) {
