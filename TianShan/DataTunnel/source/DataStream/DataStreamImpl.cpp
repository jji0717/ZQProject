...
DataStreamImpl::DataStreamImpl(ZQADAPTER_DECLTYPE& adapter, 
							   ::Freeze::DSServantLocatorPtr& dsServantLocator, 
							   DataStreamServiceImpl* parent, 
							   const std::string& space):
	_adapter(adapter), _dsServantLocator(dsServantLocator),_parent(parent), _space(space)
{
...
//				_adapter->remove(ident);
				...
				glog(Log::L_WARNING, "DataStreamImpl::destroy():\t" 
					"dsServantLocator->remove() failed. exception name: %s, "
...
/*		try {
		...
		MuxItemImpl* muxItem = new MuxItemImpl(_adapter);
		...
		if (_dsServantLocator->add(muxItem, createMuxItemIdentity(
...
bool DataStreamImpl::removeMuxItem(const ::std::string& name)
{
	...
	try {
		...
		glog(Log::L_WARNING, "DataStreamImpl::removeMuxItem():\t" 
			"_dsServantLocator->remove() failed. exception name: %s, "
			"object identity: %s:%s:%s", e.ice_name().c_str(), 
