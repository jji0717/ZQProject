...
RequestHandler::RequestHandler(ZQ::common::Log& fileLog, Environment& env,
							   IStreamSmithSite* pSite, IClientRequestWriter* pReq, const char* methodName)
:_fileLog(fileLog), _env(env), _site(pSite), _request(pReq), _method(methodName?methodName:"Unimplemented"),
_response(NULL), _sessionProxy(NULL), _playlistPrx(NULL),
...
RequestProcessResult RequestHandler::process()
{
	...
	// ticket#18544 for session-less verbs, take dummy global sessionId instead
	// session-less verbs are GET_PARAMETER, SET_PARAMETER, OPTION, DESCRIBE
	if (_session.empty())
	{
		switch (_request->getVerb())
		{
	...
	HANDLERLOG(ZQ::common::Log::L_INFO, HANDLERLOGFMT(RequestHandler, "request processed w/ ret[%d], took %dms"), ret, (int)(ZQTianShan::now() - _startTime));
	return ret;
}
...
bool RequestHandler::getSessionContext()
{
	...
	try
	{
		...
		HANDLERLOG(ZQ::common::Log::L_DEBUG, HANDLERLOGFMT(RequestHandler, "getSessionContext() got session[%s] context"), _session.c_str());
	}
