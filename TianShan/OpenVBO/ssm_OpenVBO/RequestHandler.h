...
#define HANDLERLOGFMT(_X, _T) CLOGFMT(_X, "sess(%s)seq(%s)req(%p)%s " _T), _session.c_str(), _sequence.c_str(), _request, _method.c_str()
...
public:
	...
	RequestHandler(ZQ::common::Log& fileLog, Environment& env, 
		IStreamSmithSite* pSite, IClientRequestWriter* pReq, const char* methodName = "Unsupported");
