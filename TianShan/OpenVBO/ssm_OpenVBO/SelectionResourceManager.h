...
struct ResourceStreamerAttr 
{
	...
	std::string									volume;		        //volume which relative to current streamer
...
public:
	...
	inline bool					excludeC2Linkdown( ) const
	{
		return mbExcludeC2Linkdown;
	}
...
//	RemoteAssetStack												mRemoteAssetStack;
	...
	std::string                                                     _confContentNameFmt;
};
...
struct SessionStat {	
	...
	bool										localPlaylist;
	StreamerResourcePara						selectionResource;

	std::string								streamerNetId;
	std::string								streamSessId;
	std::string								rtspSessId;
	std::string								ondemandSessId;
	int64									lastTouch;// timestamp of last touch
	uint32									bitrate;
	int64									timeOffset;
	float									scale;
	TianShanIce::Streamer::StreamState		state;// stream state
	SessionStat ():lastTouch(0),bitrate(0),timeOffset(0),scale(0.0),
		state(TianShanIce::Streamer::stsSetup) {
	}
};



class SessionStatCache {
public:
	typedef std::map<std::string,SessionStat*>	SESS2STATS;
	typedef std::set<SessionStat*> STATSET;
	typedef std::map<std::string,STATSET>		STREAMER2STAT;

	SessionStatCache();
	~SessionStatCache( );
	bool	addStat( SessionStat* stat );
	void	removeStat( const std::string& rtspSess );

	void	statChanged( const std::string& streamSess, int64 offset, 
		float newScale, TianShanIce::Streamer::StreamState state );

	STATSET getCachedStatusForStreamer( const std::string& streamer );

private:	

	SESS2STATS		mStreamSess2Stats;
	SESS2STATS		mOndemand2Stats;
	SESS2STATS		mRtspSess2Stats;
	STREAMER2STAT	mStreamer2Stat;
	ZQ::common::Mutex mLocker;
};

typedef SessionStatCache SelectionRMSessionStateCache;


//////////////////////////////////////////////////////////////////////////
///resource manager event sink interface
class IResourceManagerEventSink
{
public:
	virtual ~IResourceManagerEventSink(){}
...
public:
	...
	///for maintenance 
	///groupName , for NGOD this is the sop name , for OpenVBO this is dummy group name
	...
	void									attachEventSinker(IResourceManagerEventSink* sinker);
public:
	...
	void									applyPenalty( const std::string& sessId, const std::string& streamerNetId , int32 penaltyValue );

	void									applyIncreasablePenalty( const std::string& sessId, const std::string& streamerNetId , int32 penaltyValue , int32 maxPenaltyValue );
...
public:
	...
	/// for ComCast NGOD identifier is sopname, for OpenVBO identifier is sourceIp
	...
	bool									isStreamHealthy(const std::string& streamerNetId, int *penalty );

protected:
	
	void									processImportChannelReplica( const TianShanIce::Replica& rep );
	void									processStreamerReplica( const TianShanIce::Replica& rep );
	void									processStreamServiceReplica( const TianShanIce::Replica& rep );
	...
	bool									isImportChannelAvailable( const StreamerResourcePara& para , const std::string& importChannelName );
...
protected:
	...
	IResourceManagerEventSink*			mResourceManagerEventSinker;
};
