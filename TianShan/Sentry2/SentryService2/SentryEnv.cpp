...
SentryEnv::SentryEnv(ZQ::common::Log& log,ZQ::common::NativeThreadPool& threadPool,Ice::CommunicatorPtr& communicator,const char* endpoint,const char* databasePath)
...
_thpool(threadPool), _webPort(0), _communicator(communicator), _loopbackPort(0), _log(log),_pages(*this, NULL)
{
	...
	_httpConfg.host = _webBind;
	_httpConfg.port = _webPort;
	_httpConfg.mode = ZQ::eloop::HttpServer::MULTIPE_LOOP_MODE;
	_httpConfg.serverName = "Sentry2";
	printf("webip = %s,webport = %d\n",_webBind.c_str(),_webPort);
