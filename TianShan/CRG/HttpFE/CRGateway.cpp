...
void CRMManager::setHexDump(bool hexdump)
{
	_hexdump = hexdump;
}

bool CRMManager::getHexDump()
{
	return _hexdump;
}
...
void Response::send(bool hexdump,int64 stampCreated,char* sendhint)
{
	if(_msg)
	{
		...
		{
			if(senddata[i] == '\r' || senddata[i] == '\n')
				sendmsg += '.';
			else
				sendmsg += senddata[i];
		}
		long latencyDispatch = long(ZQ::common::now() - stampCreated);
		_Logger(ZQ::common::Log::L_INFO, CLOGFMT(Response,"socket[%s] sent:[%s] took %dms"),sendhint,sendmsg.c_str(),latencyDispatch);

		if (hexdump)
		{
			_Logger.hexDump(ZQ::common::Log::L_DEBUG, senddata.c_str(), senddata.size(), sendhint, false);
		}
	}
}
void Response::clear()
...
void ClientRequestHandler::onMessageCompleted()
{
	...
	char recvhint[100],sendhint[100];
	sprintf(recvhint,"%s:%d => %s:%d",peerip, peerport,locip, locport);
	sprintf(sendhint,"%s:%d => %s:%d",locip, locport, peerip, peerport);

	_recvData += _postedData;
	std::string recvmsg;
	for (size_t i = 0; i < _recvData.size(); i++)
	{
		if(_recvData[i] == '\r' || _recvData[i] == '\n')
			recvmsg += '.';
		else
			recvmsg += _recvData[i];
	}
	_Logger(ZQ::common::Log::L_INFO, CLOGFMT(ClientRequestHandler,"socket[%s] received[%d]bytes:%s"),recvhint,recvmsg.size(),recvmsg.c_str());

	if (_crmMgr->getHexDump())
	{
		_Logger.hexDump(ZQ::common::Log::L_DEBUG, _recvData.c_str(), _recvData.size(), recvhint, false);
	}
	_req.setContent(_postedData.data(), _postedData.size());
	_crmMgr->processRequestSync(&_req, &_resp);
	// fill the response object
	_resp.send(_crmMgr->getHexDump(),_stampCreated,sendhint);
	_recvData.clear();
}
