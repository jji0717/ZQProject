...
int  ZQ::CLIENTREQUEST::Environment::registerSnmp(GatewayCenter* gwCenter)
{
	...
	try
	{
		...
		dsmccSnmpAgent->addObject( Oid("1.298"), ManagedPtr(new SimpleObject(VariablePtr(new DsmccMeasureReset(1, 1, mStatsCollector, &GatewayStatisticsCollector::reset)),  AsnType_Integer, aReadWrite))); ++registerCount;
...
void GatewayStatisticsCollector::reset()
{
...
int64 GatewayStatisticsCollector::measuredSince() const
{
	ZQ::common::MutexGuard gd(mLocker);
	return mMeasuredSince;
}
...
{	
	...
	if(!stats)	return;
	stats->timeCostMax = timecost > stats->timeCostMax ? timecost : stats->timeCostMax;
