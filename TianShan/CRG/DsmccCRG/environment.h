...
struct GatewayStatisics
{
	...
	void reset()
	{
		...
		winSince =stampMax =ZQ::common::now();
		errorToCountMap.clear();
	}
};

#define FLOAT_WINSIZE_MSEC (10*60*1000) // 10min
...
public:
    ...
    GatewayStatisics* getStatistics();

	void	collect( GATEWAYCOMMAND cmd, uint32 resultCode, uint32	timecost);

	void	reset(const uint32& dummy);
	uint32 dummyGet(void) { return 1; }

	uint64	measuredSince();
	
public:
	GatewayStatisics	mStats[STATISTICS_COUNT];
	ZQ::common::Mutex	mLocker;
	int64				mMeasuredSince, _stampTill;

	void				_leverage(uint32 winSizeMsec=FLOAT_WINSIZE_MSEC);
};
...
public:
	Environment( ZQ::common::Log& logger , ZQADAPTER_DECLTYPE& objAdapter )
		:mLogger(logger), mAdapter(objAdapter) //, mdsmccCRGSnmpAgnet(NULL)
	{
