...
WritableMessagePtr GatewayDsmccDialog::toMessage(const char *buf, size_t &size)
{
	...
	ZQ::DSMCC::ProtocolType pType ;	
	if (0 == gwConfig.sockserver.protocolType)
		pType = ZQ::DSMCC::Protocol_MOTO;
	else if (1 == gwConfig.sockserver.protocolType)
		pType = ZQ::DSMCC::Protocol_Tangberg;
	ZQ::DSMCC::DsmccMsg::Ptr msg =  ZQ::DSMCC::DsmccMsg::parseMessage((const uint8*)buf,size,byteprocessed,pType);
...
bool GatewayDsmccDialog::fromMessage( WritableMessagePtr message, char *buf, size_t& size)
{
	...
	case ZQ::DSMCC::MsgID_SetupRequest:
		pMsg = new ZQ::DSMCC::ClientSessionSetupRequest(tmpHeader,pType);
		break;
	case ZQ::DSMCC::MsgID_SetupConfirm:
		pMsg = new ZQ::DSMCC::ClientSessionSetupConfirm(tmpHeader,pType);
		break;
	case ZQ::DSMCC::MsgID_ReleaseRequest:
		pMsg = new ZQ::DSMCC::ClientSessionReleaseRequest(tmpHeader,pType);
		break;
	case ZQ::DSMCC::MsgID_ReleaseConfirm:    
		pMsg = new ZQ::DSMCC::ClientSessionReleaseConfirm(tmpHeader,pType);
		break;
	case ZQ::DSMCC::MsgID_ReleaseIndication:    
		pMsg = new ZQ::DSMCC::ClientSessionReleaseIndication(tmpHeader,pType);
		break;
	case ZQ::DSMCC::MsgID_ReleaseResponse:   
		pMsg = new ZQ::DSMCC::ClientSessionReleaseResponse(tmpHeader,pType);
		break;
	case ZQ::DSMCC::MsgID_ProceedingIndication: 
		pMsg = new ZQ::DSMCC::ClientSessionProceedingIndication(tmpHeader,pType);
		break;
	case ZQ::DSMCC::MsgID_InProgressRequest: 
		pMsg = new ZQ::DSMCC::ClientSessionInProgressRequest(tmpHeader,pType);
		break;
	default:
		pMsg = 0;
	}
	...
	// ticket#14898, stupid STB of ChangZhou
	if (gwConfig.tailCSPI >0 && ZQ::DSMCC::MsgID_ProceedingIndication == msgId)
	{
		for (uint i =0; i < gwConfig.tailCSPI; i++)
			buf[sizeUsed++] =0x00;
	}
