	...
	else if(_protocolType == Protocol_Tangberg)
	{
		...
		case 0x01:
			...
			//_privateDataBytes.push_back(0x0);
				...
				itorMd = metadata.find(CRMetaData_StupidCodeForHeNan);
				if(itorMd == metadata.end() || itorMd->second.empty())
				{
					return false;
				}
		        int16 stupidCodeForHeNan = htons(atoi(itorMd->second.c_str()));

				uint8 buf[2];
				memset(buf, 0, sizeof(buf));
				memcpy(buf, &stupidCodeForHeNan, sizeof(stupidCodeForHeNan));
                _privateDataBytes.insert(_privateDataBytes.begin() + _privateDataBytes.size(), buf, buf + 2);
			}
