...
StreamEventSinker::StreamEventSinker( DSMCC_Environment& env )
:mEnv(env),
mbQuit( true ),
mSentinel(NULL)
{
...
{
	::Ice::ObjectPrx obj  = mEnv._objAdapter->addWithUUID(handler);
	if( !obj )
	{
		return false;
	}
...
bool StreamEventSinker::start( const std::string& endpoint )
{
	...
	mSentinel = new EventChannel::Sentinel(hlog,mEndpoint,this);
	mSentinel->start();	
...
int StreamEventSinker::run()
{
	...
	int64 interval = 60 * 60 * 1000;
	if(interval < 10 * 60 * 1000 )
		interval = 10 * 60 * 1000;
	hlog(ZQ::common::Log::L_INFO,CLOGFMT(StreamEventSinker,"event receiving status checker is running, interval[%lld]"), interval);
	while(!mbQuit)
	{
		mSem.timedWait(1000);
		if(mbQuit)	break;
		...
		if( delta > interval )
		{
			hlog(ZQ::common::Log::L_WARNING,CLOGFMT(StreamEventSinker,"no new event received for [%lld]ms , reconnect to event channel"),delta );
			try
			{
	...
	hlog(ZQ::common::Log::L_INFO,CLOGFMT(StreamEventSinker,"event receiving status checker is stopped"));
	return 0;
}
...
{
	hlog(ZQ::common::Log::L_WARNING, CLOGFMT(StreamEventSinker,"connection to eventChannel is down, try to re-connect"));
}
...
bool StreamEventSinker::subscribe( )
{
...

	{
		try
		{
			Ice::CommunicatorPtr ic = mEnv._communicator;
			mTopicManager = IceStorm::TopicManagerPrx::checkedCast( ic->stringToProxy(mEndpoint) );
			hlog(ZQ::common::Log::L_INFO,CLOGFMT(StreamEventSinker,"connected to [%s] OK"), mEndpoint.c_str() );
		}
		catch( const Ice::Exception& ex )
		{
			hlog(ZQ::common::Log::L_ERROR, CLOGFMT(StreamEventSinker,"failed to connect to [%s] due to [%s]"), mEndpoint.c_str(),ex.ice_name().c_str() );
			return false;
		}
	...
	for( ; itSub != mSubscriber.end() ; itSub ++ )
	{
		...
		catch( const IceStorm::NoSuchTopic& )
		{
			...
			{
				mTopicManager = NULL;
				hlog(ZQ::common::Log::L_ERROR,CLOGFMT(StreamEventSinker,"failed to create topic[%s] due to [%s]"),
		...
		{
			mTopicManager = NULL;
			hlog(ZQ::common::Log::L_ERROR,CLOGFMT(StreamEventSinker,"failed to subscribe event due to [%s]"),ex.ice_name().c_str() );
			return false;
		}
