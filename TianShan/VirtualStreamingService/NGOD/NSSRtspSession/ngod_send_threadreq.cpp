...
int ngod_send_threadreq::run(void)
{
	...
	case SETUP:
		...
		m_pSendSock = m_pRTSPClientSession->RTSPSocket;
		break;
	case PLAY:
		RTSPMessage::PlayMessage(*m_pRTSPClientSession, pMessageBuf, SENDBUFMAXSIZE, sendLen);
		m_pSendSock = m_pRTSPClientSession->RTSPC1Socket;
		break;
	case PAUSE:
