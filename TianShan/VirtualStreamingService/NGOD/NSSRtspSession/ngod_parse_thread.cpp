	...
	while (1)
	{
		...
		if (m_NSSSessionGroupList->size() > 0)
		{
			...
			const char *tmpstr = (*queiter).c_str();
					...
					RTSPMessageParser::GetSessionID((*queiter).c_str(), (*queiter).length(), key.strSessionId, iTimeOut);
					...
					if (findIter == pMap->end())
					{
						...
						if (b == true)
						{
							...
							if (pHeader.strNotice_code.compare(strInProgress) == 0)
							{
								...
								tmpSess->iRTSPClientState = ANNOUNCE;
								EnterCriticalSection(&(*iter)->m_CS_ClientSeq);
								tmpSess->uServerCSeq = (*iter)->usClientSeq++;
								LeaveCriticalSection(&(*iter)->m_CS_ClientSeq);
								//tmpSess->strServerPath = (*iter)->strServerPath;
								//tmpSess->uServerPort = (*iter)->uServerPort;
								tmpSess->m_pCS = &((*iter)->m_CS);
								//::ZQ::common::URLStr strUrl;
								//strUrl.setProtocol("rtsp");
								//strUrl.setPort((*iter)->uServerPort);
								//strUrl.setPath(key.strSessionId.c_str());
								//strUrl.setHost((*iter)->strServerPath.c_str());
								tmpSess->m_RTSPR2Header.m_SDPResponseContent.strHost = (*iter)->strServerPath;
								tmpSess->m_RTSPR2Header.m_SDPResponseContent.strProtocol = "rtsp";
								tmpSess->m_RTSPR2Header.m_SDPResponseContent.uPort = (*iter)->uServerPort;
								tmpSess->m_RTSPR2Header.m_SDPResponseContent.strStreamhandle = key.strSessionId;
//								tmpSess->m_RTSPR2Header.m_SDPResponseContent.strStreamhandle = strUrl.generate();
					...
					else//get session and process the response
					{
						...
						if ((*findIter).second != NULL)
						{
							...
							if (b == true)
							{
								...
								else if (ptmpNoticeHeader.strNotice_code.compare(strStreamFailure) == 0)
								{
									//TODO: event sink, start of stream
									::ZQTianShan::NSS::listmem params;
									params.type = ::ZQTianShan::NSS::E_PLAYLIST_STATECHANGED;
									params.param[EventField_PlaylistGuid] = pSession->m_RTSPR2Header.strOnDemandSessionID;
									params.param[EventField_EventCSEQ] = (long)pSession->uClientCSeq;
									params.param[EventField_PrevState] = TianShanIce::Streamer::stsStreaming;
										...
										GetPramameterRes_ExtHeader &pHead)
{
	...
	iter++)
	{
		...
		if (findIter == pMap->end())
		{
			...
			//tmpSess->strServerPath = pGroup->strServerPath;
			//tmpSess->uServerPort = pGroup->uServerPort;
			tmpSess->m_RTSPR2Header.Reason.strReason_code = "428";
			tmpSess->m_RTSPR2Header.Reason.strReason_text = "Session List Mismatch";
