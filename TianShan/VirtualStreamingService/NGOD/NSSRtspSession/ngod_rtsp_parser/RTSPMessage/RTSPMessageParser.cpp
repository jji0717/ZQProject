...
uint16 RTSPMessageParser::GetContentLength(const char *pMessageBuf, uint16 usBufSize)
{
	...
	{
		std::string strContent = strMessage.substr(index + strHeader.length());

		//skip blank
		//strContent.erase(0,strContent.find_first_not_of(" ")); 
		string::iterator new_end = remove_if(strContent.begin(), strContent.end(), bind2nd(equal_to<char>(), ' '));
		strContent.erase(new_end, strContent.end()); 
		return atoi(strContent.c_str());
	}
}
...
uint16 RTSPMessageParser::GetSessionID(const char *pMessageBuf, uint16 usBufSize, string &pSessionID, int &iTimeOut)
{
	string strMessage = pMessageBuf;
	strMessage = str2Lower(strMessage);
	...
	else
	{
		...
		index = pSessionID.find(";");
		if (index != string::npos)
		{
			//get time out
			string pTimeout = pSessionID.substr(index + 1);
			pSessionID = pSessionID.substr(0, index);
			const char* strTimeout = strstr(pTimeout.c_str(), "timeout=");
			if (strTimeout != NULL)
			{
				iTimeOut = atoi(pTimeout.c_str() + strlen("timeout="));
			}
			else
			{
				iTimeOut = 60;//default timeout
			}
...
uint16 RTSPMessageParser::MessageParser(const char *pMessageBuf, uint16 usBufSize, RTSPClientSession &pRTSPClientSession)
{
	...
	RTSPMessageParser::GetSessionID(strMessageBody.c_str(), (uint16)strMessageBody.length(), pRTSPClientSession.m_RTSPR2Header.strSessionID, pRTSPClientSession.m_RTSPR2Header.iTimeOut);
...
uint16 RTSPMessageParser::GetParameterContentParser(const char *pMessageBuf, uint16 usBufSize, GetPramameterRes_ExtHeader &pGetPramameterRes_ExtHeader)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (index != string::npos)
		{
			int iHeaderIndex = int(index + strlen(pGetPramameter_ExtHeader[i]) + 2);
	...
	if (index != string::npos)
	{
		//std::string strSessList = strMessage.substr(index);
		//string::iterator new_end = replace_if(strSessList.begin(), strSessList.end(), bind2nd(equal_to<char>(), ' '), strEndLine.c_str());
		//strSessList.erase(new_end, strSessList.end());

		size_t iHeaderIndex = int(index + strSession_list.length());

		//add by lxm for support different split character
		std::string strSessList = strMessage.substr(iHeaderIndex);
		iHeaderIndex = strSessList.find_first_not_of(' ');
		if (iHeaderIndex != std::string::npos)
			strSessList = strSessList.substr(iHeaderIndex);
		if (strSessList.find(' ') != std::string::npos)
		{
			replace_if(strSessList.begin(), strSessList.end(), bind2nd(equal_to<char>(), ' '), '\r');
			strEndLine = "\r";
		}
		///////////////////////////////////////////////////

		char *pHead = &(strSessList[0]);
		char *pTail = NULL;
...
bool RTSPMessageParser::CheckGetParameterMessage(const char *pMessageBuf, uint16 usBufSize)
{
	...
	size_t index = strMessage.find(strNotice);
	if (index != string::npos && ParseSessionStatus(pMessageBuf, usBufSize) == RTSPOK)
		return true;
	else
	{
		index = strMessage.find(strSession_list);
