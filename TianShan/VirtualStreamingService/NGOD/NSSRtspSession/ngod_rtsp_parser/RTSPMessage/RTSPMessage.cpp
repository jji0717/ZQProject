...
unsigned short RTSPMessage::SetupMessage(RTSPClientSession &rtspClient, char *fSendBuffer, const unsigned short sBufferMaxSize, unsigned short &fSendBufferSize)
{
	...
	::std::stringstream ss;
	ss << cTime;
	iSDPLen += sprintf(pSDP + iSDPLen, "o=- %s %s IN IP4 %s", strOndemandSessionID.data(), ss.str().c_str(), rtspClient.strLocalIP.data());
	...
	for (SDPRequestContentVec::iterator iter = pSDPRequestContent->begin(); iter != pSDPRequestContent->end(); iter++)
	{
	...
	fSendBufferSize += RTSPMessageLine::TransportLine(rtspClient.m_RTSPR2Header.m_RTSPTransportHeader, fSendBuffer + fSendBufferSize, sBufferMaxSize - fSendBufferSize, rtspClient._serverType);
...
unsigned short RTSPMessage::PauseMessage(RTSPClientSession &rtspClient, char *fSendBuffer, const unsigned short sBufferMaxSize, unsigned short &fSendBufferSize)
{
	...
	fSendBufferSize += sprintf(fSendBuffer + fSendBufferSize, "Range: npt=now-\r\n");
...
unsigned short RTSPMessage::TeardownMessage(RTSPClientSession &rtspClient, char *fSendBuffer, const unsigned short sBufferMaxSize, unsigned short &fSendBufferSize)
{
	...
	fSendBufferSize += sprintf(fSendBuffer + fSendBufferSize, "Reason: %s \"%s\"\r\n", rtspClient.m_RTSPR2Header.Reason.strReason_code.c_str(), rtspClient.m_RTSPR2Header.Reason.strReason_text.c_str());
...
const char* RTSPMessage::stateStr(RTSPSessionState state)
{
#define SWITCH_CASE_STATE(_ST)	case _ST: return #_ST
	switch(state)
	{
		SWITCH_CASE_STATE(RTSPOK);
		SWITCH_CASE_STATE(BadRequest);
		SWITCH_CASE_STATE(SessionNotFound);
		SWITCH_CASE_STATE(DestUnreachable);
		SWITCH_CASE_STATE(UnknownSOPNames);
		SWITCH_CASE_STATE(InsufficientBandwidth);
	default:
		return "<Unknown>";
	}
...
#define SWITCH_CASE_STATE(_ST)	case _ST: return #_ST
	...
	fSendBufferSize = 0;
	fSendBufferSize += sprintf(fSendBuffer + fSendBufferSize, "RTSP/1.0 %d %s\r\n", rtspClient.iRTSPSessionState, stateStr(rtspClient.iRTSPSessionState));
