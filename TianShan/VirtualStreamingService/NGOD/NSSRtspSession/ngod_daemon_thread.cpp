...
const char* pComPath = "com.comcast.ngod";
...
int ngod_daemon_thread::run(void)
{
	...
	while (m_bRunToken)
	{
		...
		//cout << "send get_parameter over" << endl;
				...
				NSSSessionMap tmpMap((*iter)->m_NSSSessionMap);
				LeaveCriticalSection(&(*iter)->m_CS_SessionMap);
				for (NSSSessionMap::iterator iter1 = tmpMap.begin(); iter1 != tmpMap.end(); iter1++)
				{
					//::Ice::Identity ident;
					try
					{
						::TianShanIce::Streamer::NGODStreamServer::NGODStreamExPrx streamObj = ::TianShanIce::Streamer::NGODStreamServer::NGODStreamExPrx::checkedCast(_communicator->stringToProxy(iter1->second->strStreamName));
						if (streamObj != NULL)
							streamObj->renewPathTicket();
					}
					catch (Ice::Exception& ex)
					{
						(*m_pLogFile)(ZQ::common::Log::L_ERROR, CLOGFMT(ngod_daemon_thread, "caught Ice::Exception(%s) when renew pathticket session(%s) by sync list"), ex.ice_name().c_str(), iter1->second->strStreamName.c_str());
					}
					catch (...)
					{
						(*m_pLogFile)(ZQ::common::Log::L_ERROR, CLOGFMT(ngod_daemon_thread, "caught unknown exception when renew pathticket session(%s)"), iter1->second->strStreamName.c_str());
					}
				}
			}//for(...)	
		...
		{
			while (_streamerStrList.m_StreamStrList.empty() == false)
			{
				::std::string strStreamer = _streamerStrList.First();
				try
				{
					::TianShanIce::Streamer::StreamPrx streamObj = ::TianShanIce::Streamer::NGODStreamServer::NGODStreamExPrx::checkedCast(_communicator->stringToProxy(strStreamer));
					streamObj->destroy();
				}
				catch (Ice::Exception& ex)
				{
					(*m_pLogFile)(ZQ::common::Log::L_ERROR, CLOGFMT(ngod_daemon_thread, "caught Ice::Exception(%s) when destroy session(%s) by sync list"), ex.ice_name().c_str(), strStreamer.c_str());
				}
				catch (...)
				{
					(*m_pLogFile)(ZQ::common::Log::L_ERROR, CLOGFMT(ngod_daemon_thread, "caught unknown exception when destroy session(%s)"),strStreamer.c_str());
				}
			}
		}
	}//while (m_bRunToken)
...
bool ngod_daemon_thread::addSession(RTSPClientSession *sess, string &strSessGroup)
{
	...
	NSSSessionGroupList::iterator iter = find_if(m_NSSSessionGroupList->begin(), 
												 ...
												 FindBySessionGroup(strSessGroup));
	...
	{
		LeaveCriticalSection(&_CS);
		(*m_pLogFile)(ZQ::common::Log::L_ERROR, CLOGFMT(ngod_daemon_thread, "error when join NSS Session Map(idx:%s) with Session([%s])(already in map)"), strSessGroup.c_str(), key.strSessionId.c_str());
		return false;
	}
	else
	{
...
bool ngod_daemon_thread::addSessionGroupConnection(string &strSessGroup, const char *key, const char *ip, int &port, SOCKET *&sock)
{
	NSSSessionGroupList::iterator iter = find_if(m_NSSSessionGroupList->begin(), m_NSSSessionGroupList->end(), FindBySessionGroup(strSessGroup));

	if (iter == m_NSSSessionGroupList->end())
	{
		(*m_pLogFile)(ZQ::common::Log::L_ERROR, CLOGFMT(ngod_daemon_thread,"fail to find SessionGroup(%s) to add C1 connection info"), strSessGroup.c_str());
		sock = NULL;
		return false;
	}
	
	SessionGroupSocketMap::iterator sessionSocketIter = (*iter)->m_SessionGroupSocketMap.find(key);
	if (sessionSocketIter == (*iter)->m_SessionGroupSocketMap.end())
	{
		(*m_pLogFile)(ZQ::common::Log::L_INFO, CLOGFMT(ngod_daemon_thread,"fail to find SessionGroup(%s) socket, add a new socket to group"), strSessGroup.c_str());
		SessionGroupSocket *pSessionGrouopSocket = new SessionGroupSocket();
		pSessionGrouopSocket->_iServerPort = port;
		pSessionGrouopSocket->_strServerIp = ip;
		pSessionGrouopSocket->_sessSock.m_Socket = CreateSocket(TCPSOCKET);
		pSessionGrouopSocket->_sessSock.m_Status = true;

		if (bConnection(pSessionGrouopSocket->_strServerIp, pSessionGrouopSocket->_iServerPort, pSessionGrouopSocket->_sessSock.m_Socket, 1))
		{
			(*m_pLogFile)(ZQ::common::Log::L_INFO, CLOGFMT(ngod_daemon_thread,"SessionGroup(%s) create a new connection(%08x)"), strSessGroup.c_str(), pSessionGrouopSocket->_sessSock.m_Socket);
		}
		else
		{
			(*m_pLogFile)(ZQ::common::Log::L_WARNING, CLOGFMT(ngod_daemon_thread,"SessionGroup(%s) create a new connection(%08x) faild, need to re-connect next time"), strSessGroup.c_str(), pSessionGrouopSocket->_sessSock.m_Socket);
			pSessionGrouopSocket->_sessSock.m_Status = false;
		}
		...
		sock = &(pSessionGrouopSocket->_sessSock.m_Socket);
	}
	else
	{
		(*m_pLogFile)(ZQ::common::Log::L_INFO, CLOGFMT(ngod_daemon_thread,"SessionGroup(%s) connection(%08x) already exist"), strSessGroup.c_str(), sessionSocketIter->second->_sessSock.m_Socket);
		sock = &(sessionSocketIter->second->_sessSock.m_Socket);
	}
	return true;
}