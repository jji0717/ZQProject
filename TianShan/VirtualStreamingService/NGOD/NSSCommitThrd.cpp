...
int	NSSCommitThrd::run(void)
{
	...
	else
	{
		...
		if (!pContent.strProtocol.empty())
		{
			...
			pSess->m_streamCtrlURL.setProtocol(pContent.strProtocol.c_str());
		}
		if (!pContent.strHost.empty())
		{
			_NGODStreamObj->controlURl += "://" + pContent.strHost;
			pSess->m_streamCtrlURL.setHost(pContent.strHost.c_str());
		}
		if (pContent.uPort > 0)
		{
			stringstream ss;
			...
			pSess->m_streamCtrlURL.setPort(pContent.uPort);
		}
		...
		if (_NGODStreamObj->_env._daemonThrd->addSessionGroupConnection(pSess->m_RTSPR2Header.SessionGroup.strToken, pSess->m_streamCtrlURL.generate(), pSess->m_streamCtrlURL.getHost(), iPort, pSess->RTSPC1Socket))
		{
			_NGODStreamObj->_logFile(ZQ::common::Log::L_INFO, CLOGFMT(NGODStreamImpl,"NGODStreamImpl::commit_async() successfully setup sesion(%s) C1 connection(%08x) with video server"), _NGODStreamObj->ident.name.c_str(), *(pSess->RTSPC1Socket));
		}
		else
		{
			_NGODStreamObj->_logFile(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStreamImpl,"NGODStreamImpl::commit_async() fail to setup sesion(%s) C1 connection with video server"), _NGODStreamObj->ident.name.c_str());
		}
		////////////////////////////////////////
	}
