...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/TianShan/VirtualStreamingService/NGOD/NSSEnv.cpp $
// 
// 2     1/02/14 5:59p Hui.shao
// removed NullLogger
// 
...
// Created.
// 
// 2     09-04-30 17:20 Xiaoming.li
// change for CCUR and CISCO video server
// 
// 2     09-04-17 16:59 Xiaoming.li
// change for CCUR
// 
...
_nssEventSinkI(filelog, iceStormEndpoint)
{
	...
	{
		_logFile(ZQ::common::Log::L_ERROR,CLOGFMT(NSSEnv,"Create adapter failed and open db with endpoint=%s andcaught unknow exception"),endpoint);
	}
}

NSSEnv::~NSSEnv()
{
	::Ice::Identity ident = _adapter->getCommunicator()->stringToIdentity(DBFILENAME_NssSession);
