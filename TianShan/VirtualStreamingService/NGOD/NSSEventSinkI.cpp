...
NSSEventSinkI::NSSEventSinkI(::ZQ::common::FileLog &fileLog, const char * topicManagerEndpoint)
:_fileLog(fileLog)
{
	_bQuit=false;
	_bNetworkOK=false;
...
NSSEventSinkI::NSSEventSinkI(::ZQ::common::FileLog &fileLog, const char * topicManagerEndpoint, ZQADAPTER_DECLTYPE  adpater)
:_fileLog(fileLog),
_objAdapter(adpater)
{
	_bQuit=false;
...
void NSSEventSinkI::registerEventSink()
{
...
{
	NSSEventSinkI* pThis=(NSSEventSinkI*)pExtraData;
...
bool NSSEventSinkI::dispatchEvent(DWORD eventType,::ZQ::common::Variant& params)
{
	...
	do 
	{
		...
		if(!_bQuit)
		{
			if(!_bNetworkOK)
			{
				if (!_strTopicProxy.empty()) 
				{
					MYLOG(::ZQ::common::Log::L_DEBUG,CLOGFMT(NSSEventSinkI, "no connection to ice storm service,connect it now"));
					connectToStormService();
				}
			}
			else
			{
				try
				{
					...
					{
						::ZQTianShan::NSS::listmem tmpMem = _paramsList.First();
...
void NSSEventSinkI::NotifyBadConnection()
{
	//MYLOG(ZQ::common::Log::L_WARNING, CLOGFMT(NSSEventSinkI,"Ice storm down detected,connect it again"));
	_bNetworkOK=false;
	SetEvent(_hBadConn);
}

bool NSSEventSinkI::connectToStormService()
{
	try
	{
		if(_strTopicProxy.empty())
		...
		//MYLOG(::ZQ::common::Log::L_INFO,CLOGFMT(NSSEventSinkI,"connect to ice storm service with %s"),_strTopicProxy.c_str());
		...
		MYLOG(::ZQ::common::Log::L_DEBUG,CLOGFMT(NSSEventSinkI, "connect to eventchannel service %s successfully!"),_strTopicProxy.c_str());
	}
	catch (Ice::Exception& ex) 
	{
		MYLOG(::ZQ::common::Log::L_ERROR,CLOGFMT(NSSEventSinkI, "Catch %s when connect to ice Storm %s"),	ex.ice_name().c_str(),_strTopicProxy.c_str());
		return false;
	}
	catch (...)
	{
		MYLOG(::ZQ::common::Log::L_ERROR,CLOGFMT(NSSEventSinkI, "Connect to eventchannel service %s failed"),_strTopicProxy.c_str());
		return false;
	}
	_bNetworkOK=true;
	return true;
