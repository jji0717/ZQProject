...
static ConfItem IpEdge_Cisco[] = {
	{ CiscoTotalBandwidth,	::TianShanIce::vtLongs,		false,	"20000",	false }, // in Kbps
	{ CiscoMaxStreamCount,	::TianShanIce::vtInts,		false,	"80",		false },
	{ NULL,					::TianShanIce::vtInts,		true,	"",			false },
	};

PHOCisco::PHOCisco(IPHOManager& mgr)
: IStreamLinkPHO(mgr)
{
	_phoManager=&mgr;
}
...
PHOCisco::~PHOCisco()
{
	_helperMgr.unregisterStreamLinkHelper( STRMLINK_TYPE_CISCO );
}
bool PHOCisco::getSchema(const char* type, ::TianShanIce::PDSchema& schema)
{
	::ZQTianShan::ConfItem *config = NULL;

	// address the schema definition
	if ( 0 == strcmp(type, STRMLINK_TYPE_CISCO) )
		config = IpEdge_Cisco;
...
void PHOCisco::validateConfiguration(const char* type, const char* identStr, ::TianShanIce::ValueMap& configPD)
{
	if (NULL == type)
	{
		type = "NULL";
		ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>("PHOCisco",1001,"unsupported type [%s] in PHOCisco", type);
	}
	if (0 == strcmp( STRMLINK_TYPE_CISCO, type))
	{
...
void PHOCisco::validate_IPStrmLinkConfig(const char* identStr, ::TianShanIce::ValueMap& configPD)
{
	glog(ZQ::common::Log::L_DEBUG,
		CLOGFMT(PHOCisco, " enter validate a IP StreamLink's Configuration: link[%s]"), 
		identStr );
	LinkIPAttr lia;
	...
	try
	{
		...
		//get TotalBandwidth
		var = PDField( configPD , CiscoTotalBandwidth);
		if( var.type!=::TianShanIce::vtLongs)
		{
			ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>("IpEdgaPHO",1011,"Invalid %s type,should be vtLongs", CiscoTotalBandwidth);
		}
		if ( var.lints.size() == 0 ) 
		{
			ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>("IpEdgaPHO",1012,"Invalid %s ,lints size() == 0", CiscoTotalBandwidth);
		}
		lia._totalBandwidth		= var.lints[0]*1000;
		lia._usedStreamCount	= 0 ;
		lia._usedBandwidth		= 0;
		glog(ZQ::common::Log::L_INFO , 
					CLOGFMT(PHOCisco,"stream IP link[%s] get totalBW[%lld]"),
					identStr, lia._totalBandwidth);

		//try to get max stream count
		try
		{
			var = PDField(configPD,CiscoMaxStreamCount);
			if ( var.type == TianShanIce::vtInts && var.ints.size() > 0) 
			{
				lia._maxStreamCount		= var.ints[0];
				lia._usedStreamCount	= 0;
				glog(ZQ::common::Log::L_INFO, CLOGFMT(PHOCisco,"stream IP link[%s] get totalStreamCount[%lld]"), identStr, lia._maxStreamCount);
			}
			else
			{
				lia._maxStreamCount		= 80;
				lia._usedStreamCount		= 0;
				glog(ZQ::common::Log::L_WARNING, CLOGFMT(PHOCisco,"stream IP link[%s] ,invalid %s type or data,set it to default [80]"), CiscoMaxStreamCount, identStr);
			}
		}
		catch( const ::TianShanIce::InvalidParameter&  )
		{
			...
			lia._usedStreamCount = 0;
			glog(ZQ::common::Log::L_WARNING, CLOGFMT(PHOCisco,"stream IP link[%s] ,%s is not found,set it to default [80]"), CiscoMaxStreamCount, identStr);
		}
				...
				for( ; itID != idc.end() ; itID ++ )
				{
					...
					if ( ticketprx )
					{
						...
						try
						{
							...
							{
								glog(ZQ::common::Log::L_DEBUG,
									CLOGFMT(PHOCisco,"validate_IPStrmLinkConfig ticket %s but no bandwidth resource"),
									itID->name.c_str());
							}
							else
							{	
								...
								if (bandwidth.lints.size()==0) 
								{
									glog(ZQ::common::Log::L_WARNING,
										CLOGFMT(PHOCisco,"validate_IPStrmLinkConfig() ticket %s found a invalid bandwidth,maybe db error"),
										itID->name.c_str());
									continue;
								}
								glog(ZQ::common::Log::L_DEBUG,CLOGFMT(PHOCisco,"validate_IPStrmLinkConfig() ticket %s used bandwidth %lld"),itID->name.c_str(),bandwidth.lints[0]);
								lia._usedBandwidth += bandwidth.lints[0];
								lia._usedStreamCount ++;	

								ResourceIPData rid;
								//rid._usedBandwidth = bandwidth.lints[0];
								rid._usedStreamCount++;
								{
						...
						catch(::TianShanIce::InvalidParameter& e)
						{
							glog(ZQ::common::Log::L_ERROR,
								CLOGFMT(PHOCisco,"validate_IPStrmLinkConfig() invalidParameter exception is caught:%s"),
								e.message.c_str());
						}
						catch (...)
						{
		...
		glog(ZQ::common::Log::L_INFO,
			CLOGFMT(PHOCisco,"validate_IPStrmLinkConfig() update  ip streamlink with streamlink id [%s] "
								"totalBW[%lld] usedBW[%lld] maxStreamCount[%d] usedStrmCount[%d]"),
								identStr,   
	...
	{
		glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"validate_IPStrmLinkConfig() catch a ice exception :%s"),e.ice_name().c_str());
	}
	catch (...)
	{
		glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"validate_IPStrmLinkConfig() unexpect error"));
	}
	glog(ZQ::common::Log::L_DEBUG, CLOGFMT(PHOCisco, "leave validate a IP StreamLink's Configuration: link[%s]"), identStr);
}
...
{ ::TianShanIce::SRM::Resource& res = _RESMAP[::TianShanIce::SRM::_RESTYPE]; \
 ...
 else glog(ZQ::common::Log::L_WARNING, CLOGFMT(PHOCisco, "unacceptable " #_RESTYPE " in session: " #_RESFILED "(range=%d, size=%d)"), res.resourceData[#_RESFILED].bRange, res.resourceData[#_RESFILED]._RESFIELDDATA.size()); \
}

Ice::Int PHOCisco::doEvaluation(LinkInfo& linkInfo, 
								 const SessCtx& sessCtx,
								 TianShanIce::ValueMap& hintPD,
								 const ::Ice::Int oldCost)
{
	std::string &linktype =linkInfo.linkType;
	if (0 == linktype.compare(STRMLINK_TYPE_CISCO))
	{
		return eval_IPStrmLink(linkInfo, sessCtx, hintPD, linkInfo.rcMap,oldCost);
	}
	else
	{
		glog(ZQ::common::Log::L_ERROR , CLOGFMT(PHOCisco , "unrecognized streamlink type[%s]" ),linktype.c_str());
		return ::TianShanIce::Transport::OutOfServiceCost;
	}
}
Ice::Int PHOCisco::eval_IPStrmLink(LinkInfo& linkInfo,
									const SessCtx& sessCtx, 
									TianShanIce::ValueMap& hintPD,
									...
									const ::Ice::Int oldCost)
{
	...
	{
		glog(ZQ::common::Log::L_ERROR,
			CLOGFMT(PHOCisco,"eval_IPStreamLink() Session [%s] no Streamlink is attached,return with OutOfServiceCost"),
			sessId.c_str());
		return ::TianShanIce::Transport::OutOfServiceCost;
	}
	glog(ZQ::common::Log::L_DEBUG , 
			CLOGFMT(PHOCisco , "eval_IPStreamLink() session[%s] enter evaluation with oldCost[%d]" ) , 
			sessId.c_str() , oldCost );
	if ( newCost > ::TianShanIce::Transport::MaxCost )
	{	
		glog(ZQ::common::Log::L_ERROR,
			CLOGFMT(PHOCisco, "eval_IPStreamLink() Session[%s] return oldCost[%d] because oldCost>=MaxCost"),
	...
	{
		glog(ZQ::common::Log::L_ERROR, 
			CLOGFMT(PHOCisco, "eval_IPStreamLink() Session[%s] 0 bandwidth has been specified, quit evaluation"),
			sessId.c_str());
		return ::TianShanIce::Transport::OutOfServiceCost;
	}
	...
	{
		glog(ZQ::common::Log::L_ERROR,
			CLOGFMT(PHOCisco, "eval_IPStreamLink() Session[%s]  illegal destination [%s]:[%d] has been specified, quit evaluation"), 
	...
	glog(ZQ::common::Log::L_INFO, 
		CLOGFMT(PHOCisco, "eval_IPStreamLink() Session[%s] requested allocation: [%s][%s]:[%d]; bandwidth:[%lld]bps"),
		sessId.c_str(),
		destAddr.c_str(),
		destMac.c_str(), 
		destPort, 
...

	{
		...
		{
			glog(ZQ::common::Log::L_ERROR,
				CLOGFMT(PHOCisco,"Session[%s] Can't find the streamlink attr through the id [%s]"),
	...
	//{
	//	glog(ZQ::common::Log::L_ERROR,
	//		CLOGFMT(PHOCisco,"Session[%s] no enough bandwidth ,Required bw[%lld] and used BW[%lld] totalBW[%lld]"),
	...
	{
		glog(ZQ::common::Log::L_ERROR,
			CLOGFMT(PHOCisco,"Session[%s] not enough StreamCount, totalStrmCount[%d] usedStrmCount[%d],no available strmCount "),
			sessId.c_str(), totalStrmCount ,usedStrmCount);
		return TianShanIce::Transport::OutOfServiceCost;
	}	
	glog(ZQ::common::Log::L_DEBUG,
		CLOGFMT(PHOCisco,"Session[%s] current totalBW[%lld] usedBW[%lld] MaxStreamCount[%d] usedStreamCount[%d] and required BW[%lld]"),
	...
	glog(ZQ::common::Log::L_INFO,
		CLOGFMT(PHOCisco,"Session[%s] bandwidth cost[%d] with usedBandwidth[%lld] totalBandwidth[%lld]"),
		sessId.c_str() , newCost, usedBW, totalBW );
	int		strmCountCost = usedStrmCount*::TianShanIce::Transport::MaxCost / totalStrmCount;

	glog(ZQ::common::Log::L_INFO,
		CLOGFMT(PHOCisco,"Session[%s] streamcount cost[%d] with usedStreamCount[%d] totalStreamCount[%lld]"),
	...
	if ( newCost <= ::TianShanIce::Transport::MaxCost )
	{
		try
		{
			...
			glog(ZQ::common::Log::L_DEBUG,
				CLOGFMT(PHOCisco,"eval_IPStrmLink() Session[%s] set bandwidth to %lld"),
			...
			glog(ZQ::common::Log::L_DEBUG,
				CLOGFMT(PHOCisco,"eval_IPStrmLink() Session[%s] set destPort to %d"),
			...
			glog(ZQ::common::Log::L_DEBUG,
				CLOGFMT(PHOCisco,"eval_IPStrmLink() Session[%s] set destAddr to %s"),
			...
			if ( !destMac.empty() )
			{
				...
				glog(ZQ::common::Log::L_DEBUG,
					CLOGFMT(PHOCisco,"eval_IPStrmLink() Session[%s] set destMac to %s"),
			...
			{
				glog(ZQ::common::Log::L_DEBUG,
					CLOGFMT(PHOCisco,"eval_IPStrmLink() Session[%s] no destMac"),
					sessId.c_str() );
			}
		...
		{
			glog(ZQ::common::Log::L_ERROR, 
				CLOGFMT(PHOCisco, "eval_IPStreamLink() Session[%s] exception caught while filling in the ticket private data"),
				sessId.c_str());
			return ::TianShanIce::Transport::OutOfServiceCost;
		}
	}
	glog(ZQ::common::Log::L_DEBUG,
		CLOGFMT(PHOCisco,"eval_IPStreamLink() Session[%s] return with newCost=%d"),
		sessId.c_str(),max(oldCost,newCost));
	// step 4. return the higher as the cost
	return max(oldCost, newCost);
}

IPathHelperObject::NarrowResult PHOCisco::doNarrow(const ::TianShanIce::Transport::PathTicketPtr& ticket, 
													const SessCtx& sessCtx)
{
	if (!ticket)
	...
	if ( 0 == type.compare( STRMLINK_TYPE_CISCO ) )
	{
	...
	{
		glog( ZQ::common::Log::L_ERROR,
			CLOGFMT(PHOCisco , "doNarrow() unrecognized stream link type [%s]" ),
			type.c_str() );
		return IPathHelperObject::NR_Unrecognized;
	}
}

IPathHelperObject::NarrowResult PHOCisco::narrow_IPStrmLink(::TianShanIce::Transport::StreamLinkPrx& strmLink,
															 const SessCtx& sessCtx,
															 const TianShanIce::Transport::PathTicketPtr&  ticket)
{
	...
	{
		glog(ZQ::common::Log::L_ERROR, CLOGFMT(PHOCisco,"narrow_IPStrmLink() caught [%s] when get streamlink's identity" ),
			ex.ice_name().c_str() );
		return NR_Error;
	}
	catch (...) 
	{
		glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"narrow_IPStrmLink() get streamlink's ID failed"));
		return NR_Error;
	}
	...
	glog(ZQ::common::Log::L_DEBUG,
			CLOGFMT(PHOCisco,"Session[%s] narrowed ticket with ticketid[%s]"),
...

	{
		...
		{
			glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"narrow_IPStrmLink() Session[%s] No streamlink with ID[%s] is found"),
		...
		{
			glog(ZQ::common::Log::L_ERROR, CLOGFMT(PHOCisco,"narrow_IPStrmLink() Session[%s] not enough streamcount, totalStrmCount[%d] usedStrmCount[%d] for strmLink[%s]"),
				sessId.c_str(), ipAttr._maxStreamCount, ipAttr._usedStreamCount, strLinkIdent.c_str() );
			return NR_Error;
		}
		//if ( ipAttr._usedBandwidth + bw2Alloc > ipAttr._totalBandwidth) 
		//{
		//	glog(ZQ::common::Log::L_ERROR,
		//		CLOGFMT(PHOCisco,"narrow_IPStrmLink() Session[%s] not enough bandwidth,totalBandwidth[%d] usedBandwidth[%s] for strmLink[%s]"),
	...
	varLinkType.strs.push_back( STRMLINK_TYPE_CISCO );
	...
	glog(ZQ::common::Log::L_INFO , 
				CLOGFMT(PHOCisco , "narrow_IPStrmLink() Session[%s] set sopname[%s]" ),
				sessId.c_str() ,
				strSopName.c_str() );

	glog(ZQ::common::Log::L_INFO,CLOGFMT(PHOCisco,"narrow_IPStrmLink() Session[%s] narrowed with NeedBW[%lld]"
	...
	{
		ZQ::common::MutexGuard gd(_ipResourceLocker);
		rid._usedStreamCount = 1;
		_ipResourceDataMap[ticketID] = rid;
	}
	return NR_Narrowed;
}

void PHOCisco::doFreeResources(const ::TianShanIce::Transport::PathTicketPtr& ticket)
{
	try
	{
		...
		{
			glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"no ticket owner link type is found"));
			return;
		}
		...
		{
			glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"doFreeResource() can' get streamlink from ticket"));
			return ;
		}
		std::string strStreamlinkID = itLinkId->second.strs[0];
		Ice::Identity& ticketID = ticket->ident;

		if ( strcmp( STRMLINK_TYPE_CISCO , strLinkType.c_str() ) == 0 ) 
		{
			ZQ::common::MutexGuard gd(_ipResourceLocker);
			...
			{
				glog(ZQ::common::Log::L_INFO,
					CLOGFMT(PHOCisco,"doFreeResource() streamlink [%s] ticket[%s] no allocated IP resource"),
			...
			{
				glog(ZQ::common::Log::L_ERROR,
					CLOGFMT(PHOCisco,"doFreeResource() streamlink [%s] ticket[%s] can't find strmlink attr "),
					strStreamlinkID.c_str(),ticketID.name.c_str());
				return;
			}
...
//			ResourceIPData& rid		= itAlloc->second;
			...
			glog(ZQ::common::Log::L_INFO ,
				CLOGFMT (PHOCisco , "doFreeResource() streamLink[%s] free resource and now "
		...
		{
			glog(ZQ::common::Log::L_WARNING , CLOGFMT(PHOCisco,"unrecognized stream link type [%s]") , strLinkType.c_str() );
			return ; 
		}
	...
	{
		glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"doFreeResource() catch a tianshan exception:%s"),ex.message.c_str());
		return;
	}
	catch( const Ice::Exception& e)
	{
		glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"doFreeResource() catch a ice exception:%s"),e.ice_name().c_str());
		return;
	}
	catch (...)
	{
		glog(ZQ::common::Log::L_ERROR,CLOGFMT(PHOCisco,"doFreeResource() catch a unknown exception"));
		return;
	}	
}
void PHOCisco::doCommit(const ::TianShanIce::Transport::PathTicketPtr& ticket, const SessCtx& sessCtx)
{
