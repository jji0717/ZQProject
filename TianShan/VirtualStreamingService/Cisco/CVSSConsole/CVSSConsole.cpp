...
int main(int argc, char* argv[])
{
	...
	::ZQ::common::NativeThreadPool _pCVSSThreadPoolSend(10);
	_pCVSSThreadPoolSend.resize( pConfig._rtspProp.threadPoolSize> 10 ? pConfig._rtspProp.threadPoolSize : 10 );
	...
	::ZQTianShan::CVSS::CVSSEnv *_pCVSSEnv = new ::ZQTianShan::CVSS::CVSSEnv(_fileLog, _pCVSSThreadPool, _ic, pConfig._iceStorm.endPoint.c_str(), pConfig._bind.endPoint.c_str(), pConfig._dataBase.path.c_str(), pConfig._dataBase.runtimePath.c_str());

	_cps = new ::ZQTianShan::CVSS::CiscoVirtualStreamServiceImpl(_fileLog, _pCVSSThreadPool, _pCVSSThreadPoolSend,  strServerPath,uServerPort, pConfig._bind.evictorSize, *_pCVSSEnv);
	...
	::TianShanIce::Storage::VolumePrx volumePrx;

	try{
		_contentStore->initializeContentStore();
		::std::string CSPath = pConfig._storeInfo._volumeInfo.path + FNSEPS;
		volumePrx = _contentStore->mountStoreVolume(pConfig._storeInfo._volumeInfo.name.c_str(), CSPath.c_str(), true);
	}
	catch(...)
	{
	...
	_iceLog = NULL;
	printf("\rCVSS Console is quiting                   \r");
	...
	_pCVSSThreadPool.stop();
	_pCVSSThreadPoolSend.stop();
	delete _pCVSSEnv;
	_pCVSSEnv = NULL;
