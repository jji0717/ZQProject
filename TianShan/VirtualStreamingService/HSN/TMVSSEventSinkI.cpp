...
TMVSSEventSinkI::TMVSSEventSinkI(::ZQ::common::FileLog &fileLog, const char * topicManagerEndpoint)
:_fileLog(fileLog)
{
	_bQuit=false;
	_bNetworkOK=false;
...
TMVSSEventSinkI::TMVSSEventSinkI(::ZQ::common::FileLog &fileLog, const char * topicManagerEndpoint, ZQADAPTER_DECLTYPE  adpater)
:_fileLog(fileLog),
_objAdapter(adpater)
{
	_bQuit=false;
...
TMVSSEventSinkI::~TMVSSEventSinkI()
{
	if (_bQuit == false)
	{
		_bQuit=true;
...
void TMVSSEventSinkI::registerEventSink()
{
...
{
	TMVSSEventSinkI* pThis=(TMVSSEventSinkI*)pExtraData;
...
bool TMVSSEventSinkI::dispatchEvent(DWORD eventType,::ZQ::common::Variant& params)
{
	...
	do 
	{
		...
		if(!_bQuit)
		{
			if(!_bNetworkOK)
			{
				if (!_strTopicProxy.empty()) 
				{
					MYLOG(::ZQ::common::Log::L_DEBUG,CLOGFMT(TMVSSEventSinkI, "no connection to ice storm service,connect it now"));
					connectToStormService();
				}
			}
			else
			{
				try
				{
					...
					{
						::ZQTianShan::VSS::TM::listmem tmpMem = _paramsList.First();
...
void TMVSSEventSinkI::NotifyBadConnection()
{
	//MYLOG(ZQ::common::Log::L_WARNING, CLOGFMT(TMVSSEventSinkI,"Ice storm down detected,connect it again"));
	_bNetworkOK=false;
	SetEvent(_hBadConn);
}

bool TMVSSEventSinkI::connectToStormService()
{
	try
	{
		if(_strTopicProxy.empty())
		...
		//MYLOG(::ZQ::common::Log::L_INFO,CLOGFMT(TMVSSEventSinkI,"connect to ice storm service with %s"),_strTopicProxy.c_str());
		...
		MYLOG(::ZQ::common::Log::L_DEBUG,CLOGFMT(TMVSSEventSinkI, "connect to ice storm service %s successfully!"),_strTopicProxy.c_str());
	}
	catch (Ice::Exception& ex) 
	{
		MYLOG(::ZQ::common::Log::L_ERROR,CLOGFMT(TMVSSEventSinkI, "Catch %s when connect to ice Storm %s"),	ex.ice_name().c_str(),_strTopicProxy.c_str());
		return false;
	}
	catch (...)
	{
		MYLOG(::ZQ::common::Log::L_ERROR,CLOGFMT(TMVSSEventSinkI, "Connect to Ice Storm service %s failed"),_strTopicProxy.c_str());
		return false;
	}
	_bNetworkOK=true;
	return true;
...
bool TMVSSEventSinkI::SetHandle()
{
