...
VLCVSSEventSinkI::VLCVSSEventSinkI(::ZQ::common::FileLog &fileLog, const char * topicManagerEndpoint)
:_fileLog(fileLog)
{
	_bQuit=false;
	_bNetworkOK=false;
...
VLCVSSEventSinkI::VLCVSSEventSinkI(::ZQ::common::FileLog &fileLog, const char * topicManagerEndpoint, ZQADAPTER_DECLTYPE  adpater)
:_fileLog(fileLog),
_objAdapter(adpater)
{
	_bQuit=false;
...
VLCVSSEventSinkI::~VLCVSSEventSinkI()
{
	if (_bQuit == false)
	{
		_bQuit=true;
...
void VLCVSSEventSinkI::registerEventSink()
{
...
{
	VLCVSSEventSinkI* pThis=(VLCVSSEventSinkI*)pExtraData;
...
bool VLCVSSEventSinkI::dispatchEvent(DWORD eventType,::ZQ::common::Variant& params)
{
	...
	do 
	{
		...
		if(!_bQuit)
		{
			if(!_bNetworkOK)
			{
				if (!_strTopicProxy.empty()) 
				{
					MYLOG(::ZQ::common::Log::L_DEBUG,CLOGFMT(VLCVSSEventSinkI, "no connection to EventChannel service,connect it now"));
					connectToStormService();
				}
			}
			else
			{
				try
				{
					...
					{
						::ZQTianShan::VSS::VLC::listmem tmpMem = _paramsList.First();
...
void VLCVSSEventSinkI::NotifyBadConnection()
{
	//MYLOG(ZQ::common::Log::L_WARNING, CLOGFMT(VLCVSSEventSinkI,"EventChannel down detected,connect it again"));
	_bNetworkOK=false;
	SetEvent(_hBadConn);
}

bool VLCVSSEventSinkI::connectToStormService()
{
	try
	{
		if(_strTopicProxy.empty())
		...
		//MYLOG(::ZQ::common::Log::L_INFO,CLOGFMT(VLCVSSEventSinkI,"connect to ice storm service with %s"),_strTopicProxy.c_str());
		...
		MYLOG(::ZQ::common::Log::L_DEBUG,CLOGFMT(VLCVSSEventSinkI, "connect to EventChannel service %s successfully!"),_strTopicProxy.c_str());
	}
	catch (Ice::Exception& ex) 
	{
		MYLOG(::ZQ::common::Log::L_ERROR,CLOGFMT(VLCVSSEventSinkI, "Catch %s when connect to EventChannel %s"),	ex.ice_name().c_str(),_strTopicProxy.c_str());
		return false;
	}
	catch (...)
	{
		MYLOG(::ZQ::common::Log::L_ERROR,CLOGFMT(VLCVSSEventSinkI, "Connect to EventChannel service %s failed"),_strTopicProxy.c_str());
		return false;
	}
	_bNetworkOK=true;
	return true;
...
bool VLCVSSEventSinkI::SetHandle()
{
