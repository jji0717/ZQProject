...
#define DebugLevel ZQ::common::Log::L_DEBUG
	...
	int SessionOnTimerCmd::run(void)
	{
		try
		{
			glog(InfoLevel, CLOGFMT(SessionOnTimerCmd, "session timeout, SessionID(%s)"), _sessID.c_str());
			Ice::Identity ident;
			ident.name = _sessID;
		...
		{
			glog(ErrorLevel, CLOGFMT(SessionOnTimerCmd, "SessionOnTimerCmd for %s caught unexpect exception"), _sessID.c_str());
		}
	...
	bool WatchDog::init(void)
	{
		...
		bool bRet = (NULL != _event && NULL != _pThreadPool) ? true : false;
		if (true == bRet)
			glog(NoticeLevel, CLOGFMT(WatchDog, "SessionWatchDog initialize successfully"));
		return bRet;
	}
	...
	{
		glog(NoticeLevel, CLOGFMT(WatchDog, "SessionWatchDog exist, %d session under watch now"), getWatchSize());
	}
	...
	void WatchDog::watch(std::string ident, __int64 milliSec)
	{
		...
		glog(DebugLevel, CLOGFMT(WatchDog, "watch session(%s) for %lldms, total session(%d)"), ident.c_str(), milliSec, _watchList.size() + _watchMap.size());
