...
#define	AppServiceName	"Application"
...
//	<DWH enable="1" endpoint="MoDIceInterface:tcp -h 10.0.0.0 -p 2222 -t 5000" activeConnections="8" />

struct DWH
{
	int32 _enable;
	std::string _endpoint;
	int32 _activeConnections;

	typedef ZQ::common::Config::Holder<DWH> DWHHolder;

	static void structure(DWHHolder& holder);
};
typedef DWH::DWHHolder DWHHolder;
...
struct TSConfig
{
	...
	typedef std::vector<LogPublish> LogPublishs;
	...
	void readDWH(ZQ::common::XMLUtil::XmlNode node, const ZQ::common::Preprocessor* hPP);

	void registerDWH(const std::string &full_path);
...
extern ZQ::common::Config::Loader<HSNTree::TSConfig> _tsConfig;
