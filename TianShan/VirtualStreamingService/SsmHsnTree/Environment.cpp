...
ZQ::common::Config::Loader<HSNTree::TSConfig> _tsConfig("");
...
bool Environment::doInit(const char* cfgPath, IStreamSmithSite* pSite)
{
	...
	_cfgDir += "\\ssm_hsntree.xml";

	try
	{
		_fileLog.open(_logDir.c_str(), 7);
	}
	...
	try
	{
		...
		glog(NoticeLevel, CLOGFMT(Environment, "Register SessionView successfully"));
	}
	catch (const Ice::Exception& ex)
	{
		glog(WarningLevel, CLOGFMT(Environment, "Register SessionView caught %s"), ex.ice_name().c_str());
	}
	catch (...)
	{
		glog(WarningLevel, CLOGFMT(Environment, "Register SessionView caught unexpected exception"));
	}

// implementation of interface TianShanIce::Application::AppService
	try
	{
		HsnTreeAppImplPtr pAppImpl = new HsnTreeAppImpl(*this);
		_pAdapter->ZQADAPTER_ADD(_pCommunicator, pAppImpl, AppServiceName);
//		PurchaseImplPtr purPtr = new PurchaseImpl();	
//		HsnTreeAppImpl::purPrx = ::TianShanIce::Application::PurchasePrx::uncheckedCast(_pAdapter->addWithUUID(purPtr));
		glog(NoticeLevel, CLOGFMT(Environment, "Add Application successfully"));
	}
	catch (const Ice::Exception& ex)
	{
		glog(WarningLevel, CLOGFMT(Environment, "Add Application caught %s"), ex.ice_name().c_str());
	}
	catch (...)
	{
		glog(WarningLevel, CLOGFMT(Environment, "Add Application caught unexpected exception"));
	}
...

	{
		...
		std::vector<LogPublish>::const_iterator it  = _tsConfig._logpublish.begin();
		for( ; it != _tsConfig._logpublish.end() ; it ++ )
		{
			if(!_pAdapter->publishLogger( it->logPath.c_str() , it->syntaxPath.c_str(),  it->key.c_str(), it->type.c_str()) )
			{
				glog(ZQ::common::Log::L_ERROR, "Failed to publish logger name[%s] synax[%s]", 					
					it->logPath.c_str(), it->syntaxPath.c_str());				
			}
			else
			{
				glog(ZQ::common::Log::L_INFO, "Publish logger name[%s] synax[%s] successful", 					
					it->logPath.c_str(), it->syntaxPath.c_str());				
			}
		}
	}
...
RequestProcessResult Environment::doFixupRequest(IStreamSmithSite* pSite, IClientRequestWriter* pReq)
{
	...
	if (NULL == pSite)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "site is null"));
		throw "site is null";
	}
	
	if (NULL == pReq)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "request is null"));
	...
	if (NULL == pResponse)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "response is null"));
	...
	case RTSP_MTHD_PAUSE:
		{
			pResponse->printf_preheader(ResponseNotAcceptable);
			pResponse->post();
			return RequestDone;
		}
		break;
	...
	catch (...)
	{
		...
		glog(ErrorLevel, CLOGFMT(Environment, "Req(%s:FixupRequest) caught unexpect exception")
	...
	try 
	{
		...
		if (false == ret)
		{
			...
			if (RTSP_MTHD_SETUP == verbCode && _pSite->destroyClientSession(pContentHandler->getSession().c_str()))
				glog(InfoLevel, CLOGFMT(Environment, "RtspProxySessioin(%s) destroyed, reason: Setup Failed"), pContentHandler->getSession().c_str());
...
// 				pContentHandler = NULL;
	...
	catch (...)
	{
		...
		glog(ErrorLevel, CLOGFMT(Environment, "session(%s), phase(%s), process(%s) caught unexpect exception")
...
RequestProcessResult Environment::doFixupResponse(IStreamSmithSite* pSite, IClientRequest* pReq)
{
	...
	switch (pReq->getVerb())
	{
	case RTSP_MTHD_SETUP:				pFixupResponse = new SetupResponse(*this, pSite, pReq, pResponse); break;
	...
	catch (...)
	{
		...
		glog(ErrorLevel, CLOGFMT(Environment, "Req(%s:FixupResponse) caught unexpect exception")
...
{
	_logDir = NULL != logDir ? logDir : "";
	_logDir += "\\ssm_hsn_tree.log";
}

bool Environment::loadConfig()
{
	glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "**************** (Load %s) ****************"), ZQ_PRODUCT_NAME);
	if(!_tsConfig.load(_cfgDir.c_str()))
    {
        glog(ZQ::common::Log::L_INFO, "load config %s failed", _cfgDir.c_str());
        return false;
    }
	...
	for( std::vector<std::string>::const_iterator it = temps.begin() ; it != temps.end() ; it ++ )
	{
		...
		glog(ZQ::common::Log::L_INFO,"get a forward speed [%s]",it->c_str());
	}
	...
	for( std::vector<std::string>::const_iterator it = temps.begin() ; it != temps.end() ; it ++ )
	{
		...
		glog(ZQ::common::Log::L_INFO,"get a backward speed [%s]",it->c_str());
	}
	glog(ZQ::common::Log::L_INFO," [%s] FixedSpeedSet",_tsConfig._fixedSpeedSet.enable ? "enable":"disable");
...
bool Environment::initIceRuntime()
{
	...
	try
	{
		...
		std::string iceLogDir = _logDir += "\\ssm_hsn_tree.icetrace.log";
		if( _tsConfig._iceTrace._enabled >= 1 )
		{
			...
			{
				glog(ZQ::common::Log::L_INFO, CLOGFMT(Environment, "failed to create ice trace file log"));
			}
		}
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "create communicator caught(%s)"), ex.ice_name().c_str());
		return false;
	}
	
	try
	{
		_pAdapter = ZQADAPTER_CREATE(_pCommunicator, "LocalAdapter", _tsConfig._bind._endpoint.c_str(), glog);
		_pAdapter->activate();
	}
	catch(Ice::Exception& ex) 
	{
		glog(ErrorLevel, CLOGFMT(Environment, "create adapter(%s) caught(%s)"), _tsConfig._bind._endpoint.c_str(), ex.ice_name().c_str());
		return false;
	}
...
void Environment::uninitIceRuntime()
{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "destroy event Channel adapters caught(%s)"), ex.ice_name().c_str());
	}
	...
	try
	{
		if (NULL != _pAdapter)
		{
			glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "do _pAdapter->deactivate()"));
			_pAdapter->deactivate();
			glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "_pAdapter->deactivate() successfully"));
		}
		_pAdapter = NULL;
	}
	catch (Ice::Exception& ex)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "deactive adapters caught(%s)"), ex.ice_name().c_str());
	}
...
{
	glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "do openSafeStore()"));
	_tsConfig._database._path += "\\ssm_hsn_tree";
	std::string pathStr(_tsConfig._database._path);
	std::vector<std::string> paths;
	std::string tmp_path;
	if (pathStr[pathStr.size() - 1] == '\\' || pathStr[pathStr.size() - 1] == '/')
		pathStr[pathStr.size() - 1] = '\0';
	paths.push_back(pathStr);
	tmp_path = HSNTree::String::getLeftStr(pathStr, "\\/", false);
	while(tmp_path.size())
	{
		paths.push_back(tmp_path);
		tmp_path = HSNTree::String::getLeftStr(tmp_path, "\\/", false);
	}
	...
	try
	{
		...
		_pFreezeConnection = ::Freeze::createConnection(_pCommunicator, _tsConfig._database._path);
	}
	catch (Freeze::DatabaseException& ex)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "create freeze connection caught(%s: %s)"), ex.ice_name().c_str(), ex.message.c_str());
		return false;
	}
	catch (Ice::Exception& ex)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "create freeze connection caught(%s)"), ex.ice_name().c_str());
		return false;
	}
	
	try
	{
		_pStreamIdx = new HSNTree::StreamIdx(INDEXFILENAME(StreamIdx));
		::std::vector<Freeze::IndexPtr> indexs;
		indexs.push_back(_pStreamIdx);
		ZQ::common::MutexGuard lk(_lockEvictor);

#if ICE_INT_VERSION / 100 >= 303
		_pContextEvictor = ::Freeze::createBackgroundSaveEvictor(_pAdapter, _tsConfig._database._path.c_str(), "SessionContexts", 0, indexs);
#else
		_pContextEvictor = ::Freeze::createEvictor(_pAdapter, _tsConfig._database._path.c_str(), "SessionContexts", 0, indexs);
#endif
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "create freeze evictor caught(%s: %s)"), ex.ice_name().c_str(), ex.message.c_str());
		return false;
	}
	catch (Ice::Exception& ex)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "create freeze evictor caught(%s)"), ex.ice_name().c_str());
		return false;
	}
	...
	while (tItor->hasNext())
	{
		__int64 rn_value = (__int64) _tsConfig._rtspSession._timeout * 1000 + 60000 + (20 * nSessionNum++);
	...
	glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "leave openSafeStore()"));
	return true;
}

void Environment::closeSafeStore()
{
	glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "do closeSafeStore()"));
	if (NULL != _pFreezeConnection)
	{
		try
		{
		...
		{
			glog(ErrorLevel, CLOGFMT(Environment, "close freeze connection caught(%s: %s)"), ex.ice_name().c_str(), ex.message.c_str());
		}
	}
	_pFreezeConnection = NULL;
	glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "leave closeSafeStore()"));
}

bool Environment::connectIceStorm()
{
	glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "do connectEventChannel()"));
	try
	{
		_eventChannel = new TianShanIce::Events::EventChannelImpl(_pAdapter, _tsConfig._iceStorm._endpoint.c_str());
		TianShanIce::Streamer::StreamEventSinkPtr _evtStream = new StreamEvent(*this);
//		TianShanIce::Streamer::PlaylistEventSinkPtr _evtPlaylist = new PlaylistEvent(*this);
...
//		_eventChannel->sink(_evtPlaylist, qos);
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "connectEventChannel(%s) caught(%s: %s)")
			, _tsConfig._iceStorm._endpoint.c_str(), ex.ice_name().c_str(), ex.message.c_str());
		return false;
	}
	catch (Ice::Exception& ex)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "connectEventChannel(%s) caught(%s)")
			, _tsConfig._iceStorm._endpoint.c_str(), ex.ice_name().c_str());
			return false;
	}
	
	glog(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "leave connectEventChannel()"));
	return true;
}
...
bool Environment::getWeiwooSessionPrx(TianShanIce::SRM::SessionPrx& srvrSessPrx, const std::string& srvrSessPrxID)
{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "getWeiwooSessionPrx(%s) caught(%s)")
...
bool Environment::renewWeiwooSession(const TianShanIce::SRM::SessionPrx& srvrSessPrx, const std::string& srvrSessPrxID, const __int64& rn_time)
{
	try
	{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "renewWeiwooSession(%s) caught(%s)"), srvrSessPrxID.c_str(), ex.ice_name().c_str());
		return false;
	}
	return true;
}
...
bool Environment::getPurchasePrx(TianShanIce::Application::PurchasePrx& purchasePrx, const std::string& purchasePrxID)
{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "getPurchasePrx(%s) caught(%s)")
...
bool Environment::getPlaylistPrx(TianShanIce::Streamer::PlaylistPrx& playlistPrx, const std::string& streamPrxID)
{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "getPlaylistPrx(%s) caught(%s)")
...
bool Environment::getStreamState(const TianShanIce::Streamer::PlaylistPrx& playlistPrx, const std::string& streamPrxID, TianShanIce::Streamer::StreamState& strmState, std::string& statDept)
{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "%s[%s:%04d] %s caught by stream(%s).getCurrentState")
			, ex.ice_name().c_str(), ex.category.c_str(), ex.errorCode, ex.message.c_str(), streamPrxID.c_str());
		return false;
	}
	catch(const ::Ice::Exception& ex)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "%s[SsmEnv:0300] caught by stream(%s).getCurrentState"), 
...
bool Environment::getStreamPlayInfo(const TianShanIce::Streamer::PlaylistPrx& playlist, const std::string& streamPrxID, std::string& scale, Ice::Int& iCurrentPos, Ice::Int& iTotalPos)
{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "%s[SsmEnv:0400] caught by stream(%s).getInfo"), 
			ex.ice_name().c_str(), streamPrxID.c_str());
		return false;
	}
	...
	if (!bInfoSucc)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "stream(%s).getInfo() failed[SsmEnv:0401]"), 
			streamPrxID.c_str());
		return false;
	}
...
bool Environment::getPlaylistPlayInfo(const TianShanIce::Streamer::PlaylistPrx& playlist, const std::string& streamPrxID, std::string& scale, Ice::Int& ctrlNum, Ice::Int& offset)
{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "%s[SsmEnv:0500] caught by stream(%s).getInfo"), 
			ex.ice_name().c_str(), streamPrxID.c_str());
		return false;
	}
	...
	if (!bInfoSucc)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "stream(%s).getInfo() failed[SsmEnv:0501]"), 
			streamPrxID.c_str());
		return false;
	}
...
bool Environment::PlayInfo2UtcTime(const TianShanIce ::Application ::PurchasePrx& purchasePrx, const std::string& purchasePrxID, const ::Ice::Int& ctrlNum, const ::Ice::Int& offset, std::string& utcTime)
{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "%s[%s:%04d] %s caught by purchase(%s).getParameters")
			, ex.ice_name().c_str(), ex.category.c_str(), ex.errorCode, ex.message.c_str(), purchasePrxID.c_str());
		return false;
	}
	catch(const ::Ice::Exception& ex)
	{
		glog(ErrorLevel, CLOGFMT(Environment, "%s[SsmEnv:0600] caught by purchase(%s).getParameters"), 
...
{
	glog(DebugLevel, CLOGFMT(Environment, "do openSessionCtx(%s)"), ident.name.c_str());
	try
	{
	...
	{
		glog(ErrorLevel, CLOGFMT(Environment, "%s[SsmEnv:0700] caught in openSessionCtx()"), ex.ice_name().c_str());
		return false;
	}
	glog(InfoLevel, CLOGFMT(Environment, "openSessionCtx(%s) successfully"), ident.name.c_str());
	return true;
}

bool Environment::removeSessionCtx(const Ice::Identity& ident, const std::string& reason)
{	
	glog(DebugLevel, CLOGFMT(Environment, "do removeSessionCtx(%s), reason: %s"), ident.name.c_str(), reason.c_str());
	try
	{
		ZQ::common::MutexGuard lk(_lockEvictor);
	...
	{
		glog(ZQ::common::Log::L_ERROR, CLOGFMT(Environment, "%s[SsmEnv:0800] %s caught in removeSessionCtx()")
			, ex.ice_name().c_str(), ex.message.c_str());
		return false;
	}
	catch (Ice::Exception& ex)
	{
		glog(ZQ::common::Log::L_ERROR, CLOGFMT(Environment, "%s[SsmEnv:0801] caught in removeSessionCtx()"), ex.ice_name().c_str());
		return false;
	}
	glog(InfoLevel, CLOGFMT(Environment, "removeSessionCtx(%s) successfully, reason: %s"), ident.name.c_str(), reason.c_str());
	return true;
}
