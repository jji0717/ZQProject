...
//		case getaeListWithAppUID:	
		...
		MLOG(ZQ::common::Log::L_INFO, CLOGFMT(LAMPlayListQuery3, "[%s] get LAMFacade proxy took %d ms and get playlist from LAMFacade mode successfully[took %d ms]"),
			plinfo.ident.name.c_str(), (int)(lTimeGetProxy - lstart), (int)(ZQTianShan::now() - lTimeGetProxy));
	}
	...
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(LAMPlayListQuery3, "[%s] Leave GetPlayListFromILAMFacade()"), plinfo.ident.name.c_str());
	return retCode;
}
/*
