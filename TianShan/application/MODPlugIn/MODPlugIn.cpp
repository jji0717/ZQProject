...
#define  MODPLUGINLOGFILE "MODPlugIn.log"
...
ZQAPPMOD::AssetLocation* assetLocation = NULL;
ZQAPPMOD::LAMProviderQuery*   lamproviderQuery = NULL;
			      ...
			      const char* configfile,const char* logFolder, const char * modinstanceID, Ice::CommunicatorPtr& ic)
{
...
#endif
	{
		...
		{
			strLogfile = strCurDir + "MODPlugIn_" ;
	...
	if(!OteAuthorization)
	{
		(*PMHOlog)(ZQ::common::Log::L_ERROR, "failed to create OTEAuthorization object");
		return;
	}
	...
	if(!lamplaylistQuery)
	{
		(*PMHOlog)(ZQ::common::Log::L_ERROR, "failed to create LAMPlayListQuery object");
		lamplaylistQuery = NULL;
	}
	lamplaylistQuery3 = new ZQAPPMOD::LAMPlayListQuery3(_iceComm);
	if(!lamplaylistQuery3)
	{
        (*PMHOlog)(ZQ::common::Log::L_ERROR, "failed to create LAMPlayListQuery3 object");
	...
	{
		(*PMHOlog)(ZQ::common::Log::L_ERROR, "failed to create LSMSForMODAuthorization object");
	}

	admpacement = new ZQAPPMOD::ADMPlacement(_iceComm);
	if(!admpacement)
	{
		(*PMHOlog)(ZQ::common::Log::L_ERROR, "failed to create ADMPlacement object");
	}

	lamproviderQuery = new ZQAPPMOD::LAMProviderQuery(_iceComm);

	if(!lamproviderQuery)
	{
		(*PMHOlog)(ZQ::common::Log::L_ERROR, "failed to create LAMProviderQuery object");
	}

	assetLocation = new ZQAPPMOD::AssetLocation();
	if(!assetLocation)
	{
		(*PMHOlog)(ZQ::common::Log::L_ERROR, "failed to create AesstLocation object");
	}
	...
	if(lamproviderQuery)
		mgr.registerProviderQuery(LAM_ProviderQuery_Name, *lamproviderQuery, pCtx);
	if(assetLocation)
		mgr.registerAssetLocation(R1_AssetLocation_Name, *assetLocation, pCtx);
...
void UnInitMHO(void)
{
	...
	{	
		if (mhoMgr) 
		{
			...
			mhoMgr->unregistProviderQuery(LAM_ProviderQuery_Name);
			mhoMgr->unregistProviderQuery(R1_AssetLocation_Name);
			if(OteAuthorization)
			{
			...
			if(lamproviderQuery)
			{
				delete lamproviderQuery;
				lamproviderQuery = NULL;
			}
			if(assetLocation)
			{
