...
// 			pItor->first.c_str(), pItor->second.c_str());
	...
	try
	{
		...
		Ice::Long nMod = 0;
		{
			...
			MLOG(ZQ::common::Log::L_DEBUG, AuthorFmt(OTEAuthorization, "conn[%s] connecting to MoDIceInterface of TVBS for setup-auth: %s"), strtemp, strAuthorParameter.c_str());
		}
...
//	MLOG(ZQ::common::Log::L_INFO, AuthorFmt(OTEAuthorization, "setup authorization connected to MoDIceInterface of TVBS, connId(%s)"), strtemp);
	...
	{
		MLOG(ZQ::common::Log::L_WARNING, AuthorFmt(OTEAuthorization, "setup authorization failed: %s [took %d ms], pass the authorization"), 
			ex.ice_name().c_str(), ZQTianShan::now() - lstart);
		rd.status = "0";
	...
	MLOG(ZQ::common::Log::L_INFO, AuthorFmt( OTEAuthorization, "get OTE proxy took %d ms and setup authorization successfully [took %d ms],author parameter:[%s]"),
...
int OTEAuthorization::OnDestroyPurchase(AuthorInfo& authorInfo, const ::TianShanIce::Properties& prop)
{
	...
	try
	{
		...
		MLOG(ZQ::common::Log::L_INFO, AuthorFmt(OTEAuthorization, "notifying session finish with terminate reason [%s] and teardownReason [%s] and FinalNPT[%s]"), 
			strTerminateReason.c_str(), strTeardownReason.c_str(), strFinalNPT.c_str());
	}
	catch (...)
	{
		MLOG(ZQ::common::Log::L_DEBUG, AuthorFmt(OTEAuthorization,"notifying session finish caught error(%d)"), SYS::getLastErr());
		retCode = INTERNAL;
...
/*		Ice::Long timestamp = ZQTianShan::now();
		...
		Ice::Long nMod = 0;
		{
			...
			nMod = _lLoop++ % activeconsize;
			sprintf(strtemp, "conn_%lld\0", nMod);
			MLOG(ZQ::common::Log::L_DEBUG, AuthorFmt(OTEAuthorization, "conn[%s] connectting to MoDIceInterface of TVBS for session finish notification"), strtemp);
		}
		Ice::ObjectPrx prx = _iceComm->stringToProxy(authorInfo.endpoint);
		Ice::ObjectPrx oteConnectPrx = prx->ice_connectionId(strtemp);
		otePrx = ::com::izq::ote::tianshan::MoDIceInterfacePrx::uncheckedCast(oteConnectPrx);
...
//	MLOG(ZQ::common::Log::L_INFO, AuthorFmt(OTEAuthorization, "session finish notification connected to MoDIceInterface of TVBS, connId(%s)"), strtemp);
	...
	MLOG(ZQ::common::Log::L_INFO, AuthorFmt(OTEAuthorization, "get OTE proxy took %d ms and session finish notification has been issued [took %d ms]"), 
