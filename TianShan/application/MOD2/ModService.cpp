...
ZQ::common::Config::Loader<MODCFGGROUP> gNewCfgGroup("MovieOnDemand.xml");
...
ModEnv::ModEnv() : _iceComm(NULL), _iceAdap(NULL), _pIceLog(NULL), _logForIce(NULL), _frzConn(NULL), _pWatchDog(NULL), 
				_modApp(NULL), _objFactory(NULL), _stream2Purchase(NULL), _evctPurchase(NULL), _evctPurItem(NULL),_serviceGroupPump(NULL),
				_mrtAdapter(NULL), _replicaUpdater(NULL)
{
}
...
bool ModEnv::initIceRuntime()
{
	...
	try
	{
		...
		for (; itor != gNewCfg.icePropMap.end(); itor ++)
		{
			...
			if(itor->first == "Ice.Override.Timeout")
				_IceTimeout = atoi((char*)(itor->second).c_str()) + 1500;
		}
		}
		...
		try
		{
			std::string icefilename =  gNewCfg.iceLogPath+ FNSEPS"MODSvc_Ice.log";
...
bool ModEnv::doInit(const char * logfolder, std::string InstanceID)
{
	...
	if(gNewCfg.mrtStreamServiceCfg.enable)
	{
		glog(ZQ::common::Log::L_DEBUG, CLOGFMT(ModEnv, "initialize StreamService"));

		try
		{
			glog(ZQ::common::Log::L_INFO, CLOGFMT(ModEnv, "create StreamService Adapter at endpoint[%s]"), gNewCfg.mrtStreamServiceCfg.bindEndPoint.c_str());

			_mrtAdapter=_iceComm->createObjectAdapterWithEndpoints( "MRTSS" , gNewCfg.mrtStreamServiceCfg.bindEndPoint);
		}
		catch ( const Ice::Exception& ex)
		{
...
#endif
		dbPath += FNSEPS;
		_serviceInstance = new MODMRTProxy(*this, _mrtAdapter,dbPath, gNewCfg.mrtStreamServiceCfg.nodeId, gNewCfg.mrtStreamServiceCfg.spigotIds, gNewCfg.mrtStreamServiceCfg.streamToMRTEndpointInfos, glog, gNewCfg.mrtStreamServiceCfg.targetTime);
		_serviceInstance->listenerEndpoint = gNewCfg.mrtStreamServiceCfg.replicaSubscriberEndpoint;
		_serviceInstance->updateInterval = 60*1000;
		_serviceInstance->setStreamPenalty(gNewCfg.mrtStreamServiceCfg.penalty, gNewCfg.mrtStreamServiceCfg.maxPenalty);
		_service= _serviceInstance ;

		if( !gNewCfg.mrtStreamServiceCfg.eventChannel.empty() ) {
			_serviceInstance->connectToEventChannel(gNewCfg.mrtStreamServiceCfg.eventChannel);
		}
		try
		{
			_mrtAdapter->add(_service,_iceComm->stringToIdentity("StreamService"));	
			_mrtAdapter->activate();

			if( !gNewCfg.mrtStreamServiceCfg.replicaSubscriberEndpoint.empty() )
			{
			...
			glog(ZQ::common::Log::L_INFO, CLOGFMT(ModEnv, "StreamService interface added to adatper"));
		}
		catch(const Ice::Exception& ex)
		{
			glog(ZQ::common::Log::L_ERROR, CLOGFMT(ModEnv, "caught ice exception: %s"), ex.ice_name().c_str());
			return false;
		}
		catch(...)
		{
			glog(ZQ::common::Log::L_ERROR, CLOGFMT(ModEnv, "caught unknown exception"));
			return false;
		}
	}
	...
	{
		_serviceGroupPump = new ServiceGroupPump(gNewCfg.serviceGroupPumpCfg.ip,gNewCfg.serviceGroupPumpCfg.port,gNewCfg.serviceGroupPumpCfg.sgPumpformat,gNewCfg.serviceGroupPumpCfg.timeout,gNewCfg.serviceGroupPumpCfg.interval);
		if(_serviceGroupPump == NULL)
		{
			glog(ZQ::common::Log::L_ERROR, CLOGFMT(ModEnv, "malloc ServiceGroupPump application object failed"));
			return false;
		}
		_serviceGroupPump->start();
	}
	return true;
}
...
void ModEnv::doUninit()
{
	...
	if (_serviceGroupPump != NULL)
	{
		delete _serviceGroupPump;
		_serviceGroupPump = NULL;
	}
															...
															const ::Ice::Current& c)
{
	...
	TianShanIce::SRM::ResourceMap resources = weiwooSessPrx->getReources();
	...
	{
		purchaseId = resources[TianShanIce::SRM::rtServiceGroup].resourceData["deliveryId"].strs[0];
	}
	else 
		purchaseId = IceUtil::generateUUID();
	...
	pPurchase->ident.name = purchaseId;
...
// 		}
		...
		if(gNewCfg.urlpattern.size() < 1)
		{
			glog(ZQ::common::Log::L_ERROR, CLOGFMT(ModService, 
				"MOD services configration NO URLPattern, or all URLPattern disabled"));
			return S_FALSE;
		}
...
//		showModCfg(itor->second);
		...
		_env._configFilePath = std::string(m_wsConfigFolder) + "MovieOnDemand.xml";
#else
		std::string logDir = _logDir;
		_env._configFilePath = _configDir + "MovieOnDemand.xml";
#endif
		pauseMax = gNewCfg.mrtStreamServiceCfg.pauseMaxCfg;
		pauseMin = gNewCfg.mrtStreamServiceCfg.pauseMinCfg;
