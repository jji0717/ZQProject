...
// 			pItor->first.c_str(), pItor->second.c_str());
	...
	try
	{
		...
		Ice::Long nMod = 0;
		{
			...
			_lLoop++;
			MLOG(ZQ::common::Log::L_DEBUG, AuthorFmt(LSMSForMODAuthorization, "setup authorization connectting to LSMSForMoD of TVBS, connId(%lld), %s"), nMod, strAuthorParameter.c_str());
		}
...
//	MLOG(ZQ::common::Log::L_INFO,AuthorFmt(LSMSForMODAuthorization, "setup authorization connected to LSMSForMoD of TVBS, connId(%s)"),strtemp);
	...
	{
		MLOG(ZQ::common::Log::L_DEBUG, AuthorFmt(LSMSForMODAuthorization, "setup authorization failed: %s [took %d ms], pass the authorization"), 
			ex.ice_name().c_str(), ZQTianShan::now() - lstart);
		rd.status = "0";
	...
	MLOG(ZQ::common::Log::L_INFO, AuthorFmt( LSMSForMODAuthorization, "setup authorization successfully [took %lld ms], %s"), ZQTianShan::now() - lstart, strAuthorParameter.c_str());
...
int LSMSForMODAuthorization::OnDestroyPurchase(AuthorInfo& authorInfo, const ::TianShanIce::Properties& prop)
{
	...
	try
	{
		...
		MLOG(ZQ::common::Log::L_INFO, AuthorFmt(LSMSForMODAuthorization, "notifying session finish with terminate reason [%s] and teardownReason [%s]"), 
			strTerminateReason.c_str(), strTeardownReason.c_str());
	}
	catch (...)
	{
		MLOG(ZQ::common::Log::L_DEBUG,AuthorFmt(LSMSForMODAuthorization,"notifying session finish caught error(%d) [took %d ms]"), SYS::getLastErr(), ZQTianShan::now() - lstart);
		retCode = INTERNAL;
...
/*		Ice::Long timestamp = ZQTianShan::now();
		...
		Ice::Long nMod = 0;
		{
			...
			nMod = _lLoop % activeconsize;
			_lLoop++;
			MLOG(ZQ::common::Log::L_DEBUG, AuthorFmt(LSMSForMODAuthorization, "session finish notification connectting to LSMSForMoD of TVBS, connId(%lld)"), nMod);
		}
...
#endif
		MLOG(ZQ::common::Log::L_INFO, AuthorFmt(LSMSForMODAuthorization, "session finish notification has been issued [took %d ms]"), ZQTianShan::now() - lstart);
	}
	catch (const Ice::Exception& ex)
	{
