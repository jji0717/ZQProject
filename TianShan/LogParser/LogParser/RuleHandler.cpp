...
RuleHandler::RuleHandler(ZQ::common::Log& log, const std::string& exp, const EventTemplate& tmpl, std::vector<std::string> sendTargets, const Literal::Library& lib)
				:_log(log)
				,_targets(sendTargets)
{
	try
	{
		_expr.assign(exp);
	...
	{ // setup of the template
		...
		for(EventTemplate::Properties::const_iterator it = tmpl.params.begin(); it != tmpl.params.end(); ++it)
		{
...
void RuleHandler::handle(const char* msg, int len, const MessageIdentity& mid,LogMessageHandler& MsgHandler)
{
//	_log(ZQ::common::Log::L_WARNING, CLOGFMT(LogMessageHandler, "-----------------msg:%s"), msg);
//	_log(ZQ::common::Log::L_WARNING, CLOGFMT(LogMessageHandler, "-----------------rule:%s"), _expr.begin());
	boost::cmatch result;
	try {
		...
		MsgHandler.SendEvent(_targets,evnt,mid);
...
void RuleHandler::generateEvent(const boost::cmatch& matchResult, EventTemplate& evnt, int64 stamp) const
{
	char timebuf[64];
