...
int LogMonitorRequest::run()
{
    ...
    try {
		...
		{
			if (!_logParserMgr._senderManager.detectWindow(_filepath))
			{
				_idleTime = _logParserMgr._senderManager.resetIdleTime();
				_log(Log::L_ERROR, CLOGFMT(LogMonitorRequest, "window more than the maximum value,idle[%llu]"), _idleTime);
				break;
			}
		...
		{
			_idleTime = (_idleTime < 4000) ? 4000 : _idleTime;
			std::string stampStr = time2utc(mid.stamp);
