...
#define PLUGIN_IDENT ZQ_INTERNAL_FILE_NAME "/" __N2S__(ZQ_PRODUCT_VER_MAJOR) "." __N2S__(ZQ_PRODUCT_VER_MINOR)
...
namespace LiveChannel
{
	...
	std::string RequestHandler::_dummyLastSessId="";
		...
		, _usedTime(0)
	{
		...
		if (true == _env._serverHeader.empty())
			_env._serverHeader = getRequestHeader(HeaderServer) + "; " + PLUGIN_IDENT;
	}
	...
	void RequestHandler::setResponseHeader(const char* pHeaderStr, const char* strValue)
	{
		if (NULL == strValue)
			return;
		std::string tmpStr = String::nLeftStr(strValue, _liveChannelConfig._response._maxFieldLen);
		::std::string::size_type getLen = tmpStr.size();
		if (_liveChannelConfig._response._maxFieldLen == (int) getLen)
		{
			tmpStr[getLen - 3] = '.';
			tmpStr[getLen - 2] = '.';
			tmpStr[getLen - 1] = '.';
...
*/
	...
	{
		int64 rn_value = (int64) _liveChannelConfig._rtspSession._timeout * 1000;
		try
		{
			_bcastPublishPointPrx->renew(rn_value);
		}
		...
		_env._pWatchDog->watch(_session, rn_value);
		return true;
	}
	...
	bool ContentHandler::getBcastPublishPointPrx()
	{
		if (NULL != _bcastPublishPointPrx)
			return true;
		try
		{
			_bcastPublishPointPrx = TianShanIce::Application::Broadcast::BcastPublishPointPrx::checkedCast(_env._pCommunicator->stringToProxy(_cltSessCtx.bcastPublishPointPrxID));
		}
		catch( const Ice::ObjectNotExistException& ex)
		{
			//clearSessionResource();
			snprintf(_szBuf, sizeof(_szBuf) - 1, "%s[ContentHandler:0500] caught in getBcastPublishPrx(%s)", 
				ex.ice_name().c_str(), _cltSessCtx.bcastPublishPointPrxID.c_str());
			SSMLOG(ErrorLevel, HandlerFmt(ContentHandler, "%s"), _szBuf);
			_statusCode = 404;
			return false;
		}
		...
		{
			snprintf(_szBuf, sizeof(_szBuf) - 1, "%s[ContentHandler:0600] caught in getBcastPublishPrx(%s)", 
				ex.ice_name().c_str(), _cltSessCtx.bcastPublishPointPrxID.c_str());
			SSMLOG(ErrorLevel, HandlerFmt(ContentHandler, "%s"), _szBuf);
			_statusCode = 503;
			return false;
...
//			snprintf(_szBuf, sizeof(_szBuf) - 1, "caught[%s] while execute command on stream[%s]",ex.message.c_str(), _cltSessCtx.streamPrxID.c_str());
	...
	SessionContextImplPtr ContentHandler::convertSessionData(SessionData& sessionData)
	{
		...
		pSessionContext->bcastPublishPointPrxID = sessionData.bcastPublishPointPrxID;
		...
		pSessionContext->props			=	sessionData.props;
