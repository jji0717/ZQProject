...
//	
	{
		...
		try
		{
			_pAppPrx = ::ChannelOnDemand::ChannelOnDemandAppPrx::checkedCast(_ic->stringToProxy(::std::string(CHANNEL_ONDEMAND_APPNAME ":") + endpoint));
		}
		catch(::Ice::ProxyParseException e)
		{
			errmsg = "There was an error while parsing the stringified proxy \"ChannelPublisher:" + endpoint + "\"!";
	...
	try
	{
		_publisherPrx = ::ChannelOnDemand::ChannelPublisherExPrx::checkedCast(_ic->stringToProxy(::std::string(SERVICE_NAME_ChannelPublisherEx ":") + endpoint));
	}
	catch(::Ice::ProxyParseException e)
	{
		errmsg = "There was an error while parsing the stringified proxy \"ChannelPublisher:" + endpoint + "\"!";
...
::std::string TestClient::publish(CTreeCtrl& treectrl, HTREEITEM hRoot, ::std::string name, ::std::string desc)
{
	...
	::ChannelOnDemand::ChannelPublishPointPrx channelPrx = _publisherPrx->publish(name, 0, desc);
	if(!channelPrx)
	{
...
::std::string TestClient::open(CTreeCtrl& treectrl, HTREEITEM hChannel, ::std::string& name, ::std::string& desc)
{
	...
	::ChannelOnDemand::ChannelPublishPointPrx channelPrx = _publisherPrx->open((LPCSTR)chName);
	...
	for(size_t i = 0; i<items.size(); i++)
	{
		...
		::ChannelOnDemand::ChannelItem tmpItem = channelPrx->findItem(contentName);
...
::std::string TestClient::destroy(CTreeCtrl& treectrl, HTREEITEM hChannel)
{
	...
	::ChannelOnDemand::ChannelPublishPointPrx channelPrx = _publisherPrx->open((LPCSTR)name);
								   ...
								   BOOL& playable, BOOL& forceNormalSpeed, long& inTimeOffset, long& outTimeOffset, BOOL& spliceIn, BOOL& spliceOut)
{
	...
	try
	{
		::ChannelOnDemand::ChannelItem item = channelPrx->findItem(contentName);
