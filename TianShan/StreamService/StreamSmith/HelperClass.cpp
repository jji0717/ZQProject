...
bool	HelperClass::getRegistryData( const char* path ,  const char* key , BYTE* data ,DWORD dataSize , DWORD type)
{
	...
	if(ERROR_SUCCESS != RegOpenKeyExA(HKEY_LOCAL_MACHINE,path,0,KEY_QUERY_VALUE,&hResultKey))
	{
		return false;
	}
	DWORD dwType=0;	
	if(ERROR_SUCCESS != RegQueryValueExA(hResultKey,key,0,&dwType,data,&dataSize))
	{
...
std::string HelperClass::dumpBinary( const std::string& strBin )
{
	...
	int iBinSize = static_cast<int>(strBin.length());
	int iPos = 0;
