	...
	if( playlistState == TianShanIce::Streamer::stsPause || playlistState == TianShanIce::Streamer::stsStreaming )
	{
			...
			(newSpeed > 1.0001f) ) //
		{
			...
			if( predictLeft < getEOTSize() )
			{
				ZQTianShan::_IceThrow<TianShanIce::ServerError>(ENVLOG ,"StreamService",0, PLFMT(canChangeScaleInNormalPlaylistMode,
					"item[%s] userCtrlNum[%d] is last item , EOTSize[%d] current distance [%lld][%lld],"
		...
		if( lTo < env->getConfig().iPreloadTimeInMS )
		{
			ZQTianShan::_IceThrow<TianShanIce::InvalidStateOfArt>(ENVLOG,LOGCATAGORY,0,
				PLFMT(canChangeScaleInNormalPlaylistMode,"can't change scale because current distance is [%lld]"),
				lTo	);
		}
		return true;
	}
...
bool SsStreamImpl::pause(const ::Ice::Current& c )
{
	...
	ENVLOG(ZQ::common::Log::L_INFO,PLFMT(pause,"leave pause"));
	return true;
}
...
{
	ENVLOG(ZQ::common::Log::L_DEBUG,
		PLFMT(seekStream,"enter seek steram with offset[%lld] startPos[%d]"),
		offset,startPos);
	TianShanIce::StrValues expectProps;
	...
	ENVLOG(ZQ::common::Log::L_INFO,PLFMT(seekStream,"leave Seek stream with return offset[%lld]"),newOffset);
	return newOffset;
}
...
{
	ENVLOG(ZQ::common::Log::L_DEBUG,
		PLFMT(seekToPosition,"enter seekToPosition with userCtrlNum[%d] offset[%lld] startPos[%d]"),
...
#pragma message(__MSGLOC__"TODO: do not forget the critical start item")
		...
		if( ret != ERR_RETURN_SUCCESS )
		{
			...
			ENVLOG(ZQ::common::Log::L_WARNING,
				PLFMT(getSessTimeLeft,"can't get stream sess[%s]'s attr , return 0 as durationLeft"),
				streamId.c_str() );
			return 0;
		}
		ENVLOG(ZQ::common::Log::L_INFO,PLFMT(getSessTimeLeft,"get item [%s] [%d]attribute {%s}"),
			iterCurrentItem()->setupInfo.contentName.c_str() ,
			iterCurrentItem()->userCtrlNum,
		...
		ENVLOG(ZQ::common::Log::L_INFO,PLFMT(getSessTimeLeft,"get duration left[%lld] according to item realPlayDuration[%lld] ,item[%s][%d] inTimeOffset[%lld] outTimeOffset[%lld]"),
			tLeft ,
...
{
	if( LIB_SUPPORT_NORMAL_STREAM == env->getConfig().iSupportPlaylist || LIB_SUPPORT_AUTO_PLAYLIST == env->getConfig().iSupportPlaylist  )
	{
		return true;//do not prime next item 
	}

	ENVLOG(ZQ::common::Log::L_DEBUG,PLFMT(primeNextItem,"enter primeNextItem"));
	if( !iterValid(iterNextItem()) )
	{
		updateTimer();
	...
	if( env->getConfig().iPreloadTimeInMS < 0 )
	{//load next item immediately
		//load next item
	}
	else if( env->getConfig().iPreloadTimeInMS == 0 )
	{
		return true;
	}
	else
...
//		bool bRet = false;
		if (iterValid( iterNextItem() ) && !isItemStreaming(iterNextItem()) )
		{
			...
			{
				std::string streamId;
				iter itTo = doLoadItem( iterNextItem() , 0 , 0.0f , paras, streamId );				
											 ...
											 std::string& streamId )
{
	...
	if( isItemStreaming(iterCurrentItem()))
	{
		...
		if ( ((scale - 0.00f) > 1.01f) &&
			( itTo->setupInfo.flags & TianShanIce::Streamer::PLISFlagNoFF ) )
		{
			ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>(ENVLOG,LOGCATAGORY,0,
		...
		if( ((scale - 0.00f) < -0.001f) &&
			( itTo->setupInfo.flags & TianShanIce::Streamer::PLISFlagNoRew ))
		{
			ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>(ENVLOG,LOGCATAGORY,0,
	...
	{	
		ENVLOG(ZQ::common::Log::L_DEBUG,
			PLFMT(doLoadItem,"load item [%s][%d] with timeoffset[%lld] sclae[%f]"),
	...
	else
	{
		...
		ENVLOG(ZQ::common::Log::L_INFO,PLFMT(doLoadItem,"load item [%s][%d] and sessionId[%s]"),
			itTo->setupInfo.contentName.c_str() ,
			itTo->userCtrlNum,
			streamId.c_str());
	}
...
SsStreamImpl::iter	SsStreamImpl::doChangeScale( iter itTo , Ice::Float scale , StreamParams& paras )
{
	...
	{		
		ENVLOG(ZQ::common::Log::L_DEBUG,
			PLFMT(doLoadItem,"change scale with item [%s][%d] with sclae[%f]"),
			itTo->setupInfo.contentName.c_str(),
			itTo->userCtrlNum,
			scale );
													 ...
													 Ice::Float scale , StreamParams& paras )
{
	...
	std::string streamPort = getStreamPort();
	ENVLOG(ZQ::common::Log::L_DEBUG,
		PLFMT(doLoadItem,"reposition item [%s][%d] with timeoffset[%lld] sclae[%f]"),
		itTo->setupInfo.contentName.c_str(),
		itTo->userCtrlNum,
		timeoffset,scale );
	...
	else
	{
		ENVLOG(ZQ::common::Log::L_INFO,
			PLFMT(doRepositionItem,"reposition item[%s][%d] to timeoffset[%lld] scale[%f]"),
			itTo->setupInfo.contentName.c_str(),
			itTo->userCtrlNum,
			paras.timeoffset,
			paras.scale);
									...
									TianShanIce::Streamer::StreamInfo& info )
{
	...
	ENVLOG(ZQ::common::Log::L_DEBUG,PLFMT(doPlaySession,"enter with from[%d] timeoffset[%lld] scale[%f] expectMask[%x] status[%s]"),
...
// 		setNextItem(  isReverseStreaming() ? iterCurrentItem() - 1 : iterCurrentItem() + 1 );
	...
	ENVLOG(ZQ::common::Log::L_INFO, PLFMT(SsStreamCommitRequest,"insert item, cost[%lld/0]"),
		sw.stop());
	return userCtrlNum;
}
			...
			default:
				{
					...
					ENVLOG(ZQ::common::Log::L_DEBUG,PLFMT(getInfo,"got timeoffset[%lld] scale[%f] from portal layer"),
					...
					ossScale<<paras.scale;
					ZQTianShan::Util::updateValueMapData( outValue , "scale" , ossScale.str() );
					ENVLOG(ZQ::common::Log::L_DEBUG,PLFMT(getInfo,"return timeoffset[%lld] scale[%f] to client"), itemNpt , paras.scale );
			...
			default:
				{
					...
					ENVLOG(ZQ::common::Log::L_DEBUG,PLFMT(getInfo,"SsServiceImpl::doGetStreamAttr() got timeoffset[%lld] scale[%f] from portal layer"),
						paras.timeoffset , paras.scale);
					Ice::Short from = 1;
					Ice::Long  timeOffset = paras.timeoffset;					
					...
					ossScale<<paras.scale;
					ZQTianShan::Util::updateValueMapData( outValue , "scale" , ossScale.str() );
					ENVLOG(ZQ::common::Log::L_DEBUG,PLFMT(getInfo,"SsServiceImpl::doGetStreamAttr() return timeoffset[%lld] scale[%f] to client"),
						paras.timeoffset , paras.scale);
	...
	case LIB_SUPPORT_NORMAL_PLAYLIST:
		{
			...
			{
				ENVLOG(ZQ::common::Log::L_INFO,PLFMT(onSessionExpired , "session[%s] is not current running session item [%s]"),
												sessionId.c_str() ,
												iterCurrentItem()->sessionId.c_str() );
				return;
			}	
...
void SsStreamImpl::onTimerDPC( )
{
	...
	Ice::Long targetTime = 0;
	ZQTianShan::Util::getValueMapDataWithDefault(attributes,"TARGETTIME",0,targetTime);
	IceUtil::Time now = IceUtil::Time::now();
	Ice::Long diff = now.toMilliSeconds() - targetTime;
	ENVLOG(ZQ::common::Log::L_INFO,
		PLFMT(onTimerDPC,"target time [%lld] current[%lld] diff[%lld]"),
		targetTime,	now.toMilliSeconds(),diff );
#endif
	...
	case TianShanIce::Streamer::stsStreaming:
		{
			...
			if( iterValid( iterCurrentItem()) && (iterCurrentItem() == iterLastItem()) )
			{
				if( bEnableEOT && (speed > 1.01f) && (!isReverseStreaming()) && ( getEOTSize() > 0) )
				{
					...
					ENVLOG(ZQ::common::Log::L_INFO,PLFMT(getSessTimeLeft,"adjust duration left to [%lld] because EOTSize [%d]"),
						tLeft,getEOTSize() );
				}
			}
			else
	...
	case TianShanIce::Streamer::TIMERDPC:
		{
			...
			ENVLOG(ZQ::common::Log::L_INFO,PLFMT(setTimer,"update timer [%lld] [%lld]"), t , expired.toMilliSeconds() );
		}
		break;
	default:
		{
