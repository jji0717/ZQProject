...
void ContentStoreImpl::initializePortal(ContentStoreImpl& store)
{
	...
	fileSystemEventSinker = new vstrmFileSystemSink(store,pCtx);
...
namespace ContentStore
{
vstrmFileSystemSink::vstrmFileSystemSink( ZQTianShan::ContentStore::ContentStoreImpl& st ,  PortalCtx* ctx  )
	:store(st),
	_portCtx(ctx)
{
	_bQuit = false;
		...
		if( pOldFile && pNewFile  )
		{
			...
			case VSTRM_FILE_NEW:				
				{
					...
					else
					{
						envlog(ZQ::common::Log::L_INFO,CLOGFMT(vstrmFileSystemSink,
							"[FILEEVENT] File [%s] is ignored with File NEW event"),
							pNewFile);
					}
				}
				break;
			...
			case VSTRM_FILE_CREATED:
				{							
					...
					{
						envlog(ZQ::common::Log::L_INFO,CLOGFMT(vstrmFileSystemSink,
							"[FILEEVENT] File [%s] is ignored with File CREATED event"),
							pNewFile);
					}
				}
				break;
