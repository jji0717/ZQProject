...
//
#if defined ZQ_OS_MSWIN
#define	REQFMT(x,y) 	CLOGFMT(x, "SsStreamRequest[%s] "##y),streamImpl->ident.name.c_str()
...
void SsStreamRequest::execute( )
{
	...
	if( pendingSize >= 1 )
	{
		...
		if( pendingSize > maxPendingSize )
		{
			...
			delete this;			
			(*logger)(ZQ::common::Log::L_ERROR, CLOGFMT(SsStreamRequest,"too many pendingRequest[%d], maxPendingSize[%d] poolSize[%d], active threads[%d]"),
