...
void SsStreamImpl::fillEventData( EventData& data , Ice::Int seq , Ice::Int type )
{
	...
	data.eventSeq				=	seq;
	data.eventType				=	type;
	ZQTianShan::Util::updatePropertyData(data.eventProp, "ServerSession", getContextProperty(SYS_PROP(ServerSession)));
}
...
{//because every time this method invoked , SsStreamImpl must be locked 
	...
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_ITEMSTEP_EVENT_CURFLAG, iterValid(currentIter) ? currentIter->setupInfo.flags : 0 );
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_ITEMSTEP_EVENT_PREVFLAG, iterValid(itFrom) ? itFrom->setupInfo.flags : 0 );
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_ITEMSTEP_EVENT_CURPLAYTIME, iterValid(currentIter) ? currentIter->itemRealDuration : 0 );
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_ITEMSTEP_EVENT_PREVPLAYTIME, iterValid(itFrom) ? itFrom->itemRealDuration : 0 );
	...
	oss.str("");oss.clear();
	lTotal = calculateVideoDuration(iterFirstItem(), iterLastItem());
	oss<<lTotal;
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_STREAM_VIDEOTOTALDURATION , oss.str());
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_STREAM_PREV_PLYPOSPRIMARY , calculateVideoDuration(iterFirstItem(), currentIter));
...
#pragma message(__MSGLOC__"TODO: get the sender and send out the event")
	...
	{		
		...
		std::ostringstream oss;oss<<lTotal;
		ZQTianShan::Util::updateValueMapData( data.eventData ,EVENT_STREAM_COMPLETE_TIMEOFFSET , oss.str() );
		oss.str("");oss.clear();oss<< getCurrentPlaylistNpt();
		ZQTianShan::Util::updateValueMapData( data.eventData, EVENT_STREAM_NPT_POS, oss.str());
		ZQTianShan::Util::updateValueMapData( data.eventData, EVENT_SPEEDCHANGE_CUR_SPEED, speed);
		ZQTianShan::Util::updateValueMapData( data.eventData, EVENT_ITEMSTEP_ITEM_CUR_TIMEOFFSET, getCurrentItemNpt());
		if (iterValid(iterCurrentItem()))
		{
			ZQTianShan::Util::updateValueMapData( data.eventData,"ItemIndexOnEndOfStream",iterCurrentItem()->userCtrlNum);
		}
...
void SsStreamImpl::fireScaleChangedMsg( const Ice::Float& scaleOld , const Ice::Float& scaleNew ,const TianShanIce::Properties& props  )
{
	...
	unsigned long now = ZQ::common::TimeUtil::now();
	std::string perRequested = (now < (stampSpeedChangeReq + env->getConfig().iPerRequestedInterval)) ? "1" : "0";
	ENVLOG(ZQ::common::Log::L_INFO,	PLFMT(fireScaleChangedMsg,"firing ScaleChanged from [%f]to [%f]: %s, perrequesest[%s]"),
		scaleOld , scaleNew , ZQTianShan::Util::dumpStringMap(props).c_str(),perRequested.c_str());

	Ice::Int	itemScaleChangedSeq = 0 ;
	ZQTianShan::Util::getPropertyDataWithDefault( property , EVENT_SPEEDCHANGE_EVENT_SEQ , 0 , itemScaleChangedSeq );
	++itemScaleChangedSeq;
	...
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_SPEEDCHANGE_EVENT_PREREQUESTED , perRequested );
	lTotal = calculateVideoDuration(iterFirstItem(), iterLastItem());
	oss.str(""); oss.clear(); oss<<lTotal;
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_STREAM_VIDEOTOTALDURATION,oss.str());
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_STREAM_PREV_PLYPOSPRIMARY,calculateVideoDuration(iterFirstItem(), currentIter));
	serviceImpl.getEventSender().postEvent(data);
								   ...
								   const TianShanIce::Properties& props )
{
	...
	data.eventProp = props;
	unsigned long now = ZQ::common::TimeUtil::now();
	std::string preRequested = ( now < (stampStateChangeReq + env->getConfig().iPerRequestedInterval)) ? "1" : "0";
	...
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_STATECHANGE_EVENT_PREREQUESTED, preRequested);
	Ice::Long lTotal = calculateDuration(iterFirstItem(),iterLastItem());
	std::ostringstream oss;oss<<lTotal;
	ZQTianShan::Util::updateValueMapData(data.eventData , EVENT_STREAM_COMPLETE_TIMEOFFSET , oss.str() );
	oss.clear(); oss.str("");
...
void	SsStreamImpl::fireProgressMsg( constIter it , const Ice::Long& curTimeOffset , const Ice::Long& totalDuration ,const TianShanIce::Properties& props )
{
	...
	ZQTianShan::Util::getPropertyDataWithDefault( property , EVENT_SPEEDCHANGE_EVENT_SEQ , 0 , itemProgressSeq );
	++itemProgressSeq;
