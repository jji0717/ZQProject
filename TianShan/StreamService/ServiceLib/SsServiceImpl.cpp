...
bool SsServiceImpl::reuseStreamPort( const std::string& contextKey )
{
	...
	if (strStreamerId.empty())
	{
		ENVLOG(ZQ::common::Log::L_WARNING, CLOGFMT(SsServiceImpl, "NULL streamer id, destroying the playlist[%s]"),contextKey.c_str());
		try
		{
			Ice::Context ctx;
	...
	if ( itStreamer != mReplicaInfos.end() )
	{
		...
		for ( ; itPort != ports.end() ; itPort ++ )
		{
			...
			ENVLOG(ZQ::common::Log::L_INFO, CLOGFMT(SsServiceImpl, "failed to reuse stream port[%s], destroying the playlist[%s]"), strStreamPort.c_str(), contextKey.c_str());
			try
			{
				Ice::Context ctx;
		...
		ENVLOG(ZQ::common::Log::L_INFO, CLOGFMT(SsServiceImpl, "failed to  find the reused stream port[%s], destroying the playlist[%s]"), strStreamPort.c_str(), contextKey.c_str());
		try
		{
			Ice::Context ctx;
...
{	
	...
	std::string streamId;
	ZQTianShan::Util::getPropertyDataWithDefault(ic.ctx, "hintSessionID", svcCreateSessionId(), streamId);

	if (!ticket)
	{
		ENVLOG(ZQ::common::Log::L_DEBUG, SSFMT( createStream, "creating stream by dummyResource" ));
		TianShanIce::SRM::ResourceMap dummyResource;
		dummyResource.clear();
	...
	ENVLOG(ZQ::common::Log::L_INFO, SSFMT( createStream, "creating stream by ticket[%s]" ), ticketId.c_str() );
	...
	if( streamLink)
	{
		...
		ENVLOG(ZQ::common::Log::L_DEBUG, SSFMT(createStream, "creating stream on streamer[%s] specifiled by link"), netId.c_str());
	}
	else
	{
		...
		{
			ENVLOG(ZQ::common::Log::L_ERROR, SSFMT(createStream, "failed to find resource Streamer[NetworkId] in ticket"));
			ex.ice_throw();
		}
		...
		ENVLOG(ZQ::common::Log::L_DEBUG, SSFMT(createStream, "creating stream on streamer[%s] specifiled by resource"), netId.c_str() );
	}
	...
	ENVLOG(ZQ::common::Log::L_INFO, SSFMT(createStream,"stream created on streamer[%s], took %dmsec"), netId.c_str(), (int)sw.stop());

	return newstream;
}

TianShanIce::Streamer::StreamPrx SsServiceImpl::createStreamByResource( const ::TianShanIce::SRM::ResourceMap& resource, 
											...
											const ::Ice::Current& ic) 
{
	...
	ENVLOG(ZQ::common::Log::L_DEBUG, SSFMT( createStream, "createStreamByResource() res::Streamer[%s]"), strStreamerNetId.c_str());	

	if( !strStreamerNetId.empty())
											  ...
											  TianShanIce::Properties& streamingRes )
{
	...
	if( !bGetStreamPort)
	{
		ZQTianShan::_IceThrow<TianShanIce::InvalidStateOfArt>(ENVLOG, LOGCATAGORY, 0, CLOGFMT(SsServiceImpl,"can't find a suitable stream port for streamer[%s]"),
			streamerNetId.c_str() );
	}
	...
	ENVLOG(ZQ::common::Log::L_INFO,	CLOGFMT(SsServiceImpl, "allocateStreamingResource() associated streamport[%s] for streamer[%s]"), streamPort.c_str(), streamerNetId.c_str() );
	return true;
}
...
void SsServiceImpl::releaseStreamingResource( const TianShanIce::Properties& streamingRes )
{
	...
	if( streamerId.empty() || streamPort.empty() )
	{
		ENVLOG(ZQ::common::Log::L_WARNING, CLOGFMT(SsServiceImpl,"releaseStreamingResource() empty streamer id [%s] or stream port[%s]"),
			streamerId.c_str(), streamPort.c_str() );
		return;
	}
...
TianShanIce::Streamer::StreamPrx SsServiceImpl::doCreateStream(	const std::string& streamId, 
								const TianShanIce::SRM::ResourceMap& srmResources,
								...
								const TianShanIce::Properties& props )
{
	...
	try
	{
		...
		std::ostringstream ossProps;		
		std::map<std::string, std::string>::const_iterator itProp = props.begin();
		for(; itProp != props.end(); itProp++)
		{
			ossProps<< "[" << itProp->first << " : " << itProp->second << "]  ";
		}
		ENVLOG(ZQ::common::Log::L_INFO,SSFMT(doCreateStream, "properties --> %s"), ossProps.str().c_str());
	...
	ZQTianShan::Util::getResourceDataWithDefault(srmResources, TianShanIce::SRM::rtServiceGroup, "deliveryId", "", serverSessionId);	
	streamingRes[SYS_PROP(ServerSession)] = serverSessionId;
...
{	
	...
	{
		ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>(ENVLOG,LOGCATAGORY,0,"[SsServiceImpl] failed to find stream[%s]", id.c_str() );
	}
	catch( const Ice::Exception& ex )
	{
		ZQTianShan::_IceThrow<TianShanIce::ServerError>(ENVLOG,LOGCATAGORY,0,"[SsServiceImpl] failed to open stream[%s], caught %s",
									  ...
									  const std::string& value )
{
	...
	ENVLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(SsServiceImpl,"updateIceProperty() %s=%s"), key.c_str(), value.c_str() );	
}
...
#define DEFAULT_CACHE_SIZE (160*1024*1024)
		...
		ENVLOG(ZQ::common::Log::L_INFO, CLOGFMT(SsServiceImpl,"openDatabase() successfully open database at [%s], evictor size[%d]"), dbTopPath.c_str(), iEvictorStreamSize);
		return true;
	}
	catch( const Ice::Exception& ex )
	{
		ENVLOG(ZQ::common::Log::L_ERROR, CLOGFMT(SsServiceImpl,"failed to open db, caught %s"),	ex.ice_name().c_str() );
	}
...
void SsServiceImpl::closeDatabase( )
{
	...
	{
		mStreamEvictor = NULL;
		ENVLOG(ZQ::common::Log::L_INFO, CLOGFMT(SsServiceImpl,"DB closed"));
	}
}
...
bool SsServiceImpl::addServantToDB(TianShanIce::Streamer::SsPlaylistPtr s, const Ice::Identity& id)
{
	if(!mStreamEvictor )
	{
		ENVLOG(ZQ::common::Log::L_ERROR,CLOGFMT(SsServiceImpl,"addServantToDB() failed to add servant to DB due to NULL evictor"));
		return false;
	}
	...
	try
	{
		if( bShowDBOperationTimeCost )
		{
			...
			ENVLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(SsServiceImpl,"addServantToDB() add id[%s/%s] into db, took %dmsec"),
				id.category.c_str(), id.name.c_str(), (int) (opEnd-opStart).toMilliSeconds() );
		}
		else
		{
			mStreamEvictor->add(s, id);
	...
	{
		ENVLOG(ZQ::common::Log::L_ERROR,CLOGFMT(SsServiceImpl,"failed to add [%s/%s] to DB, caught %s"),
...
void SsServiceImpl::removeServantFromDB( const Ice::Identity& id, const TianShanIce::Properties& resource )
{
	...
	if (!mStreamEvictor)
	{
		ENVLOG(ZQ::common::Log::L_ERROR, CLOGFMT(SsServiceImpl,"removeServantFromDB() can't remove servant from DB because evictor is not opened"));
		return;
	}
	...
	try
	{
		...
		if( bShowDBOperationTimeCost )
		{
			...
			IceUtil::Time opEnd = IceUtil::Time::now();
			ENVLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(SsServiceImpl,"removeServantFromDB() id[%s/%s] removed from db, took %dmsec"),
				id.category.c_str(), id.name.c_str(), (int)(opEnd-opStart).toMilliSeconds() );
		}
		else
		{
	...
	{
		ENVLOG(ZQ::common::Log::L_ERROR, CLOGFMT(SsServiceImpl, "removeServantFromDB() failed to remove [%s/%s] from DB, caught %s"),
			id.category.c_str(), id.name.c_str(), ex.ice_name().c_str() );
	}
}
										...
										const ::Ice::Current& ) 
{
	...
	{
		ENVLOG(ZQ::common::Log::L_ERROR,CLOGFMT(queryReplicas_async, "caught %s"),ex.ice_name().c_str() );
	}
	catch(...)
	{
		ENVLOG(ZQ::common::Log::L_ERROR,CLOGFMT(queryReplicas_async, "caught exception"));
	}
}
...
uint32	SsServiceImpl::reportReplica( )
{
...

	{
		...
		try
		{
			TianShanIce::ReplicaSubscriberPrx subscriber = TianShanIce::ReplicaSubscriberPrx::checkedCast(env->getCommunicator()->stringToProxy(subscriberEP) );
			if( !subscriber )
			{
			...
			else
			{
				...
				ENVLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(SsServiceImpl, "replica reported, subscriber[%s] required interval[%d]"), subscriberEP.c_str(), interval*1000);
		...
		{
			ENVLOG(ZQ::common::Log::L_ERROR, CLOGFMT(SsServiceImpl,"failed to report replica to subscriber [%s], caught %s"), subscriberEP.c_str(), ex.message.c_str()	);
		}
		catch( const Ice::Exception& ex )
		{
			ENVLOG(ZQ::common::Log::L_ERROR, CLOGFMT(SsServiceImpl,"failed to report replica to subscriber [%s], caught %s"), subscriberEP.c_str(), ex.ice_name().c_str()	);
		}
		catch(...)
		{
			ENVLOG(ZQ::common::Log::L_ERROR, CLOGFMT(SsServiceImpl,"failed to report replica to subscriber [%s], caught exception"), subscriberEP.c_str() );
		}
	}
...
void SsServiceImpl::unregisterStreamIdByContextKey( const std::string& contextKey )
{
	ZQ::common::MutexGuard gd(mSCMapLocker);
	CONTEXTTOSTREAMMAP::iterator itContext = mContextToStreamMap.find( contextKey );
	if( mContextToStreamMap.end() == itContext)
		return;
...
void SsServiceImpl::innerOnStreamEvent(StreamEvent event, const std::string& streamId, const StreamParams& currentParams, const ::TianShanIce::Properties& uparams)
{
	...
	long long timeEnd = ZQ::common::now();
	ENVLOG(ZQ::common::Log::L_INFO,CLOGFMT(SsServiceImpl,"OnStreamEvent() took %dms to compose and send event[%s] of[%s]"),
		(int)(timeEnd - timeStart), evName, streamId.c_str() );
}
...
protected:
	int run() 
	{
		try {
			...
			ENVLOG(ZQ::common::Log::L_ERROR,CLOGFMT(DispatchEventRequest,"caught exception while dispatching event"));
		}
...
{	
	...
	if( it == mReplicaInfos.end() )
	{
		...
		if( it == mReplicaInfos.end() )
		{
			ENVLOG(ZQ::common::Log::L_ERROR, CLOGFMT(SsServiceImpl,"OnStreamerEvent() failed to find info of streamer[%s]"), streamerId.c_str());
			return ;
		}		
	}
	...
	case sreInService:
		{			
			ENVLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(SsServiceImpl,"OnStreamerEvent() [sreInService] with streamer[%s]"), streamerId.c_str() );
			...
			if( infoEx.replicaState	!= TianShanIce::stInService )
			{
				...
				ENVLOG(ZQ::common::Log::L_INFO, CLOGFMT(SsServiceImpl,"OnStreamerEvent() streamer[%s] becomes InService"), streamerId.c_str() );
			}
	...
	case sreOutOfService:
		{
			ENVLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(SsServiceImpl, "OnStreamerEvent() [sreOutOfService] with streamer[%s]"), streamerId.c_str() );
			SsReplicaInfo& infoEx = it->second;			
			if( infoEx.replicaState	!= TianShanIce::stOutOfService )
			{
				infoEx.stampChanged		=	ZQTianShan::now();
				infoEx.replicaState		=	TianShanIce::stOutOfService;
				ENVLOG(ZQ::common::Log::L_INFO, CLOGFMT(SsServiceImpl,"OnStreamerEvent() streamer[%s] becomes OutService"), streamerId.c_str() );
			}
	...
	default:
		ENVLOG(ZQ::common::Log::L_ERROR,CLOGFMT(SsServiceImpl, "OnStreamerEvent() invalid event from streamer[%s]"), streamerId.c_str());
		return;
	}
