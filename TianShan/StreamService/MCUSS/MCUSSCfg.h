...
namespace ZQTianShan{
	namespace MCUSS{
		...
		struct ST_MCUSS  
		{
			...
			typedef ZQ::common::Config::Holder<ST_MCUSS> MCUSSHolder;

			static void structure(MCUSSHolder &holder);
		...
		public:
			...
			static void structure(Config::Holder<MCUSSCfg> &holder);
			void registerNothing(const std::string&){}
			void readMCUSS( ZQ::common::XMLUtil::XmlNode node , const ZQ::common::Preprocessor* hPP );
			void registerMCUSS(const std::string& full_path );
		public:
			//default in xml 
			...
			typedef ::std::vector<ST_MCUSS::MCUSSHolder> MCUSSList;
			MCUSSList _mcussList;
		};
