...
//operation of socket
//to specify if this socket can be read, return 1 if can, return 0 if no data in socket, return -1 when socket error
int16 bCheckStatus(SOCKET sock)
{
	//set the fd_set and time value
	...
	FD_ZERO(&fd);
	FD_SET(sock, &fd);
	int selectret = select(0, &fd, NULL,NULL, &tv);
