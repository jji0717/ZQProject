...
**        the SeaChange product configuration information in the registry.
**
** Revision History: 2/2/94  created  DH
** ------------------------
**
**   Rev       Date           Who               Description
**  ------ ---------------- ------  -----------------------------------
**  V2.5    11-May-2004      CEL    Removed queue, and removed a lot of code.
...
**                                  today.
**  V1.0+   04-Dec-2000      CEL    Removed coupling between sessions and thread IDs.
**  V1.0RC2 23-Mar-2000      RPD    SPR #995 Expand environment variables in CFG_GET_PARENT_VALUES
**  ItvT2   01-Apr-1999      MRK    Added CFG_GET_MGMT_PORT.
**  ????    25-Jun-1998      TMS    Made generic for all SeaChange products, Unicode.
**                                  Part of ITV work, but usable for all future products.
**  V3.5    19-Dec-1996      CJH    create key if it doesn't exist in INIT and SUBKEY
**  T3.5    19-Sep-1995      CJB    Added CFG_FREE() routine for MFC based MANPKG
**  V3.0    10-Oct-1995      PHB    Removed excessive NT Eventlogging (CfgGetValue failures)
**  V3.0    23-Oct-1995      CJH    Removed TC special case from CFG_INIT
**  V3.3    28-Aug-1995      PHB    Bullet-proofing tasks
**  T1.0    12-jul-1994      DWH    Initial Fieldtest release
*/
...
static LPCTSTR StreamPath       = _T("SOFTWARE\\ZQ\\CDCI\\StreamTopology");
static LPCTSTR MgmtPortsPath    = _T("SOFTWARE\\ZQ\\Management\\CurrentVersion\\Services");
 ...
 * ..\SeaChange\CDCI\CurrentVersion\Services. It returns a handle which must
...
{
    return CFG_INITEx(app_name, num_values, _T("CDCI"));
}
    ...
    )
{
	...
	_stprintf(pathbuff, _T("SOFTWARE\\ZQ\\%s\\CurrentVersion\\Services\\%s"),
             mainkey, app_name);
	//int iLen = lstrlen(pathbuff);
        ...
        if (NULL != pCfgInstance)
        {
			...
			_stprintf(pCfgInstance->mainkey, _T("SOFTWARE\\ZQ\\%s\\CurrentVersion"), mainkey);
...
//    catch( ExcFilter(GetExceptionInformation(), _T("CFG_")) )
	catch(...)
    {
    }
 ...
 * Services key under ..\Seachange\CDCI\CurrentVersion.
 */
CFGSTATUS
    ...
    )
{
	...
	TCHAR           CurVerPath[] = _T("SOFTWARE\\ZQ\\CDCI\\CurrentVersion");
