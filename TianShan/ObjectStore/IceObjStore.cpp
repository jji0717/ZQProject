...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/TianShan/ObjectStore/IceObjStore.cpp $
// 
// 3     6/25/15 11:56a Hui.shao
// 
// 2     6/10/15 1:51p Hui.shao
// 
...
ServantPtr IceObjStore::locate(const ::Ice::Current& current, ::Ice::LocalObjectPtr& cookie)
{
...
void IceObjStore::finished(const ::Ice::Current& current, const ::Ice::ObjectPtr& servant, const ::Ice::LocalObjectPtr& cookie)
{
//	DeactivateController::Guard deactivateGuard(_deactivateController);
	if (_bQuit || !cookie)
		return;
...
ServantPtr IceObjStore::hatchServant(const std::string& classId)
{
	::Ice::ObjectFactoryPtr pFactory = _adapter->getCommunicator()->findObjectFactory(classId);
	if (!pFactory)
		return NULL;
	return pFactory->create(classId);
}
...
::Ice::ObjectPtr IceObjStore::remove(const ::Ice::Identity& ident)
{
	...
	//DeactivateController::Guard deactivateGuard(_deactivateController);
	return ::Ice::ObjectPtr((::Ice::Object *) (ObjectStore::remove(ident.name).get()));
}
