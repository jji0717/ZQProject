...
// Revision History: 
// ---------------------------------------------------------------------------
// $Log: /ZQProjs/TianShan/ObjectStore/ObjectStore.cpp $
// 
// 2     6/23/15 4:42p Hui.shao
// 
...
#define STORELOGFMG(_FMT)   CLOGFMT(ObjectStore, "category[%s] " _FMT), _category.c_str()
...
StoredObject::Ptr ObjectStore::open(const std::string& id, bool bCreateIfNotExist)
{
	...
	if (so)
	{
		...
		so->_cOpened++;
		_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("open() found pre-opened obj[%s], cOpened[%d]"), id.c_str(), so->_cOpened);
		return so;
	}
	...
	{
		buf[vlen] ='\0';
		_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("open() found existing dbo[%s] in DB, vlen[%d]: %s"), dbOid.c_str(), vlen, buf);
		if (!unmarshal(so, std::string((char*)buf, vlen)) || NULL == so->_servant)
		{
			_log(ZQ::common::Log::L_ERROR, STORELOGFMG("open() failed to unmarshal dbo[%s]: %s"), dbOid.c_str(), vlen, buf);
			ZQ::common::_throw< BadRecord > ("%s(%d) failed to unmarshal dbo[%s]", __FILE__, __LINE__, dbOid.c_str());
		}
		...
		so->_cOpened =0;
		_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("open() dbo[%s] indices[%d] populated from DB"), dbOid.c_str(), i);
	}
	...
	if (so)
	{
		ZQ::common::MutexGuard g(so->_mutex);
		++so->_cOpened;
		_log(ZQ::common::Log::L_INFO, STORELOGFMG("open() dbo[%s] create[%c] => created[%c] cOpen[%d]"), dbOid.c_str(), bCreateIfNotExist?'T':'F', bCreated?'T':'F', so->_cOpened);
	}
	else
		_log(ZQ::common::Log::L_INFO, STORELOGFMG("open() failed to open dbo[%s] w/ create[%c]"), dbOid.c_str(), bCreateIfNotExist?'T':'F');
...
void ObjectStore::close(StoredObject::Ptr& so, bool accessedMutably)
{
	...
	_log(ZQ::common::Log::L_INFO, STORELOGFMG("close() obj[%s] closed: dirty[%c] cOpen[%d], dirtySize[%d] openSize[%d]"), oid.c_str(), bFlush?'T':'F', cOpened, _dirtyObjs.size(), _openedObjs.size());
}

void ObjectStore::stop(const ::std::string&)
{ 
	_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("stop()"));
	_bQuit = true; // stop this thread
	flush();
	_log(ZQ::common::Log::L_INFO, STORELOGFMG("stop() executed"));
}
...
size_t ObjectStore::flush()
{
	...
	_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("flush() %d dirty objects to flush"), dirtyToFlush.size());
	...
	for (StoredObject::List::iterator it = dirtyToFlush.begin(); it != dirtyToFlush.end(); it++)
	{
		...
		if (StoredObject::sos_Destroyed == (*it)->_status)
		{
			...
			if (ZQ::common::RedisSink::rdeOK != _redisClient->DEL(dbkey))
			{
				...
				_log(ZQ::common::Log::L_ERROR, STORELOGFMG("flush() failed to del obj[%s], requeued[%c]"), oid.c_str(), BOOL_CHAR(bReQueued));
				continue;
			}
			...
			for (size_t j=0; j< prevValues.size() && j< _indices.size(); j++)
			{
				...
				if (ZQ::common::RedisSink::rdeOK != _redisClient->SREM(idxkey, oid))
					_log(ZQ::common::Log::L_ERROR, STORELOGFMG("flush() failed to remove obj[%s] from idx[%s]"), oid.c_str(), idxkey.c_str());
				else cIdxDel++;
			}

			_log(ZQ::common::Log::L_INFO, STORELOGFMG("flush() obj[%s] deleted, [%d]indicies cleaned"), oid.c_str(), cIdxDel);
			continue;
		}

		// case 2. dirty object to save
		//   step 2.1. save into DB
		_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("flush() saving obj[%s]"), oid.c_str());
		std::string val;
		if (!marshal((*it), val))
		{
			_log(ZQ::common::Log::L_ERROR, STORELOGFMG("flush() failed to mashal obj[%s]"), oid.c_str());
			continue;
		}
		...
		{
			_log(ZQ::common::Log::L_ERROR, STORELOGFMG("flush() failed to save obj[%s]"), oid.c_str());
			continue;
		}
		...
		for (size_t j=0;  j< _indices.size(); j++)
		{
			...
			if (ZQ::common::RedisSink::rdeOK != _redisClient->SADD(ikey, (*it)->_id))
				_log(ZQ::common::Log::L_ERROR, STORELOGFMG("flush() failed to add new obj[%s] from idx[%s]"), (*it)->_id.c_str(), ikey.c_str());
			...
			if (ZQ::common::RedisSink::rdeOK != _redisClient->SREM(ikey, oid))
				_log(ZQ::common::Log::L_ERROR, STORELOGFMG("flush() failed to remove obj[%s] from idx[%s]"), oid.c_str(), ikey.c_str());

			cIdxChanged++;
		}

		_log(ZQ::common::Log::L_INFO, STORELOGFMG("flush() obj[%s] saved, [%d/%d]indicies updated"), oid.c_str(), cIdxChanged, cIdxUpdated);
	}

	_log( ((cSaved+ cDel) == cExpected) ? ZQ::common::Log::L_INFO : ZQ::common::Log::L_WARNING, STORELOGFMG("flush() flushed (%d +%d)/%d objects, took %dmsec: %s; %s"), cSaved, cDel, cExpected, (int)(ZQ::common::now() - stampStart), savedlist.c_str(), dellist.c_str());
	return cSaved+ cDel;
}
		...
		for (StringList::iterator itV = dirtyValues.begin(); itV < dirtyValues.end(); itV++)
		{
			...
			for (size_t v=0; v <ids.size(); v++)
			{
				...
				if (ZQ::common::RedisSink::rdeOK != _redisClient->SREM(ikey, ids[v]))
					_log(ZQ::common::Log::L_WARNING, STORELOGFMG("auditIdx() failed to clean orphan[%s] from idx[%s]"), ids[v].c_str(), ikey.c_str());
				else
				{
					cCleaned++;
					_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("auditIdx() cleaned orphan[%s] from idx[%s]"), ids[v].c_str(), ikey.c_str());
				}
			}

			_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("auditIdx() cleaned %d/%d orphans from idx[%s]"), cCleaned, cOrphan, ikey.c_str());
		}
...
StoredObject::Ptr ObjectStore::add(const ServantPtr& servant, const std::string& id)
{
	...
	_log(ZQ::common::Log::L_INFO, STORELOGFMG("add() obj[%s] added"), id.c_str());
	return so;
}
...
ServantPtr ObjectStore::remove(const ::std::string& id)
{
	...
	if (NULL == so)
	{
		_log(ZQ::common::Log::L_DEBUG, STORELOGFMG("remove() obj[%s] not found, ignore"), id.c_str());
		return NULL;
	}
	...
	close(so, false);
	_log(ZQ::common::Log::L_INFO, STORELOGFMG("remove() obj[%s] removed dirty[%c]"), id.c_str(), bDirty?'T':'F');
	return servant;
}
