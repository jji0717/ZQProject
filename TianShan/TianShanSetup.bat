...
sub main()
{
    ...
    if (!(($ins_servicesname =~ /^MOD/i)||($ins_servicesname =~ /^MediaClusterCS/i)||($ins_servicesname =~ /^NSS/i)||($ins_servicesname =~ /^C2SS/i)||($ins_servicesname =~ /^TSPump/i)||($ins_servicesname =~ /^GBVSS/i)||($ins_servicesname =~ /^GBCS/i)))
    {
    	require "profile.pl";
	...
	$ins_servicesname = "C2SS" if($ins_servicesname =~ /^C2SS/i);
	...
	if($Exist_ins)
	{
	       	...
	       	$ins_servicesname = "C2SS"."$Exist_ins" if ($ins_servicesname=~ /^C2SS/i);
	...
	$ins_servicesname = "C2SS" if(lc($ins_servicesname) eq lc(C2SS));
	...
	$servicename = "C2SS" if($ins_servicesname =~ /^C2SS/i);
...
# get original installroot if upgrade the service
	  ...
	  if ((($ins_servicesname=~ /^MODSvc/i)||($ins_servicesname=~ /^MediaClusterCS/i)||($ins_servicesname=~ /^NSS/i)||($ins_servicesname=~ /^C2SS/i)||($ins_servicesname=~ /^TSPump/i)||($ins_servicesname =~ /^GBVSS/i)||($ins_servicesname =~ /^GBCS/i))&&($Exist_ins==1))
	  {
	 ...
	 if(!$ins_silent)
	 {
	  ...
	  if(($choise =~ /^Y/i)||($ins_silent))
	  {
	  	system ("net stop snmp 2>error") if(($key_snmp)&&(!$uncopy));
	  	#system ("ren $EXEDIR bin_bk") if(!$uncopy);
	        system ("$file");
	        SetRegistry(${$servicename}{Registry});
	        if((!$upgrade)&&(($ins_servicesname =~ /^MOD/i)||($ins_servicesname =~ /^MediaClusterCS/i)||($ins_servicesname =~ /^NSS/i)||($ins_servicesname =~ /^C2SS/i)||($ins_servicesname =~ /^TSPump/i)||($ins_servicesname =~ /^GBVSS/i)||($ins_servicesname =~ /^GBCS/i)))
	        {
    			$key="$key_title"."ZQ Interactive/TianShan/Currentversion/Services/$servicesname_shell";
    			$Registry->Delimiter("/");
    			$key = $Registry->{$key};    			
...
options
   ...
   --silent
   	silent install or upgrade service, do not prompt registry key
    --help
        display this screen
...
sub PromptRegistry($)
{
	...
	$$profile[$i][3] = prompt("$$profile[$i][1]","$$profile[$i][3]") if(!$ins_silent);
